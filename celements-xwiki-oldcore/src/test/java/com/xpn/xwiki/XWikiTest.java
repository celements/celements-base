...
public class XWikiTest extends AbstractBridgedXWikiComponentTestCase {
  ...
  public void testAuthorAfterDocumentCopy() throws XWikiException {
    ...
    this.xwiki.copyDocument(this.document.getName(), this.document.getSpace() + "." + copyName,
        getContext());
  ...
  public void testClearNameWithoutStripDotsWithoutAscii() {
    assertEquals("ee{&.txt",
        this.xwiki.clearName("\u00E9\u00EA{&.txt", false, false, getContext()));
  }
  ...
  public void testGetDocumentNameFromPath() {
    ...
    assertEquals("Space.Document",
        this.xwiki.getDocumentNameFromPath("/Space/Document", getContext()));
    assertEquals("Space.WebHome", this.xwiki.getDocumentNameFromPath("/view/Space/", getContext()));
    assertEquals("Space.Document",
        this.xwiki.getDocumentNameFromPath("/view/Space/Document", getContext()));
    assertEquals("Space.Document",
        this.xwiki.getDocumentNameFromPath("/view/Space/Document/", getContext()));
    assertEquals("Space.Document",
        this.xwiki.getDocumentNameFromPath("/view/Space/Document/some/ignored/paths",
            getContext()));
  ...
  public void testSaveDocumentSendsObservationEvents() throws Exception {
    ...
    mockListener.expects(once()).method("getEvents")
        .will(returnValue(
            Arrays.asList(new DocumentCreatedEvent(ref), new DocumentCreatingEvent(ref))));
    ...
    mockListener.expects(once()).method("onEvent").with(isA(DocumentCreatingEvent.class),
        same(document),
        isA(XWikiContext.class));
    mockListener.expects(once()).method("onEvent").with(isA(DocumentCreatedEvent.class),
        same(document),
        isA(XWikiContext.class));
  ...
  public void testDeleteDocumentSendsObservationEvents() throws Exception {
    ...
    mockListener.expects(once()).method("getEvents")
        .will(returnValue(
            Arrays.asList(new DocumentDeletedEvent(ref), new DocumentDeletingEvent(ref))));
  ...
  public void testGetCurrentContentSyntaxId() {
    ...
    assertEquals(Syntax.CREOLE_1_0.toIdString(),
        this.xwiki.getCurrentContentSyntaxId(null, getContext()));
  ...
  /**
   * Check that the user validation feature works when the validation key is stored both as plain
   * text and as a hashed
   * field.
   *
   * @throws Exception
   *           when any exception occurs inside XWiki
   */
  public void testValidationKeyStorage() throws Exception {
    ...
    validationKey = ((PropertyClass) this.xwiki.getUserClass(context).get("validkey"))
        .fromString("plaintextkey");
