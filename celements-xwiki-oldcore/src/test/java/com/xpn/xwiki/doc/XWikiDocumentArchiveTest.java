...
public class XWikiDocumentArchiveTest extends AbstractBridgedXWikiComponentTestCase {
  ...
  public void testUpdateArchiveWhenSpaceInUsername() throws Exception {
    ...
    archive.updateArchive(doc, XWikiRightService.GUEST_USER_FULLNAME, new Date(), "some comment",
        null, context);
  ...
  /**
   * Verify issue "When loading a revision of a document the creation date is incorrectly set as the
   * last
   * modification date".
   *
  ...
  public void testVerifyCreationDateWhenLoadingDocumentFromArchive() throws Exception {
    ...
    // Wait for 2 seconds and make a change. We'll then load the last revision and verify it has a
    // correct
    // creation date.
    Thread.sleep(1000L);
  ...
  public void testVerifyDiffAndFullRevisionAlgorithm() throws Exception {
    ...
    // When a new revision is added the new revision is always the full revision but the previous
    // one is
  ...
  private void addRevisionToHistory(XWikiDocumentArchive archive, XWikiDocument document,
      String content,
    ...
    archive.updateArchive(document, document.getAuthor(), document.getDate(), document.getComment(),
        null, context);
  }
}
