...
public final class XWikiLDAPConfig {
   ...
   * @param cfgName
   *          the name of the property in xwiki.cfg.
   * @param def
   *          default value.
   * @param context
   *          the XWiki context.
   ...
   *           error when trying to instantiate secure provider.
   * @since 1.5M1
   */
   ...
   * @param context
   *          the XWiki context.
   * @return the mapping between XWiki users and LDAP users. The key is the XWiki group, and the
   *         value is the list of
   *         mapped LDAP groups.
   */
  public Map<String, Set<String>> getGroupMappings(XWikiContext context) {
    Map<String, Set<String>> groupMappings = new HashMap<>();
    ...
    if (param.trim().length() > 0) {
      ...
      for (String element : mappingTable) {
        String mapping = element.trim();
  ...
  /**
   * Get mapping between XWiki users attributes and LDAP users attributes. The key in the Map is
   * lower cased to easily
   * support any case.
   *
   * @param attrListToFill
   *          the list to fill with extracted LDAP fields to use in LDAP search.
   * @param context
   *          the XWiki context.
  ...
  public Map<String, String> getUserMappings(List<String> attrListToFill, XWikiContext context) {
    ...
    if ((ldapFieldMapping != null) && (ldapFieldMapping.length() > 0)) {
      ...
      for (String field2 : fields) {
        String[] field = field2.split(USERMAPPING_XWIKI_LDAP_LINK);
        if (2 == field.length) {
          ...
          LOG.error("Error parsing ldap_fields_mapping attribute in xwiki.cfg: " + field2);
        }
      }
    }
  ...
  public int getCacheExpiration(XWikiContext context) {
    try {
      ...
      return (int) context.getWiki().ParamAsLong("xwiki.authentication.ldap.groupcache_expiration",
          21800);
    }
  }
   ...
   * @param context
   *          the XWiki context.
   * @return the pattern to resolve to find the password to use to connect to LDAP server. It is
   *         based on
   *         {@link MessageFormat}.
  ...
  /**
   * @param login
   *          the login provided by the user
   * @param password
  ...
  public String getLDAPBindDN(String login, String password, XWikiContext context) {
    return MessageFormat.format(getLDAPBindDN(context),
        XWikiLDAPConnection.escapeLDAPDNValue(login),
