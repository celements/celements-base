...
public class XWikiUserManagementToolsImpl extends XWikiDefaultPlugin
    implements XWikiPluginInterface, XWikiUserManagementTools {
  ...
  public String inviteUser(String name, String email, XWikiContext context) throws XWikiException {
    if (!isValidEmail(email)) {
      throw new PluginException(getName(), ERROR_XWIKI_EMAIL_INVALID_EMAIL,
    ...
    XWikiDocument doc = context.getWiki().getDocument(getInvitationMessageDocument(context),
        context);
    String message = prepareInvitationMessage(doc, name, userObj.getStringValue("password"), email,
  ...
  protected Document createUserDocument(String name, String email, XWikiContext context)
  ...
  private String prepareInvitationMessage(XWikiDocument doc, String name, String password,
      String email,
  ...
  public boolean resendInvitation(String email, XWikiContext context) throws XWikiException {
    ...
    XWikiDocument doc = context.getWiki().getDocument(getReinvitationMessageDocument(context),
        context);
    String message = prepareInvitationMessage(doc, getUserName(userdoc.getFullName(), context),
