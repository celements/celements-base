...
public class ObjectidDataSourceFactory implements DataSourceFactory {
  ...
  public DataSource create(Map params, XWikiContext context) throws DataSourceException {
    ...
    try {
      ...
      if (id != null) {
        ...
        throw new DataSourceException(
            "source=type:objectid implies the presence of an id argument");
      }
    ...
    BaseObject xobj;
    try {
      List list = context.getWiki().getStore().search(
          "from " + BaseObject.class.getName() + " as obj where obj.id='" + objectid + "'", 0, 0,
          context);
      if (list.size() == 0) {
        throw new DataSourceException("Object ID not found");
      }
      xobj = (BaseObject) list.get(0);
      List propertyList = context.getWiki().getStore().search(
          "from " + BaseProperty.class.getName() + " as p where p.id.id='" + objectid + "'", 0, 0,
          context);
