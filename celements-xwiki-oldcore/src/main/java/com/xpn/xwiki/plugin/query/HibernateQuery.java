...
import org.apache.jackrabbit.core.query.OrderQueryNode.OrderSpec;
...
public class HibernateQuery extends DefaultQuery {
  ...
  protected class XWikiHibernateQueryTranslator
    ...
    public Object visit(PathQueryNode node, Object data) {
      ...
      for (int i = 0; i < ps.length; i++) {
        ...
        if (qn_xwiki_attachment.equals(qncl) && (qname != null)) {
          lsqn.addPredicate(new RelationQueryNode(
              lsqn, fromJCRName("filename"), qname.getLocalName(),
        ...
        if (qncl != null) { // space
          ...
          if (qn_xwiki_document.equals(qncl)) {
            if (qspace != null) {
              lsqn.addPredicate(new RelationQueryNode(
                  lsqn, fromJCRName("web"), qspace.getLocalName(),
                  QueryConstants.OPERATION_EQ_GENERAL));
            }
            if (qname != null) {
              lsqn.addPredicate(new RelationQueryNode(
                  lsqn, fromJCRName("name"), qname.getLocalName(),
                  QueryConstants.OPERATION_EQ_GENERAL));
            }
      ...
      for (int i = 0; i < ps.length; i++) {
        ...
        final Object[] res = (Object[]) traverse(lsqn,
            new Object[] { "", qncl, lastobj, lastclass });
    ...
    private Object NAryVisit(NAryQueryNode node, Object data, String operand) {
      ...
      for (QueryNode operand2 : operands) {
        _userwhere.append(or);
        traverse(operand2, data);
        or = operand;
      }
      if (bracket) {
        _userwhere.append(")");
    ...
    public Object visit(TextsearchQueryNode node, Object data) { // jcr:contain
       ...
       * _userwhere.append(prop.getHqlName()).append(" LIKE ").append("'%").append(node.getQuery()).
       * append("%'");
       * else if (type==TYPE_LIST)
       * _userwhere.append("'").append(node.getQuery()).append("'").append(" in elements(").append(
       * prop.getHqlName()).append(")");
       * return data;
       */
    }
    ...
    public Object visit(RelationQueryNode node, Object data) {
      ...
      if (isMultyValue && isGeneralComp) {
        ...
        _userwhere.append(svalue.toString()).append(sop).append(" all elements(")
            .append(oprop.getHqlName()).append(')');
      } else {
    ...
    public Object visit(OrderQueryNode node, Object data) {
      ...
      for (OrderSpec spec : specs) {
        final ObjProperty oprop = getProp(spec.getProperty(), obj, objclass);
    ...
    public Object visit(DerefQueryNode node, Object data) {
      ...
      if (qn_xwiki_object.equals(parentclass) || qn_xwiki_attachment.equals(parentclass)) {
        ...
        throw new TranslateException(
            "jcr:deref is possible only from xwiki:object and xwiki:attachment");
      }
    }
    ...
    private RelationQueryNode getXWikiQNameRelation(LocationStepQueryNode par, String prop,
        QName qspace, QName qname) {
      if ((qspace == null) && (qname == null)) {} else if ((qspace != null) && (qname != null)) {
        return new RelationQueryNode(par, fromJCRName(prop),
            qspace.getLocalName() + "." + qname.getLocalName(),
            QueryConstants.OPERATION_EQ_GENERAL);
    ...
    class ObjFlexProperty extends ObjPropProperty {
      ...
      public ObjFlexProperty(String obj, Class objclass, Class baseclass, String prop,
          String hqlname, Class propertyclass) {
