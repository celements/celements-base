...
public final class RightsManager {
   ...
   */
  private DocumentReferenceResolver<String> explicitDocumentReferenceResolver = Utils
   ...
   *          </ul>
   * @param withdetails
   *          indicate if the methods return {@link List} or {@link String} or {@link List} of
   *          {@link XWikiDocument}.
   * @param limit
   *          the maximum number of result to return and index of the first element.
   * @param order
   ...
   *         <code>withdetails</code> is false,
  ...
  public List<?> getAllMatchedUsersOrGroups(boolean user, Object[][] matchFields,
    ...
    if (newstart < nbGlobalUsersOrGroups) {
      userOrGroupList
          .addAll(getAllMatchedGlobalUsersOrGroups(user, matchFields, withdetails, new RequestLimit(
    ...
    if (limit.getNb() > userOrGroupList.size()) {
      userOrGroupList
          .addAll(getAllMatchedLocalUsersOrGroups(user, matchFields, withdetails, new RequestLimit(
              limit.getNb() - userOrGroupList.size(), newstart), order, context));
    } else if (limit.getNb() <= 0) {
      userOrGroupList.addAll(
          getAllMatchedLocalUsersOrGroups(user, matchFields, withdetails, new RequestLimit(0,
  ...
  public List<?> getAllMatchedGlobalUsersOrGroups(boolean user, Object[][] matchFields,
    ...
    return getAllMatchedWikiUsersOrGroups(user, context.getMainXWiki(), matchFields, withdetails,
        limit, order,
        context);
  }
  ...
  public List<?> getAllMatchedWikiUsersOrGroups(boolean user, String wikiName,
    ...
    try {
      ...
      List<?> localGroupList = getAllMatchedLocalUsersOrGroups(user, matchFields, withdetails,
          limit, order, context);
  ...
  public Collection<String> getAllGroupsNamesForMember(String member, int nb, int start,
      XWikiContext context)
      throws XWikiException {
    return context.getWiki().getGroupService(context).getAllGroupsNamesForMember(member, nb, start,
        context);
  }
  ...
  public Collection<String> getAllMembersNamesForGroup(String group, int nb, int start,
      XWikiContext context)
      throws XWikiException {
    return context.getWiki().getGroupService(context).getAllMembersNamesForGroup(group, nb, start,
        context);
  }
   ...
   *          the index of the first found user to return.
   * @param orderAsc
   *          if true, the result is ordered ascendent, if false it descendant. If null no order is
   *          applied.
   * @param context
   *          the XWiki context.
   ...
   * @since 1.6M1
   */
  public Collection<String> getAllMatchedMembersNamesForGroup(String group, String matchField,
      int nb, int start,
      Boolean orderAsc, XWikiContext context) throws XWikiException {
    return context.getWiki().getGroupService(context).getAllMatchedMembersNamesForGroup(group,
        matchField, nb,
   ...
   * @param global
   *          indicate it is global rights (wiki or space) or document rights.
   * @param context
   *          the XWiki context.
   ...
   */
  private Map<String, LevelTree> getLevelTreeMap(XWikiDocument preferences,
   ...
   *           error when browsing rights preferences.
   */
  public Map<String, LevelTree> getLevelTreeMap(String spaceOrPage, List<String> levelsToMatch,
      XWikiContext context)
      throws XWikiException {
   ...
   *
   * @param rightsMap
   *          the {@link LevelTree} {@link Map} to fill.
   * @param levelInherited
   ...
   *          the levels names to check ("view", "edit", etc.).
   * @param direct
   *          if true fill the {@link LevelTree#direct} field, otherwise fill the
   *          {@link LevelTree#inherited}.
   * @param context
   *          the XWiki context.
   */
  private void fillLevelTreeMap(Map<String, LevelTree> rightsMap, List<String> levelInherited,
      BaseObject bobj,
      List<String> levelsToMatch, boolean direct, XWikiContext context) {
    List<String> users = ListClass.getListFromString(bobj.getStringValue(RIGHTSFIELD_USERS),
        RIGHTSLISTFIELD_SEP, false);
    List<String> groups = ListClass.getListFromString(bobj.getStringValue(RIGHTSFIELD_GROUPS),
        RIGHTSLISTFIELD_SEP, false);
    List<String> levels = ListClass.getListFromString(bobj.getStringValue(RIGHTSFIELD_LEVELS),
        RIGHTSLISTFIELD_SEP, false);
   ...
   *           error when browsing rights preferences.
   */
  private void fillLevelTreeMapInherited(Map<String, LevelTree> rightsMap,
      List<String> levelInheritedIn,
  ...
  private void fillLevelTreeMap(Map<String, LevelTree> rightsMap, XWikiDocument preferences,
      List<String> levelsToMatch, boolean global, boolean direct, XWikiContext context)
      throws XWikiException {
   ...
   * @param currentGlobal
   *          indicate if current preferences document is global.
   * @param context
   *          the XWiki context.
  ...
  public XWikiDocument getParentPreference(XWikiDocument currentPreference, boolean currentGlobal,
    ...
    if (currentGlobal) {
      if (currentPreference.getFullName().equals(WIKI_PREFERENCES)) {
        if (!context.isMainWiki()) {
          parentPreferences = context.getWiki().getDocument(
        ...
        if (parentspace.trim().length() > 0) {
          parentPreferences = context.getWiki()
              .getDocument(parentspace + SPACEPAGENAME_SEP + SPACE_PREFERENCES, context);
        } else {
   ...
   *          the document containing rights preferences.
   * @param levelName
   *          the level right name ("view", "delete"...).
   * @param global
   *          indicate it is global rights (wiki or space) or document rights.
   * @param context
   *          the XWiki context.
   ...
   * @throws XWikiException
   *           error when getting document from database.
   */
  private XWikiDocument getXWikiPreferencesDoc(String spaceOrPage, XWikiContext context)
    ...
    if (spaceOrPage != null) {
      ...
      if (xwikidoc.isNew()) {
        xwikidoc = context.getWiki()
            .getDocument(spaceOrPage + SPACEPAGENAME_SEP + SPACE_PREFERENCES, context);
      }
    } else {
   ...
   * @param allow
   *          indicate if user is removed from allow right or deny right.
   * @param comment
   *          the comment to use when saving preferences document.
   * @param context
   *          the XWiki context.
   * @throws XWikiException
   *           error when browsing rights.
   ...
   *
   * @param right
   *          the object containing the right preferences.
   * @param userOrGroupWiki
   *          the name of the wiki of the use or group.
   * @param userOrGroupSpace
   *          the name of the space of the use or group.
   * @param userOrGroupName
   *          the name of the use or group.
   * @param user
   *          indicate if it is a user or a group.
   * @param context
   *          the XWiki context.
  ...
  public boolean removeUserOrGroupFromRight(BaseObject right, String userOrGroupWiki,
    ...
    List<String> usersOrGroups = ListClass.getListFromString(right.getStringValue(userOrGroupField),
        RIGHTSLISTFIELD_SEP, false);
    ...
    if (needUpdate) {
      right.setStringValue(userOrGroupField,
          StringUtils.join(usersOrGroups.toArray(), RIGHTSLISTFIELD_JOIN));
    }
   ...
   *
   * @param rightsDocument
   *          the document containing the rights preferences.
   * @param userOrGroupWiki
   *          the name of the wiki of the use or group.
   * @param userOrGroupSpace
   *          the name of the space of the use or group.
  ...
  public boolean removeUserOrGroupFromRights(XWikiDocument rightsDocument, String userOrGroupWiki,
      String userOrGroupSpace, String userOrGroupName, boolean user, boolean global,
      XWikiContext context) {
    boolean needUpdate = false;
    ...
    if (vobj != null) {
      for (BaseObject bobj : vobj) {
        ...
        needUpdate |= removeUserOrGroupFromRight(bobj, userOrGroupWiki, userOrGroupSpace,
  ...
  public void removeUserOrGroupFromAllRights(String userOrGroupWiki, String userOrGroupSpace,
    ...
    String fieldTypeName = ((PropertyClass) rightClass.get(fieldName)).newProperty().getClass()
        .getSimpleName();

    StringBuffer where = new StringBuffer(
        ", BaseObject as obj" + ", " + fieldTypeName + " as prop where doc.fullName=obj.name"
    ...
    if ((context.getDatabase() == null)
      ...
      if ((userOrGroupSpace == null) || userOrGroupSpace.equals(DEFAULT_USERORGROUP_SPACE)) {
        ...
        parameterValues
            .add(HQLLIKE_ALL_SYMBOL + userOrGroupSpace + SPACEPAGENAME_SEP + userOrGroupName
                + HQLLIKE_ALL_SYMBOL);
      }
    } else {
    ...
    List<XWikiDocument> documentList = context.getWiki().getStore()
        .searchDocuments(where.toString(), parameterValues, context);
  ...
  /**
   * Remove "direct" rights for wiki, space or document. This means that after that inherited right
   * will be used.
   *
   * @param spaceOrPage
   *          the space of page where to get XWikiRights. If null get wiki rights.
   ...
   *           error when browsing rights.
   */
  public void removeDirectRights(String spaceOrPage, List<String> levelNames, String comment,
      XWikiContext context)
      throws XWikiException {
    XWikiDocument preferences = getXWikiPreferencesDoc(spaceOrPage, context);
   ...
   *
   * @param groupName
   *          the group name where to search for member.
   * @param memberName
  ...
  public boolean groupContainsMember(String groupName, String memberName,
    ...
    if (memberList == null) {
      memberList = context.getWiki().getGroupService(context).getAllMembersNamesForGroup(groupName,
          0, 0, context);
   ...
   *          the user or group
   * @param context
   *          the XWikiContext the XWiki context
  ...
  public Collection<DocumentReference> resolveUsers(DocumentReference userOrGroup,
    ...
    if (!document.isNew()) {
      ...
      DocumentReference userClassReference = new DocumentReference(document.getDatabase(),
          DEFAULT_USERORGROUP_SPACE, "XWikiUsers");
      ...
      DocumentReference groupClassReference = new DocumentReference(document.getDatabase(),
          DEFAULT_USERORGROUP_SPACE, "XWikiGroups");
      ...
      if (members != null) {
        for (BaseObject memberObj : members) {
          ...
          users.addAll(
              resolveUsers(this.explicitDocumentReferenceResolver.resolve(member, userOrGroup),
                  context));
        }
      }
    }
   ...
   *           error when getting documents
   */
  public Collection<DocumentReference> resolveUsers(String userOrGroup, XWikiContext context)
      throws XWikiException {
