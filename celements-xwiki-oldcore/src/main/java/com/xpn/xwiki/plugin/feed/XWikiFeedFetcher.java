...
public class XWikiFeedFetcher extends AbstractFeedFetcher {
  ...
  private SyndFeedInfo buildSyndFeedInfo(URL feedUrl, String urlStr, HttpMethod method,
      SyndFeed feed, int statusCode) throws MalformedURLException {
    ...
    if ((imHeader != null) && (imHeader.getValue().indexOf("feed") >= 0)
      ...
      if ((cache != null) && (statusCode == 226)) {
        // client is setup to use http delta encoding and the server supports it and has returned a
  ...
  private SyndFeed getFeed(SyndFeedInfo syndFeedInfo, String urlStr, HttpMethod method,
      int statusCode) throws IOException, HttpException, FetcherException, FeedException {
