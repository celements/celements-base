...
public class XWikiPluginManager {
  ...
  public void addPlugin(String name, String className, XWikiContext context) {
    ...
    try {
      ...
      args[1] = className;
      args[2] = context;
      Class<XWikiPluginInterface> pluginClass = (Class<XWikiPluginInterface>) Class
          .forName(className);
      ...
      LOG.error("Cannot initialize plugin [" + className + "]. This plugin will not be available.",
          ex);
    }
  }
  ...
  public XWikiAttachment downloadAttachment(XWikiAttachment attachment, XWikiContext context) {
    ...
    for (XWikiPluginInterface plugin : getPlugins("downloadAttachment")) {
      try {
        ...
        LOG.warn("downloadAttachment failed for plugin [" + plugin.getClassName() + "]: "
            + ex.getMessage());
      }
    }
    return attach;
