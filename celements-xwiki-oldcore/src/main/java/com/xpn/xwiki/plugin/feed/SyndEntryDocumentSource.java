...
public class SyndEntryDocumentSource implements SyndEntrySource {
  ...
  public static class PropertySelector {
     ...
     *
     * @param strRep
     *          a string like "ClassName_ObjectIndex_PropertyName", where class name and object
     *          index are
     *          optional
     */
  ...
  /**
   * Strategy instance parameters. Each concrete strategy can define its own (paramName, paramValue)
   * pairs. These
   ...
   * @throws XWikiException
   */
  public void sourceDocument(SyndEntry entry, Document doc, Map<String, Object> params,
  ...
  protected String getURI(Document doc, Map<String, Object> params, XWikiContext context)
      throws XWikiException {
  ...
  protected String getLink(Document doc, Map<String, Object> params, XWikiContext context)
      throws XWikiException {
  ...
  protected String getDefaultDescription(Document doc, Map<String, Object> params,
      XWikiContext context) {
  ...
  protected SyndContent getDescription(Document doc, Map<String, Object> params,
      XWikiContext context)
      throws XWikiException {
    String description;
    ...
    if (mapping == null) {
      ...
      description = doc.getRenderedContent(getStringValue(mapping, doc, context),
          doc.getSyntaxId());
    }
  ...
  protected List<SyndCategory> getCategories(Document doc, Map<String, Object> params,
      XWikiContext context)
      throws XWikiException {
  ...
  protected Date getDefaultPublishedDate(Document doc, Map<String, Object> params,
      XWikiContext context) {
  ...
  protected Date getDefaultUpdateDate(Document doc, Map<String, Object> params,
      XWikiContext context) {
    return doc.getDate();
  }
  ...
  protected String getDefaultAuthor(Document doc, Map<String, Object> params,
      XWikiContext context) {
  ...
  protected List<String> getDefaultContributors(Document doc, Map<String, Object> params,
      XWikiContext context) {
    XWiki xwiki = context.getWiki();
  ...
  protected List<String> getContributors(Document doc, Map<String, Object> params,
      XWikiContext context)
      throws XWikiException {
   ...
   * evaluated to a string with
   * the following format: "[item1,item2,...,itemN]".
   */
  protected List<Object> parseList(String mapping, Document doc, XWikiContext context)
      throws XWikiException {
    if (!isVelocityCode(mapping)) {
      return null;
    }
  ...
  protected String getStringValue(String mapping, Document doc, XWikiContext context)
    ...
    if (ps.getClassName() == null) {
      ...
      return xdoc.getObject(ps.getClassName(), ps.getObjectIndex())
          .getStringValue(ps.getPropertyName());
    }
  }
  ...
  protected Date getDateValue(String mapping, Document doc, XWikiContext context)
    ...
    if (ps.getClassName() == null) {
      ...
      return xdoc.getObject(ps.getClassName(), ps.getObjectIndex())
          .getDateValue(ps.getPropertyName());
    }
  }
   ...
   */
  protected List<Object> getListValue(String mapping, Document doc, XWikiContext context)
      throws XWikiException {
    XWikiDocument xdoc = context.getWiki().getDocument(doc.getFullName(), context);
    PropertySelector ps = new PropertySelector(mapping);
    if (ps.getClassName() == null) {
      ...
      return xdoc.getObject(ps.getClassName(), ps.getObjectIndex())
          .getListValue(ps.getPropertyName());
    }
  }
   ...
   *
   * @param xmlFragment
   *          the XML fragment to be cleaned up
   * @param config
  ...
  public static org.w3c.dom.Document tidy(String xmlFragment, Properties config) {
    ...
    // Even if we add a message listener we still have to redirect the output. Otherwise all the
    // messages will be
   ...
   *
   * @param node
   *          the root of the DOM sub-tree containing the text
   ...
   * only characters in
   * XML text nodes. The XML fragment is cleaned up before extracting the prefix to be sure that the
   * result is
   * well-formed.
   *
   * @param xmlFragment
   *          the full XML text
   * @param previewLength
   *          the maximum number of characters allowed in the preview, considering only the XML text
   *          nodes
   * @return a prefix of the given XML fragment summing at most <code>previewLength</code>
  ...
  /**
   * Extracts the first characters of the given HTML fragment, up to the given length limit, adding
   * only characters in
   * HTML text nodes. The HTML fragment is cleaned up before extracting the prefix to be sure the
   * result is
   * well-formed.
   *
   * @param htmlFragment
   *          the full HTML text
   * @param previewLength
   *          the maximum number of characters allowed in the preview, considering only the HTML
   *          text
   *          nodes
   * @return a prefix of the given HTML fragment summing at most <code>previewLength</code>
   *         characters in its text
   *         nodes
   */
   ...
   * @return a prefix of the <code>plainText</code> having at most <code>previewLength</code>
   *         characters
   */
  public static String getPlainPreview(String plainText, int previewLength) {
    ...
    return plainText.substring(0, spaceIndex);
  }
   ...
   * @throws XWikiException
   *           if the given object is neither a {@link Document} instance, a {@link XWikiDocument}
