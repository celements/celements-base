...
/**
 * Listener to user and groups events to apply related automatic task like cleaning groups and
 * rights objects.
 *
 * @version $Id$
...
public final class RightsManagerListener implements EventListener {
  ...
  public void onEvent(Event event, Object source, Object data) {
    ...
    if (!Utils.getComponent(RemoteObservationManagerContext.class).isRemoteState()) {
      ...
      if (document.getObject("XWiki.XWikiUsers") != null) {
        try {
          cleanDeletedUserOrGroup(userOrGroupWiki, userOrGroupSpace, userOrGroupName, true,
              context);
        ...
        try {
          cleanDeletedUserOrGroup(userOrGroupWiki, userOrGroupSpace, userOrGroupName, false,
              context);
        } catch (XWikiException e) {
   ...
   *          the wiki name of the group or user.
   ...
   *           error when browsing groups or rights.
   */
  private void cleanDeletedUserOrGroupInLocalWiki(String userOrGroupWiki, String userOrGroupSpace,
    ...
    RightsManager.getInstance().removeUserOrGroupFromAllRights(userOrGroupWiki, userOrGroupSpace,
        userOrGroupName,
        user, context);
    ...
    if (!context.getWiki().isVirtualMode()) {
      ...
      try {
        ...
        for (String wikiName : wikiList) {
          ...
          cleanDeletedUserOrGroupInLocalWiki(userOrGroupWiki, userOrGroupSpace, userOrGroupName,
              user,
              context);
        }
