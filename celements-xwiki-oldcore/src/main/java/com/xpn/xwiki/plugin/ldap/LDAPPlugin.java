...
public class LDAPPlugin extends XWikiDefaultPlugin implements XWikiPluginInterface {
  ...
  public HashMap search(String searchstr, String[] params, XWikiContext context)
      throws LDAPException {
    return search(searchstr, LDAPConnection.SCOPE_SUB, params, null, context);
  }

  public HashMap search(String searchstr, int scope, String[] params, XWikiContext context)
      throws LDAPException {
  ...
  public HashMap search(String searchstr, HashMap connection, XWikiContext context)
      throws LDAPException {
    return search(searchstr, LDAPConnection.SCOPE_SUB, null, connection, context);
  }

  public HashMap search(String searchstr, int scope, String[] params, HashMap connection,
  ...
  public HashMap getEntry(String dn, HashMap connection, XWikiContext context)
      throws LDAPException {
    LDAPConnection lc = connect(connection, context);
    try {
  ...
  /**
   * Method to create an XWiki user from LDAP information. Information is retrieved from the LDAP
   * server specified in
   * the XWiki Preferences. Bind to the LDAP server can be done using Admin binding (in this case
   * bindusernamd and
   * bindpassword are not used) or User binding (in this case bindusername and bindpassword are
   * used)
   *
   * @param wikiname
   *          Wiki page name to use for the user. If null it will be generated from LDAP
   * @param uid
   *          UID to search user information in LDAP
   * @param bindusername
   *          bind username if binding is user binding
   * @param bindpassword
   *          bind password if binding is user binding
   * @param context
   *          XWiki Context
   * @return success or failure of create user
   * @throws XWikiException
   */
  public boolean createUserFromLDAP(String wikiname, String uid, String bindusername,
      String bindpassword,
    ...
    try {
      ...
      if (lc != null) {
        ...
        LDAPSearchResults searchResults = lc.search(baseDN, LDAPConnection.SCOPE_SUB, searchquery,
            null, // return
                  // all
                  // attributes
  ...
  /**
   * Method allowing to create a user from LDAP attributes stored in a HashMap The mapping is
   * declared in the XWiki
   * Parameters The 'name' mapping is used to define which ldap field to use to define the wiki page
   * name All special
   * characters are cleared to generate the wiki page name
   *
   * @param wikiname
   *          Wiki page name to use. If null generate from ldap fields
   * @param attributes
   * @param context
   * @throws XWikiException
   */
