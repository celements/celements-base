...
public class ImagePlugin extends XWikiDefaultPlugin {
   ...
   * {@code xwiki.plugin.image.cache.capacity}.
   */
  ...
  public void init(XWikiContext context) {
    ...
    if (!StringUtils.isBlank(defaultQualityParam)
      ...
      try {
        this.defaultQuality = Math.max(0,
            Math.min(1, Float.parseFloat(defaultQualityParam.trim())));
      } catch (NumberFormatException e) {
        LOG.warn(String
            .format("Failed to parse xwiki.plugin.image.defaultQuality configuration parameter. "
                + "Using %s as the default image quality.", this.defaultQuality),
            e);
      }
    }
  }
  ...
  private void initCache(XWikiContext context) {
    ...
    if (!StringUtils.isBlank(capacityParam) && StringUtils.isNumeric(capacityParam.trim())) {
      try {
        ...
        LOG.warn(String
            .format("Failed to parse xwiki.plugin.image.cache.capacity configuration parameter. "
                + "Using %s as the cache capacity.", this.capacity),
            e);
      }
    }
   ...
   * {@inheritDoc}
   * <p>
   * Allows to scale images server-side, in order to have real thumbnails for reduced traffic. The
   * new image
   * dimensions are passed in the request as the {@code width} and {@code height} parameters. If
   * only one of the
   * dimensions is specified, then the other one is computed to preserve the original aspect ratio
   * of the image.
   *
   ...
   * @param width
   *          the desired image width; this value is taken into account only if it is greater than
   *          zero and less
   *          than the current image width
   * @param height
   *          the desired image height; this value is taken into account only if it is greater than
   *          zero and less
   ...
   * @throws Exception
   *           if transforming the image fails
   */
  private XWikiAttachment downloadImage(XWikiAttachment image, int width, int height, float quality,
    ...
    String key = String.format("%s;%s;%s;%s;%s;%s", image.getId(), image.getVersion(), width,
   ...
   *          {@code true} to preserve the image aspect ratio even when both requested dimensions
   *          are
   *          properly specified (in this case the image will be resized to best fit the rectangle
   *          with the
   ...
   * @throws Exception
   *           if shrinking the image fails
   */
  private XWikiAttachment shrinkImage(XWikiAttachment attachment, int requestedWidth,
      int requestedHeight,
    ...
    int[] dimensions = reduceImageDimensions(currentWidth, currentHeight, requestedWidth,
        requestedHeight, keepAspectRatio);
   ...
   */
  private int[] reduceImageDimensions(int currentWidth, int currentHeight, int requestedWidth,
      int requestedHeight,
