...
public class FeedPluginApi extends PluginApi<FeedPlugin> {
  ...
  static {
    ...
    BLOG_FIELDS_MAPPING.put(SyndEntryDocumentSource.FIELD_DESCRIPTION,
        "Blog.BlogPostClass_content");
    BLOG_FIELDS_MAPPING.put(SyndEntryDocumentSource.FIELD_CATEGORIES,
        "Blog.BlogPostClass_category");
    BLOG_FIELDS_MAPPING.put(SyndEntryDocumentSource.FIELD_PUBLISHED_DATE,
        "Blog.BlogPostClass_publishDate");
  ...
  public SyndFeed getFeeds(String sfeeds, boolean ignoreInvalidFeeds, boolean force)
      throws IOException {
  ...
  public SyndFeed getFeed(String sfeed, boolean ignoreInvalidFeeds, boolean force)
      throws IOException {
  ...
  public int updateFeeds(String feedDoc, boolean fullContent, boolean oneDocPerEntry)
      throws XWikiException {
    if (hasProgrammingRights()) {
      return getProtectedPlugin().updateFeeds(feedDoc, fullContent, oneDocPerEntry,
          getXWikiContext());
    } else {
      return -1;
    }
  ...
  public int updateFeeds(String feedDoc, boolean fullContent, boolean oneDocPerEntry, boolean force)
      throws XWikiException {
    if (hasProgrammingRights()) {
      return getProtectedPlugin().updateFeeds(feedDoc, fullContent, oneDocPerEntry, force,
          getXWikiContext());
    } else {
      return -1;
    }
  ...
  public int updateFeedsInSpace(String space, boolean fullContent, boolean force)
      throws XWikiException {
    if (hasProgrammingRights()) {
      return getProtectedPlugin().updateFeedsInSpace(space, fullContent, true, force,
          getXWikiContext());
    } else {
      return -1;
    }
  ...
  public int updateFeed(String feedname, String feedurl, boolean fullContent) {
    if (hasProgrammingRights()) {
      return getProtectedPlugin().updateFeed(feedname, feedurl, fullContent, true,
          getXWikiContext());
    } else {
      return -1;
    }
  ...
  public int updateFeed(String feedname, String feedurl, boolean fullContent,
    ...
    if (hasProgrammingRights()) {
      return getProtectedPlugin().updateFeed(feedname, feedurl, fullContent, oneDocPerEntry,
          getXWikiContext());
    } else {
      return -1;
    }
  ...
  public int updateFeed(String feedname, String feedurl, boolean fullContent,
      boolean oneDocPerEntry, boolean force) {
    if (hasProgrammingRights()) {
  ...
  public boolean startUpdateFeedsInSpace(String space, boolean fullContent, int scheduleTimer)
      throws XWikiException {
    return getProtectedPlugin().startUpdateFeedsInSpace(space, fullContent, scheduleTimer,
        this.context);
  }
  ...
  /**
   * Tries to instantiate a class implementing the {@link SyndEntrySource} interface using the given
   * parameters
   *
   * @param className
   *          the name of a class implementing {@link SyndEntrySource} interface
   * @param params
  ...
  public SyndEntrySourceApi getSyndEntrySource(String className, Map<String, Object> params) {
    ...
    try {
      SyndEntrySource source = getProtectedPlugin().getSyndEntrySource(className, params,
          getXWikiContext());
  ...
  /**
   * Creates a new instance of the default feed image. The default image file name is taken from the
   * <i>logo</i> skin
   ...
   * @param sourceApi
   *          the strategy to use for computing feed entries from source items
   * @param sourceParams
   *          strategy parameters
   * @return a new feed
   */
  public SyndFeed getFeed(List<Object> list, SyndEntrySourceApi sourceApi,
   ...
   *
   * @param list
   *          a list of {@link Document} objects, {@link XWikiDocument} objects or document names
   * @param params
   *          strategy parameters
   * @return a new feed
   * @see Document
  ...
  /**
   * Creates a new feed from a list of articles, using the default strategy for converting articles
   * in feed entries.
  ...
  /**
   * Creates a new feed from the result of an HQL query and a corresponding strategy for converting
   * the retrieved
   * documents in feed entries.
   *
   * @param query
   *          the HQL query used for retrieving the documents
   * @param count
   ...
   *
   * @param query
   *          the HQL query used for retrieving the articles
   * @param count
   ...
   *          strategy parameters
   * @param metadata
   *          feed meta data (includes the author, description, copyright, encoding, url, title)
   * @return a new feed
   */
  public SyndFeed getFeed(List<Object> list, SyndEntrySourceApi sourceApi,
      Map<String, Object> sourceParams,
    ...
    getXWikiContext().remove(FEED_PLUGIN_EXCEPTION);
    try {
      return getProtectedPlugin().getFeed(list, sourceApi.getSyndEntrySource(), sourceParams,
          metadata,
          getXWikiContext());
    } catch (XWikiException e) {
      getXWikiContext().put(FEED_PLUGIN_EXCEPTION, e);
   ...
   * Creates a new feed from a list of documents, using the default strategy for converting
   * documents in feed entries,
   * filling in the feed meta data. You can customize the default strategy by using strategy
   * parameters.
   *
   * @param list
   *          a list of {@link Document} objects, {@link XWikiDocument} objects or document names
   ...
   * Creates a new feed from a list of articles, using the default strategy for converting articles
   * in feed entries,
   * filling in the feed meta data. By articles we mean any document containing an
   * <code>XWiki.ArticleClass</code>
   * object.
   *
   * @param list
   *          a list of articles
  ...
  public SyndFeed getBlogFeed(List<Object> list, Map<String, Object> metadata) {
    Map<String, Object> params = Collections.emptyMap();
    SyndFeed blogFeed = getFeed(list,
        getSyndEntrySource(SyndEntryDocumentSource.class.getName(), BLOG_FIELDS_MAPPING), params,
  ...
  public SyndFeed getFeed(String query, int count, int start, SyndEntrySourceApi sourceApi,
    ...
    getXWikiContext().remove(FEED_PLUGIN_EXCEPTION);
    try {
      return getProtectedPlugin().getFeed(query, count, start, sourceApi.getSyndEntrySource(),
          sourceParams,
   ...
   * Creates a new feed from the result of an HQL query, using the default strategy for converting
   * documents in feed
   * entries, filling in the feed meta data. You can customize the default strategy by using
   * strategy parameters.
   *
   * @param query
   *          the HQL query used for retrieving the documents
  ...
  public SyndFeed getBlogFeed(String query, int count, int start, String blogPostClassName,
    ...
    if (query == null) {
      ...
      if ((category == null) || category.equals("")) {
        query = ", BaseObject as obj where obj.name=doc.fullName and obj.className='"
            + BLOG_POST_CLASS_NAME
            + "' and obj.name<>'" + BLOG_POST_TEMPLATE_NAME + "' order by doc.creationDate desc";
      } else {
        query = ", BaseObject as obj, DBStringListProperty as prop join prop.list list where obj.name=doc.fullName and obj.className='"
    ...
    if (blogPostClassName == null) {
      ...
      blogMappings.put(SyndEntryDocumentSource.FIELD_PUBLISHED_DATE,
          blogPostClassName + "_publishDate");
      blogMappings.put(SyndEntryDocumentSource.CONTENT_LENGTH, new Integer(400));
    }
    SyndFeed blogFeed = getFeed(query, count, start,
        getSyndEntrySource(SyndEntryDocumentSource.class.getName(), blogMappings),
   ...
   *
   * @param feed
   *          any type of feed, implementing the {@link SyndFeed} interface
   * @param type
   *          the feed type (syntax) to use, <b>null</b> if none. It can be any version of RSS or
   *          Atom. Some
   ...
   * @see #getFeed(List, SyndEntrySourceApi, Map, Map)
   */
  public String getFeedOutput(List<Object> list, SyndEntrySourceApi sourceApi,
      Map<String, Object> sourceParams,
      Map<String, Object> metadata, String type) {
    getXWikiContext().remove(FEED_PLUGIN_EXCEPTION);
    try {
      return getProtectedPlugin().getFeedOutput(list, sourceApi.getSyndEntrySource(), sourceParams,
          metadata,
   ...
   * @see SyndEntryDocumentSource
   */
  public String getDocumentFeedOutput(List<Object> list, Map<String, Object> params,
      Map<String, Object> metadata,
      String type) {
   ...
   * @see SyndEntryDocumentSource
   */
  public String getDocumentFeedOutput(String query, int count, int start,
   ...
   * @see SyndEntryArticleSource
   */
  public String getArticleFeedOutput(String query, int count, int start,
      Map<String, Object> metadata, String type) {
    Map<String, Object> params = Collections.emptyMap();
   ...
   * @see #getFeedOutput(SyndFeed, String)
   */
  public String getWebFeedOutput(String query, int count, int start, Map<String, Object> metadata,
      String type) {
   ...
   * @see #getFeedOutput(SyndFeed, String)
   */
  public String getBlogFeedOutput(String query, int count, int start, Map<String, Object> metadata,
      String type) {
