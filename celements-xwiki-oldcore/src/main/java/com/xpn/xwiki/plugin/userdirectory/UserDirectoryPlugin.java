...
public class UserDirectoryPlugin extends XWikiDefaultPlugin implements XWikiPluginInterface {
  ...
  private static Log mLogger = LogFactory.getLog(UserDirectoryPlugin.class);
  ...
  public String inviteToGroup(String name, String firstName, String email, String group,
    ...
    XWikiUserManagementTools userTools = (XWikiUserManagementTools) context.getWiki()
        .getPlugin("usermanagementtools", context);
    XWikiDocument doc = context.getWiki()
        .getDocument(userTools.getUserSpace(context) + "." + pageName, context);
    if (doc.isNew()) {
  ...
  private String prepareDeactivationMessage(String name, String email, String grp,
      XWikiContext context) throws XWikiException {
    XWikiDocument doc = context.getWiki().getDocument(getDeactivationMessageDocument(context),
        context);
