...
public class RightsManagerPlugin extends XWikiDefaultPlugin {
   ...
   * @param className
   *          the class name of the entry point of the plugin.
   * @param context
   *          the XWiki context.
   */
  ...
  public void init(XWikiContext context) {
    // Register a listener so that the RightsManager is called when a document with a XWikiUsers
    // object is
    // removed in order to remove that user from its groups.
    // Make sure we allow this plugin to be initialized several times in a row; i.e. don't
    // re-register the
