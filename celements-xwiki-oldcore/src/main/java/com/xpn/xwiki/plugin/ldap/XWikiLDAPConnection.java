...
public class XWikiLDAPConnection {
   ...
   *
   * @param ldapUserName
   *          the user name to connect to LDAP server.
   * @param password
   ...
   *           error when trying to open connection.
   */
   ...
   * @param loginDN
   *          the user DN to connect to LDAP server.
   * @param password
   *          the password to connect to LDAP server.
   * @param pathToKeys
   *          the path to SSL keystore to use.
   * @param ssl
   ...
   *
   * @param baseDN
   *          the root DN where to search.
   * @param filter
   *          the LDAP filter.
   * @param attr
   *          the attributes names of values to return.
   * @param ldapScope
   *          {@link LDAPConnection#SCOPE_SUB} oder {@link LDAPConnection#SCOPE_BASE}.
   * @return the found LDAP attributes.
   */
  public List<XWikiLDAPSearchAttribute> searchLDAP(String baseDN, String filter, String[] attr,
      int ldapScope) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(MessageFormat.format(
          "LDAP search: baseDN=[{0}] query=[{1}] attr=[{2}] ldapScope=[{3}]", baseDN,
