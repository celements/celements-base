...
public class ZipExplorerPlugin extends XWikiDefaultPlugin {
   ...
   * return a new
   * attachment containing the file pointed to inside the ZIP. If the original attachment does not
   * point to a ZIP file
   * or if it doesn't specify a location inside the ZIP then do nothing and return the original
   * attachment.
   *
   * @param attachment
  ...
  /**
   * @param document
   *          the document containing the ZIP file as an attachment
   * @param attachmentName
   *          the name under which the ZIP file is attached in the document
   * @param context
   *          not used
  ...
  /**
   * Finds the ZIP attachment with passed name from the passed document matching and parse the ZIP
   * to generate a list
   * of {@link com.xpn.xwiki.objects.classes.ListItem} elements representing a tree view of all
   ...
   *          the name under which the ZIP file is attached in the document
   * @param fileName
   *          the filename to concatenate at the end of the attachment URL
   * @param context
   *          not used
   * @return the attachment URL of the passed attachment located in the passed document to which the
   *         passed filename
   *         has been suffixed.
  ...
  /**
   * @param url
   *          the URL to parse and from which to extract the relative file location
   * @param action
   *          the XWiki requested action (for example "download", "edit", "view", etc).
   * @return the relative file location of a file in the ZIP file pointed to by the passed URL. The
   *         ZIP URL must be of
   *         the format <code>http://[...]/zipfile.zip/SomeDirectory/SomeFile.txt</code>. With the
   *         example above this
   *         method would return <code>SomeDirectory/SomeFile.txt</code>. Return an empty string if
   *         the zip URL
   *         passed.
  ...
  protected boolean isZipFile(InputStream filecontent) {
    ...
    try {
      ...
      // Reset the input stream to the beginning. This may be needed for further reading the
      // archive.
      try {
        filecontent.reset();
      } catch (IOException e) {
