...
public class BooleanClass extends PropertyClass {
  ...
  public void displaySelectEdit(StringBuffer buffer, String name, String prefix,
    ...
    if (getDefaultValue() == -1) {
      options = new option[] { new option("---", ""), new option(String1, "1"),
  ...
  public void displayCheckboxEdit(StringBuffer buffer, String name, String prefix,
    ...
    // If the (visible) checkbox is unchecked, it will not post anything back so the hidden input by
    // the same
   ...
   * <ol>
   * <li>let V = the internal value of the option, 0 1 or 2, T = the value of the displayType
   * meta-property, and D =
  ...
  public String displaySelectSearch(String name, String prefix, XWikiCriteria criteria,
    ...
    List<Number> selectlist = (selectArray != null) ? Arrays.asList(selectArray)
        : new ArrayList<>();
     ...
     * options[1].setSelected(true); else if (value==0) options[2].setSelected(true); } else { int
     * value =
     * getDefaultValue(); if (value==1) options[1].setSelected(true); else if (value==0)
     * options[2].setSelected(true); } } } catch (Exception e) { // This should not happen
     * e.printStackTrace(); }
     */
    select.addElement(options);
  ...
  public String displayCheckboxSearch(String name, String prefix, XWikiCriteria criteria,
     ...
     * Integer ivalue =
     * (Integer)prop.getValue(); if (ivalue!=null) { int value = ivalue.intValue(); if (value==1)
     * check.setChecked(true); else if (value==0) check.setChecked(false); } else { int value =
     * getDefaultValue();
     * if (value==1) check.setChecked(true); else check.setChecked(false); } }} catch (Exception e)
     * { // This should
