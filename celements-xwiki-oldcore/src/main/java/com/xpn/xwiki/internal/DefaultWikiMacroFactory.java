 ...
 */
@Component
public class DefaultWikiMacroFactory extends AbstractLogEnabled
    implements WikiMacroFactory, WikiMacroConstants {
   ...
   *
   * @param doc
   *          the {@link XWikiDocument} to look for a macro definition
   * @return the {@link WikiMacro} found inside the document
   * @throws WikiMacroException
   *           when an invalid macro definition or no macro definition was found
   */
  private WikiMacro buildMacro(XWikiDocument doc) throws WikiMacroException {
    ...
    WikiMacroVisibility macroVisibility = WikiMacroVisibility
        .fromString(macroDefinition.getStringValue(
            MACRO_VISIBILITY_PROPERTY));
    boolean macroSupportsInlineMode = (macroDefinition
        .getIntValue(MACRO_INLINE_PROPERTY) != 0);
    String macroContentType = macroDefinition.getStringValue(MACRO_CONTENT_TYPE_PROPERTY);
    String macroContentDescription = macroDefinition
        .getStringValue(MACRO_CONTENT_DESCRIPTION_PROPERTY);
    ...
    if (StringUtils.isEmpty(macroName)) {
      macroName = macroId;
      getLogger().debug(
          String.format("Incomplete macro definition in [%s], macro name is empty",
              documentReference));
    }
    ...
    if (StringUtils.isEmpty(macroDescription)) {
      getLogger().debug(
          String.format("Incomplete macro definition in [%s], macro description is empty",
              documentReference));
    }
    ...
    if (null != macroParameters) {
      for (BaseObject macroParameter : macroParameters) {
        ...
        boolean parameterMandatory = (macroParameter
            .getIntValue(PARAMETER_MANDATORY_PROPERTY) != 0);
        String parameterDefaultValue = macroParameter
            .getStringValue(PARAMETER_DEFAULT_VALUE_PROPERTY);
        ...
        parameterDescriptors
            .add(new WikiMacroParameterDescriptor(parameterName, parameterDescription,
    ...
    if (!macroContentType.equals(MACRO_CONTENT_EMPTY)) {
      contentDescriptor = new DefaultContentDescriptor(macroContentDescription,
          macroContentType.equals(MACRO_CONTENT_MANDATORY));
    }

    // Create macro descriptor.
    MacroId id = new MacroId(macroId, doc.getSyntax());
    MacroDescriptor macroDescriptor = new WikiMacroDescriptor(id, macroName, macroDescription,
