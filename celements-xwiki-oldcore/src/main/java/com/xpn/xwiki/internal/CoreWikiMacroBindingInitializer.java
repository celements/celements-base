 ...
 */
@Component("core")
public class CoreWikiMacroBindingInitializer extends AbstractLogEnabled
    implements WikiMacroBindingInitializer {

  /**
   * The key under which macro can access the document where it's defined. Same as
   * CONTEXT_DOCUMENT_KEY (Check style
   * fix).
   */
  ...
  public void initialize(DocumentReference macroDocumentReference, WikiMacroParameters parameters,
    ...
    try {
      XWikiDocument document = getContext().getWiki().getDocument(macroDocumentReference,
          getContext());
