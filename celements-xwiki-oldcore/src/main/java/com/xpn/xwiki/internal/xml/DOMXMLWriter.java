...
/**
 * This a minimal implementation to transform an XMLWriter into a
 * <code>{@link org.dom4j.dom.DOMDocument}</code>
 * builder.
 * <p>
 * This implementation allow the use of a same function accepting an XMLWriter, to either produce
 * output into an
 * <code>{@link java.io.OutputStream}</code> or to create a
 * <code>{@link org.dom4j.dom.DOMDocument}</code>. Here a
 ...
 * </pre></code>
 * <p>
 * <b>WARNING</b> - This implementation in INCOMPLETE and a minimal support. It should be improve as
 * needed over time
 * </p>
 *
 * @version $Id$
...
public class DOMXMLWriter extends XMLWriter {
  ...
  /**
   * Create a new <code>{@link DOMXMLWriter}</code> that will build into the provided document using
   * the default
   * encoding.
   *
   * @param doc
   *          <code>{@link Document}</code> that will be build by this writer
   */
   ...
   *          <code>{@link Document}</code> that will be build by this writer
   * @param format
   *          <code>{@link OutputFormat}</code> used to retrieve the proper encoding
   */
   ...
   * {@inheritDoc}
   * <p>
   * Add the element into the <code>{@link Document}</code> as a children of the element at the top
   * of the stack of
   * opened elements, putting the whole stream content as text in the content of the
   * <code>{@link Element}</code>. The
