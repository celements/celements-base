...
/**
 * Import action event listener to handle registering of the wiki macros after import. The problem
 * is with the wiki
 * macros imported in the empty wiki, which cannot be registered at the save of the document on
 * import time due to lack
...
public class RegisterMacrosOnImportListener extends AbstractLogEnabled implements EventListener {
  ...
  private final List<Event> eventsList = new ArrayList<>(
      Arrays.asList(new XARImportedEvent()));
  ...
  public void onEvent(Event event, Object source, Object data) {
    ...
    try {
      // clean the grouplist cache from the context since now, after import, the groups have
      // changed. All
      // cache saved on the context during the import (save listeners) should be cleaned since it
      // might not be
      // accurate.
      // FIXME: somehow, it's a bit of time loss to clean the context grouplist, especially when
      // this is done
      // for a subsequent import (not the initial import of the .xar), when macros are registered
      // anyway, there
