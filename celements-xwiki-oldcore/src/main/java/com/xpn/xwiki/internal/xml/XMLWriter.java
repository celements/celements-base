 ...
 * write out the tags
 * produced without building the document tree in memory. It is not compatible with the SAX part of
 * the original
 ...
 * tags by maintaining a state stack of opened tags. New writeDocumentStart/End function also ensure
 * proper starting and
...
public class XMLWriter extends org.dom4j.io.XMLWriter {
   ...
   * <code>{@link Element}</code> is the document root element, and the top of the stack is the last
   * opened
  ...
  /**
   * Create a new XMLWriter writing to a provided OutputStream in a given format. Note that other
   * constructor of the
   * original DOM4J XMLWriter are unsupported since a OutputStream is the only way we can support
   * the extensions
   * provided here.
   * <p>
   * Note that the writer is buffered and only a call to flush() or writeDocuemntEnd() will ensure
   * the output has been
   * fully written to the <code>{@link OutputStream}</code>.
   * </p>
   *
   * @param out
   *          an <code>{@link OutputStream}</code> where to output the XML produced.
   * @param format
   *          an <code>{@link OutputFormat}</code> defining the encoding that should be used and
   *          esthetics of the
   *          produced XML.
   ...
   *
   * @param doc
   *          <code>{@link Document}</code> to be started, may specify a
   *          <code>{@link DocumentType}</code>.
   * @throws IOException
   *           a problem occurs during writing
   */
  ...
  /**
   * Close all remaining opened <code>{@link Element}</code> including the root element to terminate
   * the current
   * document. Also flush the writer to ensure the whole document has been written to the
   * <code>{@link OutputStream}</code>.
   *
   * @param doc
   *          <code>{@link Document}</code> to be end, actually unused.
   * @throws IOException
   *           a problem occurs during writing.
   */
   ...
   * for its content.
   * <p>
   * Note that proper decoding/encoding will occurs during this operation, converting the encoding
   * of the Reader into
   * the encoding of the Writer.
   * </p>
   *
   * @param element
   *          <code>{@link Element}</code> to output.
   * @param rd
   *          <code>{@link Reader}</code> that will be fully read and transfered into the element
   *          content.
   * @throws IOException
   *           a problem occurs during reading or writing.
   */
   ...
   * <p>
   * Note that no decoding/encoding of the InputStream will be ensured during this operation. The
   * byte content is
   ...
   *          <code>{@link Element}</code> to output.
   * @param is
   *          <code>{@link InputStream}</code> that will be fully read and transfered into the
   *          element content.
   * @throws IOException
   *           a problem occurs during reading or writing.
   */
   ...
   *          <code>{@link Element}</code> to output.
   * @param is
   *          <code>{@link InputStream}</code> that will be fully read and encoded in Base64 into
   *          the element
   *          content.
   * @throws IOException
   *           a problem occurs during reading or writing.
   ...
   * <p>
   * Compared to the DOM4J implementation, this function ensure closing of all opened element
