 ...
 * <p>
 * Note: We extend {@link PdfExportImpl} for convenience. This is just a temporary solution. The
 * entire export code
...
public class OfficeExporter extends PdfExportImpl {
  ...
  /**
   * The object used to get the media type corresponding to a filename extension. This object knows
   * all the document
   * formats supported by the office converter.
   * <p>
   * Note: This is a hack! We rely on the fact that currently the office converter uses JODConverter
   * to convert
   * from/to office document formats and so we use its {@link DocumentFormatRegistry} class. We
   * can't expose this in
   * the office importer module because all its interfaces are independent of the underlying tool
   * used for conversion.
   * Writing our own classes to wrap {@link DocumentFormatRegistry} and {@link DocumentFormat} is
   * not worth at this
   * point.
   */
  ...
  /**
   * @param format
   *          the export format; usually this is a filename extension
   * @return the export type matching the specified format, or {@code null} if the specified format
   *         is not supported
   */
  ...
  public void exportXHtml(String xhtml, OutputStream out, ExportType type, XWikiContext context)
    ...
    exportXHtml(xhtml, out, documentFormatRegistry.getFormatByExtension(type.getExtension()),
        context);
  }
   ...
   *
   * @param xhtml
   *          the XHTML to be exported
   * @param out
   *          where to write the export result
   * @param format
   *          the office format to convert the XHTML to
   * @param context
   *          the XWiki context, used to access the mapping between images embedded in the given
   *          XHTML and their
   *          location on the file system
   * @throws XWikiException
   *           if the conversion fails
   */
  private void exportXHtml(String xhtml, OutputStream out, DocumentFormat format,
    ...
    try {
      Map<String, byte[]> ouput = documentConverter.convert(inputStreams, inputFileName,
          outputFileName);
      ...
      throw new XWikiException(XWikiException.MODULE_XWIKI_EXPORT,
          XWikiException.ERROR_XWIKI_APP_SEND_RESPONSE_EXCEPTION, String.format(
              "Exception while exporting to %s (%s).", format.getName(), format.getExtension()),
          e);
    }
  }

  /**
   * Adds the objects referenced in the exported XHTML to the map of input streams, allowing them to
   * be embedded in
   * the output office document.
   *
   * @param inputStreams
   *          the map of input streams that is passed to the office converter
   * @param context
   *          the XWiki context, used to access the mapping between images embedded in the given
   *          XHTML and their
   *          location on the file system
