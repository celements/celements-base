...
/**
 * Special URL Factory used during exports, which stores referenced attachments on the filesystem so
 * that they can be
...
public class PdfURLFactory extends XWikiServletURLFactory {
   ...
   */
  @Override
  public URL createAttachmentURL(String filename, String space, String name, String action,
      String querystring,
   ...
   */
  @Override
  public URL createAttachmentRevisionURL(String filename, String space, String name,
      String revision, String wiki,
   ...
   * @param space
   *          the name of the owner document's space
   * @param name
   *          the name of the owner document
   * @param filename
   *          the name of the attachment
   * @param revision
   *          an optional attachment version
   * @param context
   *          the current request context
   * @return a {@code file://} URL where the attachment has been stored
   * @throws Exception
   *           if the attachment can't be retrieved from the database and stored on the filesystem
   */
  public URL getURL(String wiki, String space, String name, String filename, String revision,
      XWikiContext context)
      throws Exception {
    ...
    if (!usedFiles.containsKey(key)) {
      ...
      XWikiDocument doc = context.getWiki().getDocument(
          new DocumentReference(StringUtils.defaultString(wiki, context.getDatabase()), space,
              name),
          context);
  ...
  private String getAttachmentKey(String space, String name, String filename, String revision) {
    try {
      ...
      return space + SEPARATOR + name + SEPARATOR + filename + SEPARATOR
          + StringUtils.defaultString(revision);
    }
  }
}
