...
public class DeleteAttachmentAction extends XWikiAction {
  ...
  public boolean action(XWikiContext context) throws XWikiException {
    ...
    if (request.getParameter("trashId") != null) {
      ...
      DeletedAttachment da = xwiki.getAttachmentRecycleBinStore().getDeletedAttachment(trashId,
          context, true);
      // If the attachment hasn't been previously deleted (i.e. it's not in the deleted attachment
      // store) then
      // don't try to delete it and instead redirect to the attachment list.
      if (da != null) {
        com.xpn.xwiki.api.DeletedAttachment daapi = new com.xpn.xwiki.api.DeletedAttachment(da,
            context);
        if (!daapi.canDelete()) {
    ...
    if (attachment.isImage(context)) {
      ...
      newdoc
          .setComment(context.getMessageTool().get("core.comment.deleteAttachmentComment", params));
    }

    newdoc.deleteAttachment(attachment, context);

    // Needed to counter a side effect of XWIKI-1982: the attachment is deleted from the
    // newdoc.originalDoc as well
