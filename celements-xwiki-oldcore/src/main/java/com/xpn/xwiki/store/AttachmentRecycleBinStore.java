 ...
 * in the recycle bin
 * using {@link #saveToRecycleBin(XWikiAttachment, String, Date, XWikiContext, boolean)}, restored
 * using
...
public interface AttachmentRecycleBinStore {
   ...
   * @param deleter
   *          The user which deleted the attachment.
   * @param date
   ...
   *
   * @return Restored attachment, or <code>null</code> if an entry with the requested ID does not
   *         exist.
   * @param attachment
   *          Optional attachment to restore. If a non-null value is passed, then this object will
   *          be changed
   *          to reflect the contents/history of the deleted attachment.
   * @param index
   *          What deleted attachment to restore. See {@link DeletedAttachment#getId()}.
   * @param context
   *          The current context.
   * @param bTransaction
   *          Should use old transaction (false) or create new (true).
   * @throws XWikiException
   *           If an error occurs while loading or restoring the attachment.
   ...
   *
   * @param attachment
   *          Optional attachment template. If <code>null</code>, return information about all
   *          deleted
   *          attachments from the database. Otherwise, filter by the document and filename provided
   *          in the passed
   *          attachment.
   * @param context
   *          The current context.
   ...
   * @param doc
   *          The document for which to retrieve deleted attachments.
   * @param context
   *          The current context.
   * @param bTransaction
   *          Should use old transaction (false) or create new (true).
