...
public class JcrQueryExecutor implements QueryExecutor {
  ...
  public <T> List<T> execute(final Query query) throws QueryException {
    ...
    try {
      ...
      return (List<T>) store.executeRead(getContext(), session -> {
  ...
  protected javax.jcr.query.Query createQuery(Query query, Session session)
      throws RepositoryException {
    return session.getWorkspace().getQueryManager().createQuery(createNativeStatement(query),
        query.getLanguage());
  }
}
