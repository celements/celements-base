...
public interface XWikiStoreInterface {
   ...
   *
   * @param wheresql
   *          Query to use, similar to the ones accepted by
   *          {@link #searchDocuments(String, XWikiContext)}. It
   ...
   * @since 2.2M2
   */
  List<DocumentReference> searchDocumentReferences(String wheresql, int nb, int start,
   ...
   */
  @Deprecated
  List<String> searchDocumentsNames(String wheresql, int nb, int start, String selectColumns,
      XWikiContext context)
      throws XWikiException;
   ...
   * @param context
   *          the XWiki context required for getting information about the execution context
  ...
  /**
   * Same as {@link #searchDocumentReferences(String, int, int, List, XWikiContext)} but returns all
   * rows.
   *
   ...
   *          {@link #searchDocuments(String, List, XWikiContext)}. It should not contain
   ...
   *          when a document has multiple version for each language it is returned as one document
   *          a
   *          language.
   * @param context
   *          the XWiki context required for getting information about the execution context.
   ...
   *          language.
   * @param customMapping
   *          inject custom mapping in session.
   * @param context
   *          the XWiki context required for getting information about the execution context.
   * @return a list of XWikiDocument.
   * @throws XWikiException
   ...
   *           in case of error while performing the query.
   */
  List<XWikiDocument> searchDocuments(String wheresql, boolean distinctbylanguage, int nb,
      int start,
      XWikiContext context) throws XWikiException;
   ...
   *
   * @param sql
   *          the HQL request clause. For example <code>" where doc.fullName
   *        <> ? and (doc.parent = ? or (doc.parent = ? and doc.space = ?))"</code>.
   * @param nb
   *          the number of rows to return. If 0 then all rows are returned.
   * @param start
   ...
   * query which will
   * automatically encode the passed values (like escaping single quotes). This API is recommended
   * to be used over the
   * other similar methods where the values are passed inside the where clause and for which you'll
   * need to do the
   ...
   *          the number of rows to skip. If 0 don't skip any row.
   * @param whereParams
   *          if not null add to <code>sql</code> a where clause based on a table of table
   *          containing field
  ...
  /**
   * @return QueryManager used for creating queries to store. Use QueryManager instead of #search*
   *         methods because it
