...
public class XWikiHibernateStore extends XWikiHibernateBaseStore implements XWikiStoreInterface {
            ...
            for (String oldclass : classes) {
              if (!oldclass.equals(nc.newProperty().getClass().getName())) {
                Query q = session.createQuery(
                    ...
                    .setString(0, bclass.getName()).setString(
                        1, nc.getName());
                for (Iterator it = q.list().iterator(); it.hasNext();) {
  ...
  public XWikiDocument loadXWikiDoc(XWikiDocument doc, XWikiContext context) throws XWikiException {
    ...
    try {
      ...
      if (doc.hasElement(XWikiDocument.HAS_OBJECTS)) {
        Query query = session
            .createQuery("from BaseObject as bobject where bobject.name = :name order by "
                + "bobject.number");
        ...
        while (it.hasNext()) {
          ...
          if ((classReference == null)
              || !object.getDocumentReference().equals(doc.getDocumentReference())) {
            continue;
          }
  ...
  public void loadXWikiCollection(BaseCollection object1, XWikiDocument doc, XWikiContext context,
    ...
    try {
      ...
      if (classReference != null) {
        ...
        Query query = session
            .createQuery("select prop.name, prop.classType from BaseProperty as prop where "
                + "prop.id.id = :id");
        query.setInteger("id", object.getId());
      ...
      Object[] args = { object.getName(), object.getClass(),
          Integer.valueOf(object.getNumber() + "") };
      throw new XWikiException(XWikiException.MODULE_XWIKI_STORE,
          XWikiException.ERROR_XWIKI_STORE_HIBERNATE_LOADING_OBJECT,
  ...
  @Deprecated
  public void deleteXWikiCollection(BaseCollection object, XWikiContext context,
      boolean bTransaction, boolean evict)
      throws XWikiException {
    if (object == null) {
      return;
    }
    try {
      ...
      throw new XWikiException(XWikiException.MODULE_XWIKI_STORE,
          ...
          "Exception while deleting object {0}", e,
          args);
    } finally {
      try {
        if (bTransaction) {
  ...
  public void saveXWikiClass(BaseClass bclass, XWikiContext context, boolean bTransaction)
    ...
    try {
      ...
      if (bclass.getFieldsToRemove().size() > 0) {
        for (Object element : bclass.getFieldsToRemove()) {
  ...
  public BaseClass loadXWikiClass(BaseClass bclass, XWikiContext context, boolean bTransaction)
    ...
    try {
      ...
      try {
        ...
        Query query = session
            .createQuery(
                "select prop.name, prop.classType from PropertyClass as prop where prop.id.id = :id order by prop.number asc");
        query.setInteger("id", bclass.getId());
        Iterator it = query.list().iterator();
        while (it.hasNext()) {
          Object obj = it.next();
  ...
  public void saveXWikiClassProperty(PropertyClass property, XWikiContext context,
    ...
    try {
      ...
      try {
        Query query = session
            .createQuery(
                "select prop.name from PropertyClass as prop where prop.id.id = :id and prop.id.name= :name");
        query.setInteger("id", property.getId());
        query.setString("name", property.getName());
        if (query.uniqueResult() == null) {
          isSave = true;
