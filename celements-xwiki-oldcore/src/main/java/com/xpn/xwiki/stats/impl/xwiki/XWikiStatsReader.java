...
public class XWikiStatsReader {
   ...
   * @param paramList
   *          the values to insert in the SQL query.
   ...
   * @param period
   *          the period of time, including its start date but excluding its end date.
   * @param step
  ...
  public Map<DateTime, Integer> getActionStatistics(String action, Scope scope, Period period,
    ...
    while (stepStart.compareTo(periodEnd) < 0) {
      ...
      List<DocumentStats> stats = getDocumentStatistics(action, scope,
          new Period(stepStart.getMillis(), stepEnd.getMillis()),
  ...
  public List<DocumentStats> getDocumentStatistics(String action, Scope scope, Period period,
    ...
    try {
      ...
      List<?> solist = store.search(query, range.getAbsoluteSize(), range.getAbsoluteStart(),
          paramList, context);
  ...
  public List<RefererStats> getRefererStatistics(String domain, Scope scope, Period period,
    ...
    XWikiHibernateStore store = context.getWiki().getHibernateStore();
    try {
      String query = MessageFormat.format("select referer, sum(pageViews) from RefererStats"
   ...
   * {@link com.xpn.xwiki.stats.impl.RefererStats}
   * instances.
   *
   * @param resultSet
   *          The result of a database query for referer statistics
  ...
  /**
   * Generate the HQL query used in
   * {@link #getVisitStatistics(String, Period, Range, XWikiContext)}.
   *
   * @param action
   *          the action the results should be ordered by. It can be one of: "view", "save" or
   *          "download". If the
   ...
   *          the sub-range to return from the entire result set. Use this parameter for pagination.
   * @param paramList
   *          the list of query parameters to fill.
   * @param context
   *          the XWiki context.
  ...
  private String createVisitStatisticsQuery(String action, Period period, Range range,
    ...
    if (action.equals(SaveAction.ACTION_NAME)) {
      ...
      query.append(MessageFormat.format(
          "order by sum(pageSaves) {0}, sum(pageViews) {0}, sum(downloads) {0}",
          sortOrder));
    }
   ...
   *
   * @param resultSet
   *          the result of a database query for visitor statistics.
   * @param startDate
   *          the start date used in the query.
   * @param endDate
  ...
  private List<VisitStats> getVisitStatistics(List<?> resultSet, DateTime startDate,
    ...
    for (Iterator<?> it = resultSet.iterator(); it.hasNext();) {
      ...
      VisitStats vs = new VisitStats(name, uniqueID, cookie, ip, userAgent,
          new Date(startDate.getMillis()), PeriodType.DAY);
