...
public class XWikiRCSArchive extends Archive {
   ...
   *
   * @param nodeInfos
   *          - collection of {@link XWikiRCSNodeInfo} in any order
   * @param context
   *          - for loading nodes content
   * @throws XWikiException
   *           if can't load nodes content
   */
  public XWikiRCSArchive(Collection<XWikiRCSNodeInfo> nodeInfos, XWikiContext context)
      throws XWikiException {
    super(new Object[0], "");
    nodes.clear();
    head = null;
  ...
  private static class XWikiJRCSNode extends TrunkNode {
     ...
     *          - version of node
     * @param next
     *          - next node (with smaller version) in history
    ...
    /**
     * @param other
     *          - create class from copying this node
     */
    ...
    /**
     * @return is this revision has old format. (xwiki-core<1.2, without author,comment,state
     *         fields)
     */
   ...
   * @param docId
   *          - docId which will be wrote in {@link XWikiRCSNodeId#setDocId(long)}
   * @throws PatchFailedException
   * @throws InvalidFileFormatException
  ...
  public Collection getNodes(long docId)
    ...
    for (Iterator it = nodes.values().iterator(); it.hasNext();) {
      ...
      if (!node.hasOldFormat()) {
        ...
        // If the archive node is old so there is no author, comment and date fields so we set them
        // using the
        ...
        try {
          ...
          // 1) Revision 1.1 doesn't exist. Some time in the past there was a bug in XWiki where
          // version
