...
public class XWikiAttachment implements Cloneable {
   ...
   * @param bWithVersions
   *          if true, all archived versions are also included
   * @param context
   *          current XWikiContext
   ...
   * {@link com.xpn.xwiki.internal.xml.XMLWriter}
   *
   * @param wr
   ...
   *          if true, binary content of the attachment is included (base64 encoded)
   * @param bWithVersions
   *          if true, all archive version is also included
   * @param context
   *          current XWikiContext
   * @throws IOException
   *           when an error occurs during streaming operation
   * @throws XWikiException
   *           when an error occurs during xwiki operation
   * @since 2.3M2
   */
  public void toXML(XMLWriter wr, boolean bWithAttachmentContent, boolean bWithVersions,
      XWikiContext context)
  ...
  public synchronized Version[] getVersions() {
    try {
      ...
      LOG.warn(String.format("Cannot retrieve versions of attachment [%s@%s]: %s", getFilename(),
          getDoc()
              .getFullName(),
          ex.getMessage()));
