 ...
 * <li>document language: if provided the proper language version of the document is loaded. If not
 * the default one is
 * loaded.</li>
 * <li>document version: if provided the proper version of the document is loaded. Also make extra
 * sure to bypass cache
 * storage for remote observation use case.</li>
 * </ul>
 * <p>
 * originalDocument remain the property of {@link LazyXWikiDocument} object and is not taken from
 * the lazy loaded
 * document since it depends on how this {@link XWikiDocument} object is used (its technical meaning
 * is its state in the
 * database before any modification but in the case of observation it's used as the previous version
 * of the document).
 * TODO: we should probably think about a separation of theses two notions in something more clear,
 * something for the
 * new model.
 *
 * @version $Id$
    ...
    if (this.document == null) {
      ...
      try {
        ...
        if (this.version == null) {
          ...
          // This is
          // safer for example when LazyXWikiDocument is used in the context of remote events. This
          // is for
          ...
          if (doc.getRCSVersion().equals(this.version)) {
            ...
            try {
              this.document = context.getWiki().getVersioningStore()
