...
/**
 * XWikiLinkFilter finds [text] in its input and transforms this to <a href="text">...</a> if the
 * wiki page exists. If
...
public class XWikiLinkFilter extends LocaleRegexTokenFilter {
  ...
  /**
   * The regular expression for detecting WikiLinks. Overwrite in subclass to support other link
   * styles like
  ...
  @Override
  public void handleMatch(StringBuffer buffer, org.radeox.regex.MatchResult result,
    ...
    if (engine instanceof WikiRenderEngine) {
      ...
      if (str != null) {
        ...
        /*
         * // We need to keep this in XWiki int colonIndex = name.indexOf(':'); // typed link ? if
         * (-1 !=
         * colonIndex) { // for now throw away the type information name = name.substring(colonIndex
         * + 1); }
         */
  ...
  /**
   * Returns the view of the wiki name that is shown to the user. Overwrite to support other views
   * for example
