...
/**
 * Builds a Radeox {@link Table} by parsing its textual representation, usually the content of a
 * {table} macro.
 *
 * @version $Id$
 */
public class TableBuilder {
  ...
  public static Table build(String content) {
    ...
    while (tokenizer.hasMoreTokens()) {
      ...
      // If a token contains [, then all tokens up to one containing a ] are concatenated. Kind of a
      // block marker.
      ...
      if ("\n".equals(token)) {
        ...
        if (!StringUtils.endsWith(lastToken, "\\")) {
          ...
          // If the last cell didn't contain any data, then it was skipped. Add a blank cell to
          // compensate.
        ...
        // Cell data
        if (!token.endsWith("\\")) {
          // If the cell data ends with \\, then it will be continued. Current data is stored in
          // lastToken.
