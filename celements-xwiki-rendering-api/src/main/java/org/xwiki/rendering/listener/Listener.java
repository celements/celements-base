...
/**
 * Contains callback events called when a document has been parsed and when it needs to be modified
 * or rendered. More
 * specifically when a document is parsed it generates an {@link org.xwiki.rendering.block.XDOM}
 * object. That object has
 * a {@link org.xwiki.rendering.block.XDOM#traverse(Listener)} method that accepts a
 * {@link Listener} object. For each
 * {@link org.xwiki.rendering.block.Block} element found in the document its
 * {@link org.xwiki.rendering.block.Block#traverse} method is called leading to the generation of
 * events from this
 * interface.
 * <p>
...
public interface Listener extends LinkListener, ImageListener {
  ...
  /**
   * Start a group of elements. Groups are used to allow using standalone elements in list items,
   * table cells, etc.
   ...
   * 
   * @param format
   *          the formatting type (bold, italic, etc)
   * @param parameters
   *          a generic list of parameters. Example: style="background-color: blue"
   * @see Format
   */
   ...
   * 
   * @param listType
   *          the type of list (bulleted, numbered, etc)
   * @param parameters
  ...
  /**
   * Start of a definition list description. For example in HTML this is the equivalent of
   * &lt;dd&gt;.
   * 
   * @since 1.6M2
   */
  ...
  /**
   * End of a definition list description. For example in HTML this is the equivalent of
   * &lt;/dd&gt;.
   * 
   * @since 1.6M2
   */
   ...
   * 
   * @param level
   *          the header level (1, 2, 3, etc)
   * @param id
   *          the header unique identifier
   * @param parameters
   *          a generic list of parameters. Example: style="background-color: blue"
   * @see org.xwiki.rendering.listener.HeaderLevel
   * @since 1.9M1
  ...
  /**
   * Start of marker containing a macro definition. This is a special that Macro Blocks emits when
   * they are executed
   * so that it's possible to reconstruct the initial macro syntax even after Macros have been
   * executed. This is used
   * for exemple by the WYSIWYG editor to let use see the result of executing a macro and still let
   * them modify the
   * macro definition.
   * 
   * @param name
   ...
   * 
   * @param parameters
   *          a generic list of parameters for the quotation. Example: "style"/"background-color:
   *          blue"
   */
  ...
  /**
   * A word. Note that sentences ar broken into different events: word events, special symbols
   * events, space events,
  ...
  /**
   * A special symbol ("*", "<", ">", "=", quote, etc). Any non alpha numeric character is a special
   * symbol.
   * 
   * @param symbol
  ...
  /**
   * A reference/location in a page. In HTML for example this is called an Anchor. It allows
   * pointing to that
   * location, for example in links. Note that there is no wiki syntax for this in general and it's
   * often generated by
   ...
   *          the string to protected from rendering
   * @param isInline
   *          if true the text content is located in a inline content (like paragraph, etc.)
   * @param parameters
   *          a generic list of parameters. Example: style="background-color: blue"
   */
  ...
  /**
   * Some text to inject directly into the listener output without parsing it. For example a HTML
   * macro could inject
   * directly some HTML entered by the user into the rendered HTML output. Note that it's not
   * recommended to use this
   * event in most cases since it can lead to invalid content being generated and in addition most
   * listener
   ...
   *          the text to inject
   * @param syntax
   *          the syntax in which the text is written. This is useful so that listener
