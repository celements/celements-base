...
public class XHTMLChainingRenderer extends AbstractChainingPrintRenderer {
  ...
  public void onVerbatim(String protectedString, boolean isInline, Map<String, String> parameters) {
    if (isInline) {
      // Note: We generate a tt element rather than a pre element since pre elements cannot be
      // located inside
      // paragraphs for example. There also no tag in XHTML that has a semantic of preserving inline
      // content so
  ...
  public void onEmptyLines(int count) {
    // We need to use a special tag for empty lines since in XHTML the BR tag cannot be used outside
    // of content
    // tags.
    // Note: We're using <div><div/> and not <div/> since some browsers do not support the <div/>
    // syntax (FF3)
