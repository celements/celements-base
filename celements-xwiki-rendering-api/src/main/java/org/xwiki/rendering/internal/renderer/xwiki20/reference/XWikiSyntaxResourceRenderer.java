...
public class XWikiSyntaxResourceRenderer {
  ...
  public void beginRenderLink(XWikiSyntaxEscapeWikiPrinter printer, ResourceReference reference,
    ...
    // find if the last printed char is part of a syntax (i.e. consumed by the parser before
    // starting to parse the
    // link)
  ...
  public boolean forceFullSyntax(XWikiSyntaxEscapeWikiPrinter printer, boolean isLastSyntax,
    ...
    // 3: it follows a character which is not a white space (newline/space) and is not consumed by
    // the parser (like
    // a another link)
    // 4: it's followed by a character which is not a white space (TODO: find a better way than this
    // endless list of
    ...
    return !isFreeStandingURI
        || !parameters.isEmpty()
        || (!isLastSyntax
            && !printer.isAfterWhiteSpace() && (!PlainTextStreamParser.SPECIALSYMBOL_PATTERN
                .matcher(
                    String.valueOf(
        ...
        || (nextEvent != null && nextEvent.eventType != EventType.ON_SPACE
            ...
            && nextEvent.eventType != EventType.END_QUOTATION_LINE
            && nextEvent.eventType != EventType.END_SECTION);
  }
