...
public class XWikiSyntaxEscapeHandler {
  ...
  public void escape(StringBuffer accumulatedBuffer, XWikiSyntaxListenerChain listenerChain,
    ...
    // Note: This needs to be the first replacement since other replacements below also use the
    // tilde symbol
    ...
    // When in a paragraph we need to escape symbols that are at beginning of lines and that could
    // be confused
    // with list items, headers or tables.
    if (blockStateListener.isInLine() && isOnNewLine()) {

      // Look for list pattern at beginning of line and escape the first character only (it's
      // enough)
      escapeFirstMatchedCharacter(LIST_PATTERN, accumulatedBuffer);

      // Look for header pattern at beginning of line and escape the first character only (it's
      // enough)
      escapeFirstMatchedCharacter(HEADER_PATTERN, accumulatedBuffer);

      // Look for table character patterns at beginning of line and escape the first character only
      // (it's enough)
      escapeFirstMatchedCharacter(TABLE_PATTERN, accumulatedBuffer);

      // Look for quote pattern at beginning of line and escape the first character only (it's
      // enough)
    ...
    // When in a header we need to escape "=" symbols since otherwise they would be confused for end
    // of section
    // characters.
    ...
    // Note: even though there are some restriction in the URI specification as to what character is
    // valid after
    // the ":" character following the scheme we only check for characters greater than the space
    // symbol for
    // simplicity.
    ...
    // - onWord("hello:") followed by onFormat(ITALIC) which would lead to "hello://" if the ":"
    // wasn't escaped
    ...
    if (linkLevel > 0) {
      // This need to be done after anything else because link label add another level of escaping
      // (escaped as
