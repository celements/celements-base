 ...
 * @version $Id$
 */
public class WikiManagerTest extends AbstractComponentTest {
  ...
  private Map<String, XWikiDocument> getDocuments(String database, boolean create)
    ...
    if (database == null) {
      database = getXContext().getDatabase();
    }
  ...
  public void prepare() throws Exception {
    ...
    getXContext().setDatabase(MAIN_WIKI_NAME);
    getXContext().setMainXWiki(MAIN_WIKI_NAME);

    this.databases.put(MAIN_WIKI_NAME, new HashMap<>());

    expect(getMock(XWiki.class).Param(isA(String.class))).andReturn("").anyTimes();

    Mock mockXWikiStore = mock(XWikiHibernateStore.class,
        new Class[] { XWiki.class, XWikiContext.class }, new Object[] {
            mockXWiki.proxy(), getXContext() });
    ...
    getXContext().setWiki((XWiki) mockXWiki.proxy());
  ...
  public void testGetDocument() throws XWikiException {
    ...
    XWikiDocument doc = this.wikiManager.getDocument(TARGET_WIKI_NAME, DOCFULLNAME, getXContext());

    assertFalse(doc.isNew());
    assertEquals(MAIN_WIKI_NAME, getXContext().getDatabase());
  }
  ...
  public void testGetWikiFromNameWhenInAnotherWiki() throws XWikiException {
    ...
    getXContext().setDatabase("anotherwiki");

    Wiki wiki = this.wikiManager.getWikiFromName("WikInamE", getXContext());
  ...
  public void testGetWikiAliasWhenDocumentDoesNorExists() throws XWikiException {
    try {
      this.wikiManager.getWikiAlias("WikInamE", 0, true, getXContext());
  ...
  public void testGetWikiAliasWhenDocumentDoesNotContainsClass() throws XWikiException {
    ...
    try {
      ...
      // getWikiAlias should throw XObjectDocumentDoesNotExistException when alias document does not
      // exists
      assertEquals(WikiManagerException.ERROR_WM_WIKIDOESNOTEXISTS, expected.getCode());
    }
  }
