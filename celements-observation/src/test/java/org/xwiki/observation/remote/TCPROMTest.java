...
public class TCPROMTest extends AbstractROMTestCase {
  ...
  /**
   * Validate sharing a simple Serializable event between two instances of
  ...
  public void testSerializableEvent() throws InterruptedException {
    ...
    this.context.checking(new Expectations() {

      {
        ...
        will(returnValue(Arrays.asList(event)));
        oneOf(localListener).onEvent(with(same(event)), with(equal("some source")),
            with(equal("some data")));
        oneOf(localListener).onEvent(with(same(event)), with(same(unserializable)),
            with(same(unserializable)));
        oneOf(remoteListener).onEvent(with(equal(event)), with(equal("some source")),
            with(equal("some data")));
      }
    });
