...
public class ObservationManagerTest {
  ...
  public void testRemoveListener() {
    ...
    this.context.checking(new Expectations() {

      {
        ...
        allowing(listener).getEvents();
        will(returnValue(Arrays.asList(event)));
        never(listener).onEvent(with(any(Event.class)), with(any(Object.class)),
  ...
  public void testAddEvent() throws Exception {
    ...
    this.context.checking(new Expectations() {

      {
        ...
        oneOf(listener).onEvent(with(any(Event.class)), with(any(Object.class)),
            with(any(Object.class)));

        // Since the observation returns the first matching event, return false from initialEvent so
        // that
  ...
  public void testRemoveEvent() {
    ...
    this.context.checking(new Expectations() {

      {
        ...
        // Since the observation returns the first matching event, return false from initialEvent so
        // that
        // the second event can be called (if there's a second event - in our case it'll be removed
        // but
  ...
  public void testRegisterSeveralListenersForSameEvent() {
    ...
    this.context.checking(new Expectations() {

      {
        ...
        will(returnValue(true));
        oneOf(listener1).onEvent(with(any(Event.class)), with(any(Object.class)),
            with(any(Object.class)));
        oneOf(listener2).onEvent(with(any(Event.class)), with(any(Object.class)),
            with(any(Object.class)));
      }
    });
  ...
  /**
   * Verify that we can register a listener with two events of the same type but with not matching
   * the same thing and it will receive both events.
   */
  @Test
  public void testRegisterListenerForTwoEventsOfSameType() {
    ...
    this.context.checking(new Expectations() {

      {
        ...
        oneOf(listener).onEvent(with(eventMatcher1), with(any(Object.class)),
            with(any(Object.class)));
        oneOf(listener).onEvent(with(eventMatcher2), with(any(Object.class)),
            with(any(Object.class)));
      }
    });
