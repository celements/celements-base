...
public abstract class AbstractFilterableEvent implements FilterableEvent, Serializable {
   ...
   * will match any other event of the same type.
   */
   ...
   *
   * @param name
   *          a generic name that uniquely identifies an event type
   */
  ...
  /**
   * <p>
   * This type of events match only events of the same type, and only if the internal
   * {@link #eventFilter}s also
  ...
  public boolean matches(Object otherEvent) {
    ...
    if (this.getClass().isAssignableFrom(otherEvent.getClass())) {
      isMatching = getEventFilter()
          .matches(((AbstractFilterableEvent) otherEvent).getEventFilter());
    }
    return isMatching;
  }
}
