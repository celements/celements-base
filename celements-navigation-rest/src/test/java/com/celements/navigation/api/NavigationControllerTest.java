...
public class NavigationControllerTest {
  ...
  @Before
  public void prepare() {
    requestResolver = createMock(NavigationRequestResolver.class);
    treeBuilder = createMock(NavigationTreeBuilder.class);
    mockMvc = MockMvcBuilders
        .standaloneSetup(new NavigationController(requestResolver, treeBuilder))
        .setControllerAdvice(new NavigationExceptionHandler()).build();
  }
  ...
  @Test
  public void getNavigation_returnsExactJsonAndPrivateNoStoreHeader() throws Exception {
    var request = request();
  ...
  @Test
  public void getNavigation_rejectsNonnumericInactiveLevel() throws Exception {
    replay(requestResolver, treeBuilder);
    var result = mockMvc
  ...
  @Test
  public void getNavigation_returnsStableSafeApiError() throws Exception {
  ...
  @Test
  public void getNavigation_returnsPrivateNoStoreHeaderForNotFound() throws Exception {
    var request = request();
    expect(requestResolver.resolve("Content", "Content.Missing", null, null, 0)).andReturn(request);
    expect(treeBuilder.build(request)).andThrow(new NavigationApiException(HttpStatus.NOT_FOUND,
        "navigation_node_not_found", "The navigation node was not found."));
    replay(requestResolver, treeBuilder);
    var result = mockMvc
  ...
  @Test
  public void getNavigation_mapsUnexpectedFailureWithoutLeakingDetails() throws Exception {
    var request = request();
    expect(requestResolver.resolve("Content", null, null, null, 0)).andReturn(request);
    expect(treeBuilder.build(request)).andThrow(new IllegalStateException("secret backend detail"));
    replay(requestResolver, treeBuilder);
