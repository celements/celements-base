 ...
 * <ul>
 * <li>Path-based multiwiki:
 * {@code http://server/(ignorePrefix)/wiki/wikiname/type/action/space/page/attachment}</li>
 * <li>Domain-based multiwiki:
 * {@code http://server/(ignorePrefix)/type/action/space/page/attachment}</li>
 * </ul>
 *
 * @version $Id$
 * @since 2.3M1
...
public class StandardXWikiURLFactory implements XWikiURLFactory<URL> {
   ...
   * <ul>
   * <li>"ignorePrefix": the starting part of the URL Path (i.e. after the Authority part) to
   * ignore. This is
   * useful for example for passing the Web Application Context (for a web app) which should be
   * ignored.
   * Example: "/xwiki".</li>
   * </ul>
   *
  ...
  public XWikiURL createURL(URL url, Map<String, Object> parameters) throws InvalidURLException {
    ...
    // Note that the reason is because we need to ignore the Servlet Context if this code is called
    // in a Servlet
    // environment and since the XWiki Application can be installed in the ROOT context, as well as
    // in any Context
    ...
    // The location of the wiki name depends on whether the wiki is configured to use domain-based
    // multiwiki or
    // path-based multiwiki. If domain-based multiwiki then extract the wiki reference from the
    // domain, otherwise
    ...
    // Note that the type could have been found already if the wiki was configured in path-based (in
    // this case
    ...
    try {
      builder = this.componentManager.lookup(XWikiURLBuilder.class,
          state.urlType.toString().toLowerCase());
  ...
  /**
   * Extract segments between "/" characters in the passed path. Also remove any path parameters
   * (i.e. content
   * after ";" in a path segment; for ex ";jsessionid=...") since we don't want to have these params
   * in the
   * segments we return and act on (otherwise we would get them in document names for example).
   *
   * Note that we only remove ";" characters when they are not URL-encoded. We want to allow the ";"
   * character to be
  ...
  protected List<String> extractPathSegments(String rawPath) {
    ...
    // Note that we use -1 in the call below in order to get empty segments too. This is needed
    // since in our
    ...
    for (String pathSegment : rawPath.split("/", -1)) {
      ...
      try {
        ...
        // "A sequence of escaped octets is decoded by replacing it with the sequence of characters
        // that it
        // represents in the UTF-8 character set. UTF-8 contains US-ASCII, hence decoding has the
        // effect of
        // de-quoting any quoted US-ASCII characters as well as that of decoding any encoded
        // non-US-ASCII
        // characters."
        ...
        throw new RuntimeException(
            "Failed to URL decode [" + normalizedPathSegment + "] using UTF-8.", e);
      }
   ...
   * For path-based multiwiki setup we get the path segment after the first segment, if this first
   * segment has the
   * predefined {@link org.xwiki.url.standard.StandardURLConfiguration#getWikiPathPrefix()} value.
   * If not then we
  ...
  protected WikiReference extractWikiReference(URI uri, URLParsingState state) {
    String host = null;
    // If the first path element isn't the value of the wikiPathPrefix configuration value then we
    // fall back
    // to the host name. This also allows the main wiki URL to be domain-based even for a
    // path-based multiwiki.
    if (this.configuration.isPathBasedMultiWiki()
        && state.urlSegments.get(0).equalsIgnoreCase(this.configuration.getWikiPathPrefix())) {
      host = state.urlSegments.get(1);
      // Remove the first 2 segments so that when this method returns the remaining URL segments
      // point to
