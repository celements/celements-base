...
public class EntityXWikiURLBuilder implements XWikiURLBuilder {
   ...
   * {@inheritDoc}
   *
   * @see org.xwiki.url.standard.XWikiURLBuilder#build(org.xwiki.model.reference.WikiReference,
   *      java.util.List)
   */
  @Override
  public XWikiURL build(WikiReference wikiReference, List<String> pathSegments) {
    ...
    // - 1 segment (e.g. "/", "/Document"): default space, specified document (and default if
    // empty), "view" action
    // - 2 segments (e.g. "/Space/", "/Space/Document"): specified space, document (and default doc
    // if empty), "view" action
    // - 3 segments (e.g. "/action/Space/Document"): specified space, document (and default doc if
    // empty), specified action
    // - 4 segments (e.g. "/download/Space/Document/attachment"): specified space, document and
    // attachment (and default doc if empty), "download" action
    // - 4 segments or more (e.g. "/action/Space/Document/whatever/else"): specified space, document
    // (and default doc if empty), specified "action" (if action != "download"), trailing segments
    // ignored
    ...
    // Normalize the extracted space/page to resolve empty/null values and replace them with default
    // values.
