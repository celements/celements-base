...
/**
 * Information about a deleted attachment in the recycle bin. Note that this does not hold much
 * information about the
 * real attachment, but only meta-information relevant to the trash: original document and filename,
 * deleter, deletion
...
public class DeletedAttachment extends Api {
  ...
  /**
   * Retrieve the internal entry index, used to uniquely identify this entity in the trash. This is
   * needed because a
   * file can be attached and deleted multiple times, so the document name and filename are not
   * enough to uniquely
   ...
   *
   * @return the attachment as it was before being deleted, and as it currently is in the recycle
   *         bin
   */
  public Attachment getAttachment() {
    try {
      Document doc = this.context.getWiki().getDocument(getDocName(), this.context)
          .newDocument(this.context);
      return new Attachment(doc, this.deletedAttachment.restoreAttachment(null, this.context),
          this.context);
   ...
   *
   * @return original deleted attachment if the current user has programming rights, else
   *         {@code null}.
   */
  @Programming
  ...
  public boolean canDelete() {
    try {
      ...
      LOG.warn(
          "Exception while checking if entry [" + getId() + "] can be removed from the recycle bin",
          ex);
      return false;
    }
  }
  ...
  /**
   * Permanently delete this attachment from the trash. Throws an access denied exception if the
   * user does not have
   * the right to perform this action, which will trigger the generic Access Denied message. Any
   * other failures will
   * be silently ignored.
   *
