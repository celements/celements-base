...
public class Api {
   ...
   *
   * @see #getXWikiContext()
   * @todo make this variable private after we agree on it on the mailing list. It'll break non-core
   *       plugins but
  ...
  /**
   * Get the current context. For the moment, this is a crucial part of the request lifecycle, as it
   * is the only
   * access point to all the components needed for handling a request. Note: This method is
   * protected so that users of
  ...
  /**
   * Check if the current document has programming rights, meaning that it was last saved by a user
   * with the
   ...
   *
   * @return <code>true</code> if the current user has the <code>admin</code> right or
  ...
  public boolean hasAccessLevel(String right, String docname) throws XWikiException {
    com.xpn.xwiki.XWiki xwiki = this.context.getWiki();
    return xwiki.getRightService().hasAccessLevel(right, this.context.getUser(), docname,
        this.context);
  }
   ...
   * If {@link Context#dropPermissions()} has been called then this will return the guest user no
   * matter
   * who the real author is.
   * If there is no current document then the guest user is returned because there is no reason for
   * script to
  ...
  /**
   * @return true if the code has dropped it's permissions by calling
   *         {@link Context#dropPermissions()}
   */
