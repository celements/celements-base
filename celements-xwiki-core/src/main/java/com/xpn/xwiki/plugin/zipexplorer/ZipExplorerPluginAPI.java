 ...
 * that it's possible
 * to display contents found inside ZIP files. This plugin accepts specially formatted URLs pointing
 * to files inside ZIP
 * files by using the following syntax:
 * <code>http://[...]/download/Document/zipfile.zip/SomeDirectory/SomeFile.txt</code>. In this
 * example, the URL points
 * to the <code>SomeFile.txt</code> file located in a directory named <code>SomeDirectory</code>
 * inside a ZIP file named
...
public class ZipExplorerPluginAPI extends PluginApi<ZipExplorerPlugin> {
  ...
  /**
   * @param plugin
   *          the ZIP Explorer plugin that this class is hiding.
   * @param context
