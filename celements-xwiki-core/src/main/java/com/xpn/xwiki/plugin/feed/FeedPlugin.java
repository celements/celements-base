...
public class FeedPlugin extends XWikiDefaultPlugin implements XWikiPluginInterface {
  ...
  public void init(XWikiContext context) {
    ...
    this.refreshPeriod = (int) context.getWiki().ParamAsLong("xwiki.plugins.feed.cacherefresh",
        3600);
  ...
  public SyndFeed getFeeds(String sfeeds, boolean ignoreInvalidFeeds, boolean force,
    ...
    for (String feed2 : feeds) {
      SyndFeed feed = getFeed(feed2, ignoreInvalidFeeds, force, context);
      if (feed != null) {
  ...
  public SyndFeed getFeedForce(String sfeed, boolean ignoreInvalidFeeds, XWikiContext context)
      throws IOException {
    try {
  ...
  public int updateFeeds(String feedDoc, boolean fullContent, XWikiContext context)
  ...
  public int updateFeeds(String feedDoc, boolean fullContent, boolean oneDocPerEntry, boolean force,
    ...
    for (BaseObject obj : objs) {
      if (obj != null) {
        ...
        nbfeeds++;
        int nb = updateFeed(feedDoc, feedname, feedurl, fullContent, oneDocPerEntry, force, space,
            context);
        if (nb != -1) {
          total += nb;
        } else {
        ...
        if (updateThread != null) {
          ...
          updateThread
              .setNbLoadedFeedsErrors(nbfeedsErrors + updateThread.getNbLoadedFeedsErrors());
        }
  ...
  public int updateFeedsInSpace(String space, boolean fullContent, boolean oneDocPerEntry,
      boolean force,
      XWikiContext context) throws XWikiException {
    // Make sure we have this class
    getAggregatorURLClass(context);

    String sql = ", BaseObject as obj where doc.fullName=obj.name and obj.className='XWiki.AggregatorURLClass' and doc.space='"
        + space + "'";
    int total = 0;
    List<String> feedDocList = context.getWiki().getStore().searchDocumentsNames(sql, context);
    if (feedDocList != null) {
      for (String feedDocName : feedDocList) {
  ...
  public int updateFeed(String feedname, String feedurl, boolean fullContent,
    ...
    return this.updateFeed("", feedname, feedurl, fullContent, oneDocPerEntry, force, space,
        context);
  }

  public int updateFeed(String feedDocumentName, String feedname, String feedurl,
      boolean fullContent, boolean oneDocPerEntry, boolean force,
      String space, XWikiContext context) {
    try {
      // Make sure we have this class
      getFeedEntryClass(context);
      ...
      if (feed != null) {
        ...
        return saveFeed(feedDocumentName, feedname, feedurl, feed, fullContent, oneDocPerEntry,
            force, space, context);
      } else {
        return 0;
      }
  ...
  private int saveFeed(String feedDocumentName, String feedname, String feedurl, SyndFeed feed,
      boolean fullContent, boolean oneDocPerEntry,
      ...
      if (oneDocPerEntry) {
        ...
        if (doc.isNew() || force) {
          ...
          if (!StringUtils.isBlank(
              context.getWiki().Param("xwiki.plugins.feed.creationDateIsPublicationDate"))) {
            // Set the creation date to the feed date if it exists, otherwise the current date
            doc.setCreationDate(
                (entry.getPublishedDate() == null) ? new Date() : entry.getPublishedDate());
          }
          if (StringUtils.isBlank(doc.getContent())) {
            this.prepareFeedEntryDocument(doc, context);
          }
  ...
  /**
   * Prepares a feed entry document by setting its content, syntax and creator according to the
   * configuration.
   *
   * @param document
   *          the document to prepare as a feed entry document
   * @param context
   *          the XWiki context when preparing the feed entry document
   */
  private void prepareFeedEntryDocument(XWikiDocument document, XWikiContext context) {
    ...
    if (StringUtils.isBlank(context.getWiki().Param("xwiki.plugins.feed.entryContent"))) {
      ...
      content = StringUtils.equals(Syntax.XWIKI_1_0.toIdString(), syntaxId)
    ...
    String feedEntryCreator = context.getWiki().getXWikiPreference("feed_documentCreator",
        "xwiki.plugins.feed.documentCreator",
    ...
    // Let the document try to create the Syntax object from the syntax id. If it fails (for example
    // if the syntax
    // ID precised in configuration
    // does not correspond to any syntax) it will fallback on the document default Syntax ID (which
    // is a XWiki core
    // configuration entry).
  ...
  private void saveEntry(String feedname, String feedurl, SyndEntry entry, XWikiDocument doc,
      boolean fullContent,
      XWikiContext context) throws XWikiException {
  ...
  private void saveEntry(String feedname, String feedurl, SyndEntry entry, XWikiDocument doc,
    ...
    try {
      ...
      // If some day wiki syntax is supported in document titles, we might want to convert to wiki
      // syntax instead.
      ...
      if ((url != null) && (!url.trim().equals(""))) {
        try {
          ...
          obj.setLargeStringValue("fullContent",
              (sfullContent.length() > 65000) ? sfullContent.substring(0,
                  65000) : sfullContent);
        } catch (Exception e) {
          obj.setLargeStringValue("fullContent",
              "Exception while reading fullContent: " + e.getMessage());
        }
      } else {
  ...
  public List<com.xpn.xwiki.api.Object> search(String query, XWikiContext context)
      throws XWikiException {
    ...
    if (queryTab.length > 0) {
      ...
      for (String element : queryTab) {
        sql += " and (prop.value LIKE '%" + element + "%' or lprop.value LIKE '%" + element
            + "%')";
      }
      ...
      for (Object obj[] : res) {
        try {
          ...
          if (context.getWiki().getRightService().checkAccess("view", doc, context)) {
            BaseObject bObj = doc.getObject("XWiki.FeedEntryClass", ((Integer) obj[0]));
  ...
  public com.xpn.xwiki.api.Object getFeedInfosbyGuid(String guid, XWikiContext context)
      throws XWikiException {
    return getFeedInfosbyGuid("XWiki.FeedList", guid, context);
  }

  public com.xpn.xwiki.api.Object getFeedInfosbyGuid(String feedDoc, String guid,
      XWikiContext context)
      throws XWikiException {
    XWikiDocument doc = context.getWiki().getDocument(feedDoc, context);
    Vector<BaseObject> objs = doc.getObjects("XWiki.AggregatorURLClass");
   ...
   */
  public SyndEntrySource getSyndEntrySource(String className, Map<String, Object> params,
      XWikiContext context)
      throws XWikiException {
    try {
      Class<? extends SyndEntrySource> sesc = Class.forName(className)
          .asSubclass(SyndEntrySource.class);
      ...
      throw new XWikiException(XWikiException.MODULE_XWIKI_PLUGINS,
          XWikiException.ERROR_XWIKI_UNKNOWN, "", t);
    }
  }
   ...
   */
  public SyndFeed getFeed(List<Object> list, SyndEntrySource source,
   ...
   */
  public SyndFeed getFeed(String query, int count, int start, SyndEntrySource source,
      Map<String, Object> sourceParams, Map<String, Object> metadata, XWikiContext context)
      throws XWikiException {
   ...
   */
  public String getFeedOutput(List<Object> list, SyndEntrySource source,
      Map<String, Object> sourceParams,
   ...
   *          the string to strip the HTML tags from
  ...
  private String stripHtmlTags(String originalString) throws ConversionException {
    ...
    this.syntaxConverter.convert(new StringReader(originalString), Syntax.HTML_4_01,
