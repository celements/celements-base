...
public class XWikiLDAPUtils {
   ...
   *
   * @param connection
   *          the XWiki LDAP connection tool.
   */
   ...
   *
   * @param configuration
   *          the configuration to use to create the cache and to find it if it's already created.
   * @param context
   *          the XWiki context.
   * @return the cache.
   ...
   * @param memberMap
   *          the result: maps DN to member id.
   * @param subgroups
   *          return all the subgroups identified.
   * @param context
   *          the XWiki context.
   */
   ...
   *
   * @param groupDN
   *          the group to retrieve the members of and scan for subgroups. If <code>groupDN</code>
   *          is a user DN
  ...
  public boolean getGroupMembers(String groupDN, Map<String, String> memberMap,
    ...
    if (searchAttributeList == null) {
      ...
      searchAttributeList = searchUserAttributesByUid(fixedDN,
          new String[] { LDAP_FIELD_DN, getUidAttributeName() });
   ...
   *           error when getting the group cache.
   */
   ...
   * @param groupDN
   *          the DN of the group where to search.
   * @param context
   *          the XWiki context.
   * @return true if provided members in the provided group.
   * @throws XWikiException
   *           error when searching for group members.
   */
  public boolean isMemberOfGroup(String memberDN, String groupDN, XWikiContext context)
      throws XWikiException {
   ...
   *
   * @param memberDN
   *          the DN to find in the provided groups.
   * @param groupDNList
   *          the list of DN of the groups where to search.
   * @param context
   *          the XWiki context.
   ...
   *           error when searching for group members.
   */
  public boolean isMemberOfGroups(String memberDN, Collection<String> groupDNList,
      XWikiContext context)
      throws XWikiException {
  ...
  public static CacheConfiguration getGroupCacheConfiguration(XWikiContext context) {
    if (cacheConfigurationGroups == null) {
      ...
      cacheConfigurationGroups.put(EntryEvictionConfiguration.CONFIGURATIONID, lru);
    }
   ...
   *           error when getting the group cache.
   */
  public String isUidInGroup(String userName, String groupDN, XWikiContext context)
      throws XWikiException {
    String userDN = null;
  ...
  public List<XWikiLDAPSearchAttribute> searchUserAttributesByUid(String uid,
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Searching for the user in LDAP: user:" + uid + " base:" + this.baseDN + " query:"
          + filter
          + " uid:" + uidAttributeName);
    }

    return this.connection.searchLDAP(this.baseDN, filter, attributeNameTable,
