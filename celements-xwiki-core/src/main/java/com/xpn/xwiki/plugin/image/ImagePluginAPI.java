...
public class ImagePluginAPI extends PluginApi<ImagePlugin> {
   ...
   *
   * @param imagePlugin
   *          the underlying image plugin that is exposed by this API
   * @param context
   *          the XWiki context
   */
  ...
  public int getHeight(String pageName, String attachmentName) {
    try {
      return getProtectedPlugin().getHeight(getAttachment(pageName, attachmentName),
          getXWikiContext());
    } catch (Exception e) {
      LOG.error(String.format("Failed to detect the height of %s attached to %s.", attachmentName,
          pageName), e);
      return -1;
    }
  }
  ...
  public int getWidth(String pageName, String attachmentName) {
    try {
      return getProtectedPlugin().getWidth(getAttachment(pageName, attachmentName),
          getXWikiContext());
    } catch (Exception e) {
      LOG.error(String.format("Failed to detect the width of %s attached to %s.", attachmentName,
          pageName), e);
      return -1;
    }
  }
   ...
   *          the name of a wiki page
   * @param attachmentName
   *          the name of an attachment of the specified page
   * @return the specified attachment
   * @throws XWikiException
   *           if retrieving the attachment fails
   */
  private XWikiAttachment getAttachment(String pageName, String attachmentName)
      throws XWikiException {
    DocumentReference documentReference = currentMixedDocumentReferenceResolver.resolve(pageName);
    XWikiDocument document = getXWikiContext().getWiki().getDocument(documentReference,
        getXWikiContext());
