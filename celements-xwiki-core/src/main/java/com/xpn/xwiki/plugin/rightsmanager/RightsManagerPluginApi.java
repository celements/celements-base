...
public class RightsManagerPluginApi extends PluginApi<RightsManagerPlugin> {
   ...
   *
   * @param plugin
   *          the entry point of the Rights Manager plugin.
   * @param context
   *          the XWiki context.
   */
  ...
  public Collection<String> getAllGroupsNamesForMember(String member) throws XWikiException {
    ...
    try {
      memberList = RightsManager.getInstance().getAllGroupsNamesForMember(member, 0, 0,
          this.context);
  ...
  public Collection<String> getAllMatchedMembersNamesForGroup(String group, String matchField,
    ...
    try {
      memberList = RightsManager.getInstance().getAllMatchedMembersNamesForGroup(group, matchField,
          nb, start, orderAsc,
          this.context);
    } catch (RightsManagerException e) {
      logError(MessageFormat.format("Try to get all matched member of group [{0}]",
          group), e);
