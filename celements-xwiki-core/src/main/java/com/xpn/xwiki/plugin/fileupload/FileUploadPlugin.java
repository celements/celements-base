...
public class FileUploadPlugin extends XWikiDefaultPlugin implements XWikiPluginInterface {
   ...
   * <tt>upload_maxsize</tt>
   * XWiki preference.
   */
  private static final long UPLOAD_DEFAULT_MAXSIZE = 33554432L;

  /**
   * The default maximum size for in-memory stored uploaded documents. If a file is larger than this
   * limit, it will be
   ...
   *           An XWikiException is thrown if the request could not be parsed.
   * @see FileUploadPluginApi#loadFileList()
   */
  public void loadFileList(XWikiContext context) throws XWikiException {
    XWiki xwiki = context.getWiki();
    loadFileList(
        xwiki.getXWikiPreferenceAsLong(UPLOAD_MAXSIZE_PARAMETER, UPLOAD_DEFAULT_MAXSIZE, context),
        (int) xwiki
            .getXWikiPreferenceAsLong(UPLOAD_SIZETHRESHOLD_PARAMETER, UPLOAD_DEFAULT_SIZETHRESHOLD,
                context),
   ...
   * @param uploadSizeThreashold
   *          Threashold over which the file data should be stored on disk, and not in memory.
   * @param tempdir
   *          Temporary directory to store the uploaded files that are not kept in memory.
   * @param context
   *          Context of the request.
   * @throws XWikiException
   *           if the request could not be parsed, or the maximum file size was reached.
  ...
  public void loadFileList(long uploadMaxSize, int uploadSizeThreashold, String tempdir,
    ...
    // Make sure the factory only ever creates file items which will be deleted when the jvm is
    // stopped.
    DiskFileItemFactory factory = new DiskFileItemFactory() {
      ...
      public FileItem createItem(String fieldName, String contentType, boolean isFormField,
          String fileName) {
        try {
          final DiskFileItem item = (DiskFileItem) super.createItem(fieldName, contentType,
   ...
   * {@link #loadFileList(XWikiContext)}
   ...
   * {@link #loadFileList(XWikiContext)} needs to be
   * called beforehand.
   *
   * @param formfieldName
   *          The name of the form field.
   ...
   * @since 2.3M2
   */
  public InputStream getFileItemInputStream(String formfieldName, XWikiContext context)
      throws IOException {
  ...
  /**
   * Retrieve the size of a file content in byte. {@link #loadFileList(XWikiContext)} needs to be
   * called beforehand.
   *
   * @param formfieldName
   *          The name of the form field.
  ...
  public String getFileName(String formfieldName, XWikiContext context) {
    ...
    // We need to strip the file path. See
