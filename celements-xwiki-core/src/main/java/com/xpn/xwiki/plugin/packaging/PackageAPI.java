...
public class PackageAPI extends Api {
  ...
  /**
   * Indicate if the current user has the right to import a package as a backup pack. In this
   * implementation, to be
   * able to import has backup pack the user must have the admin right on the XWiki.XWikiPreferences
   * document from
   * the main wiki (xwiki:XWiki.XWikiPreferences). The goal is to prevent local wiki administrators
   * from importing
   * documents saved with a global administrator as the author (rights escalation).
   *
   * @return true if the current user has the rights to import a package as a backup pack, false
   *         otherwise
   */
  ...
  /**
   * Sets the flag for the packager to preserve or not existing versions of documents when
   * installing
   * with {@link #install()}. If set to true, the existing history revisions of documents will be
   * preserve,
   ...
   * This flag will be ignored if {@link #isWithVersions()} flag is set to true. This means it's not
   * possible
   * to import with versions, preserving the existing document history. The behavior of the packager
   * in this case
   * fall backs on just adding a new version to the exsting history (ignoring the history from the
   * package).
   *
   * @param withVersions
  ...
  /**
   * Similar to {@link #Import(byte[])}, except expected errors are catch. This version should be
   * privileged
   * when using the packager API from velocity scripts since it will not display stack-trace in case
   * of error
  ...
  /**
   * Load a package in memory from an InputStream. It may be installed later using
   * {@link #install()}.
   *
   * @param is
