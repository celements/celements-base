...
public class QueryPlugin extends XWikiDefaultPlugin implements IQueryFactory {
  ...
  /**
   * @deprecated This version si buggy since it use the initial context of the plugin and
   *             not the current context. For the store itself this is not major as it is a
   *             singleton
   */
  @Override
  @Deprecated
  ...
  protected QueryRootNode parse(String query, String language) throws InvalidQueryException {
    ...
    return QueryParser.parse(query, language,
        XWikiNamespaceResolver.getInstance());
  }
    ...
    if (isHibernate(qf)) {
      try {
        ...
        throw new XWikiException(XWikiException.MODULE_XWIKI_PLUGINS,
            XWikiException.ERROR_XWIKI_UNKNOWN, "Invalid xpath query: " + q, e);
      }
    }
    if (isJcr(qf)) {
   ...
   *
   * @param web,
   *          docname - document.space & .name. it may consist xpath []-selection. if any - *
   * @param prop
   *          - return property, start with @, if null - return document
   * @param order
   *          - properties for sort, separated by ','; order: ascending/descending after prop. name,
   *          or +/- before. if null - not sort
   * @throws XWikiException
   * @deprecated This version si buggy since it use the initial context of the plugin and
   *             not the current context
   */
  ...
  public IQuery getDocs(String docname, String prop, String order, IQueryFactory qf)
      throws XWikiException {
  ...
  public IQuery getChildDocs(String docname, String prop, String order, IQueryFactory qf)
      throws XWikiException {
    return xpath("/*/*[@parent='" + getXWikiName(docname) + "']" + getPropertyXPath(prop)
   ...
   *
   * @param oweb,
   *          oclass - object web & class. if any - *
   * @param prop.
  ...
  public IQuery getObjects(String docname, String oclass, String prop, String order,
      IQueryFactory qf) throws XWikiException {
    return xpath("/" + getXPathName(docname) + "/obj/" + getXPathName(oclass)
        + getPropertyXPath(prop) + getOrderXPath(order), qf);
  }
  ...
  public String makeQuery(XWikiQuery query) throws XWikiException {
    ...
    if (classes.size() > 1) {
      throw new XWikiException(XWikiException.MODULE_XWIKI_STORE,
          XWikiException.ERROR_XWIKI_STORE_SEARCH_NOTIMPL,
          "Search with more than one class is not implemented");
    }
    if (classes.size() == 0) {
      return "";
    }
    ...
    if ((oProps != null) && (oProps.size() > 0)) {
      for (Object oProp : oProps) {
        OrderClause clause = (OrderClause) oProp;
  ...
  public List search(XWikiQuery query, IQueryFactory qf) throws XWikiException {
    return xpath(makeQuery(query), qf).list();
    /*
     * if (doclist==null)
     * return null;
