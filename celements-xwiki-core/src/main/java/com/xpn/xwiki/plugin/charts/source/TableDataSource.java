...
public class TableDataSource extends DefaultDataSource implements DataSource {
  ...
  public TableDataSource(BaseObject defObject, XWikiContext context) throws DataSourceException {
    init(defObject.getName(), defObject.getIntValue(TABLE_NUMBER),
        defObject.getStringValue(RANGE) == ""
            ? DEFAULT_RANGE
            : defObject.getStringValue(RANGE),
        defObject.getIntValue(HAS_HEADER_ROW) == 1, defObject
            .getIntValue(HAS_HEADER_COLUMN) == 1,
  ...
  private void init(String docName, int tableNumber, String range, boolean hasHeaderRow,
      boolean hasHeaderColumn,
      String decimalSymbolSelector, boolean ignoreAlpha, XWikiContext context)
    ...
    try {
      ...
      makeHeaders(table);
    } catch (NumberFormatException | XWikiException e) {
    ...
    if (decimalSymbolSelector != null) {
      if (decimalSymbolSelector.equals(COMMA_SELECTOR)) {
        ...
        throw new DataSourceException(
            "Invalid argument " + DECIMAL_SYMBOL + " for parameter source:"
  ...
  private Number toNumber(String str) throws NumberFormatException {
    ...
    for (int i = 0; i < str.length(); i++) {
      ...
      if (('0' <= ch) && (ch <= '9')) {
        ...
        if ((sb.length() > 0) && (i < (str.length() - 1)) && ('0' <= str.charAt(i - 1))
            && (str.charAt(i - 1) <= '9')
  ...
  /**
   * Valid string formats: Xm-Yn => rectangular range X-Y => whole columns range m-n => whole rows
   * range * => enire
