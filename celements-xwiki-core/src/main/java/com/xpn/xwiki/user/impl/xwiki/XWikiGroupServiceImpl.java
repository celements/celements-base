...
public class XWikiGroupServiceImpl implements XWikiGroupService, EventListener {
   ...
   * {@inheritDoc}
   *
   * @see com.xpn.xwiki.user.api.XWikiGroupService#init(com.xpn.xwiki.XWiki,
   ...
   * @return {@code true} if the group is implicit and all users should be by default in it,
   *         {@code false} if the
   *         group behaves as all other groups, containing only the users/subgroups that are
   *         explicitly listed inside
   *         the document.
   */
   ...
   * {@inheritDoc}
   *
   * @see com.xpn.xwiki.user.api.XWikiGroupService#listGroupsForUser(java.lang.String,
   *      com.xpn.xwiki.XWikiContext)
   */
  @Override
  public Collection<String> listGroupsForUser(String member, XWikiContext context)
   ...
   * {@inheritDoc}
   *
   * @see com.xpn.xwiki.user.api.XWikiGroupService#addUserToGroup(java.lang.String,
   *      java.lang.String,
  ...
  public void addUserToGroup(String username, String database, String group, XWikiContext context)
    ...
    list.add(
        this.currentMixedDocumentReferenceResolver.resolve(group, new WikiReference(database)));
  }
   ...
   *
   * @param currentMember
   *          the member name found in XWiki.XWikiGroups object.
   * @param memberWiki
   ...
   *
   * @param groupDocument
   *          the {@link XWikiDocument} containing group object.
   * @param memberWiki
   *          the name of the wiki of the member.
   * @param memberSpace
   *          the name of the space of the member.
  ...
  public void removeUserOrGroupFromAllGroups(String memberWiki, String memberSpace,
    ...
    if ((context.getDatabase() == null) || context.getDatabase().equalsIgnoreCase(memberWiki)) {
      if ((memberSpace == null) || memberSpace.equals(DEFAULT_MEMBER_SPACE)) {
        ...
        parameterValues.add(
            HQLLIKE_ALL_SYMBOL + memberSpace + SAME_NAME_SEP + memberName + HQLLIKE_ALL_SYMBOL);
      }
    } else {
      parameterValues.add(
          HQLLIKE_ALL_SYMBOL + memberWiki + WIKI_FULLNAME_SEP + memberName + HQLLIKE_ALL_SYMBOL);
    }
   ...
   * {@inheritDoc}
   *
   * @see com.xpn.xwiki.user.api.XWikiGroupService#listMemberForGroup(java.lang.String,
   *      com.xpn.xwiki.XWikiContext)
   */
  @Override
  @Deprecated
  public List<String> listMemberForGroup(String group, XWikiContext context) throws XWikiException {
    ...
    try {
      if (group == null) {
        try {
          return context.getWiki().getStore().getQueryManager().getNamedQuery("getAllUsers")
              .execute();
  ...
  public void onEvent(Event event, Object source, Object data) {
    ...
    if ((document.getObject(CLASS_XWIKIGROUPS) != null)
        || (oldDocument.getObject(CLASS_XWIKIGROUPS) != null)) {
      flushCache();
    }
  }
   ...
   * {@inheritDoc}
   *
   * @see com.xpn.xwiki.user.api.XWikiGroupService#getAllMatchedUsers(java.lang.Object[][], boolean,
   *      int, int,
   ...
   *
   * @param user
   *          if true search for users, otherwise search for groups.
   * @param matchFields
   ...
   *          the list of values to fill for use with HQL named request.
   * @return the formated HQL named request.
   */
  protected String createMatchUserOrGroupWhereClause(boolean user, Object[][] matchFields,
      Object[][] order,
   ...
   *          </ul>
   * @param withdetails
   *          indicate if a {@link List} containing {@link String} names is returned or {@link List}
   *          containing {@link XWikiDocument}.
   * @param nb
   *          the maximum number of results to return. Infinite if 0.
   * @param start
   *          the index of the first found user or group to return.
   * @param order
   *          the field to order from. It is a table of table with :
   *          <ul>
   *          <li>fieldname : the name of the field</li>
  ...
  protected List<?> getAllMatchedUsersOrGroups(boolean user, Object[][] matchFields,
    ...
    if (context.getWiki().getHibernateStore() != null) {
      ...
      if (withdetails) {
        groups = context.getWiki().getStore().searchDocuments(where, false, nb, start,
  ...
  /**
   * Create a query to filter provided group members. Generate the entire HQL query except the
   * select part.
   *
   * @param groupFullName
   ...
   *          applied.
   * @param parameterValues
   *          the values to insert in names query.
   * @return the HQL query.
   * @since 1.6M1
   */
  protected String createMatchGroupMembersWhereClause(String groupFullName, String matchField,
      Boolean orderAsc,
    ...
    if (matchField != null) {
      ...
      parameterValues.put("matchfield",
          HQLLIKE_ALL_SYMBOL + matchField.toLowerCase() + HQLLIKE_ALL_SYMBOL);
    }
   ...
   * {@inheritDoc}
   *
   * @see com.xpn.xwiki.user.api.XWikiGroupService#getAllGroupsNamesForMember(java.lang.String, int,
   *      int,
   *      com.xpn.xwiki.XWikiContext)
   */
  @Override
  public Collection<String> getAllGroupsNamesForMember(String member, int nb, int start,
    ...
    try {
      ...
      Collection<DocumentReference> groupReferences = getAllGroupsReferencesForMember(
          memberReference, nb, start, context);
  ...
  @Override
  public Collection<DocumentReference> getAllGroupsReferencesForMember(
      ...
      if (groupReferences == null) {
        ...
        try {
          ...
          if (memberReference.getWikiReference().getName().equals(context.getDatabase())
            ...
            query = context.getWiki().getStore().getQueryManager()
        ...
        // If the 'XWiki.XWikiAllGroup' is implicit, all users/groups except XWikiGuest and
        // XWikiAllGroup
        ...
        groupReferences = new ArrayList<>(groupNames.size());
   ...
   * {@inheritDoc}
   *
   * @see com.xpn.xwiki.user.api.XWikiGroupService#getAllMembersNamesForGroup(java.lang.String, int,
   *      int,
   *      com.xpn.xwiki.XWikiContext)
   */
  @Override
  ...
  public Collection<String> getAllMatchedMembersNamesForGroup(String group, String matchField,
    ...
    XWikiDocument groupDocument = new XWikiDocument(
        this.currentMixedDocumentReferenceResolver.resolve(group));
    ...
    queryString.append(' ').append(
        createMatchGroupMembersWhereClause(groupDocument.getFullName(), matchField, orderAsc,
            parameterValues));
