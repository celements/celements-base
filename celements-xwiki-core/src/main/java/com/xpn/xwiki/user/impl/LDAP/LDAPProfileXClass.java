...
public class LDAPProfileXClass {
   ...
   * @param dn
   *          the dn to store in the LDAP profile.
   * @param uid
   *          the uid to store in the LDAP profile.
   * @throws XWikiException
   *           error when storing information in user profile.
   */
  public void updateLDAPObject(String xwikiUserName, String dn, String uid) throws XWikiException {
    XWikiDocument userDocument = this.context.getWiki()
        .getDocument(XWIKI_USER_SPACE + "." + xwikiUserName, this.context);
  ...
  public XWikiDocument searchDocumentByUid(String uid) {
    ...
    try {
      ...
      String sql = ", BaseObject as obj, StringProperty as prop where doc.fullName=obj.name and obj.className=? and obj.id=prop.id.id and prop.name=? and lower(prop.value)=?";

      documentList = this.context.getWiki().getStore().searchDocuments(sql, false, false, false, 0,
          0,
