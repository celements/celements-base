...
public class LDAPAuthServiceImpl extends XWikiAuthServiceImpl {
          ...
          if ((principal == null) && (attributes.size() > 0)) {
            ...
            if (context.getWiki().isVirtualMode()) {
              ...
              try {
                ...
                if ((context.getWiki().getXWikiPreference("ldap_server", context) == null)
                    || (context.getWiki().getXWikiPreference("ldap_server", context)
                        .length() == 0)) {
  ...
  protected boolean checkUserPassword(String username, String password, HashMap attributes,
    ...
    try {
      ...
      if (log.isInfoEnabled()) {
        log.info("LDAP Password check for user " + username + " failed with exception "
            + e.getMessage());
      }

      if ((e.getResultCode() == LDAPException.NO_SUCH_OBJECT)
          || (e.getResultCode() == LDAPException.NO_SUCH_ATTRIBUTE)) {
        notinLDAP = true;
      }
    } catch (Throwable e) {
      notinLDAP = true;
      if (log.isErrorEnabled()) {
        log.error("LDAP Password check for user " + username + " failed with exception "
            + e.getMessage());
      }
    } finally {
      if (log.isDebugEnabled()) {
  ...
  public boolean createUserFromLDAP(String username, String authusername, String password,
      XWikiContext context) throws XWikiException {
    LDAPConnection lc = new LDAPConnection();
    boolean result = false;
    String foundDN;
    ...
    try {
      ...
      if (log.isInfoEnabled()) {
        log.info(
            "LDAP create user for user " + username + " failed with exception " + e.getMessage());
      }
    } catch (Throwable e) {
      if (log.isErrorEnabled()) {
        log.error(
