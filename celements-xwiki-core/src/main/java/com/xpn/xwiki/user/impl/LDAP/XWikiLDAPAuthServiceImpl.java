 ...
 * directory. It gives LDAP
 * users access if they belong to a particular group, creates XWiki users if they have never logged
 * in before and
...
public class XWikiLDAPAuthServiceImpl extends XWikiAuthServiceImpl {
  ...
  public Principal authenticate(String login, String password, XWikiContext context)
    ...
    /*
     * TODO: Put the next 4 following "if" in common with XWikiAuthService to ensure coherence This
     * method was
     * returning null on failure so I preserved that behaviour, while adding the exact error
     * messages to the context
     * given as argument. However, the right way to do this would probably be to throw
     * XWikiException-s.
     */
   ...
   * @return the {@link Principal}.
   * @throws XWikiException
   *           error when checking user name and password.
   */
   ...
   * @param validXWikiUserName
   *          the name of the XWiki user to log in.
   * @param password
   *          the password of the user to log in.
   * @param context
   *          the XWiki context.
   ...
   *           error when login.
   */
  protected Principal ldapAuthenticateInContext(String ldapUid, String validXWikiUserName,
      String password,
   ...
   *          the XWiki context.
   * @param local
   *          indicate if it's a local authentication. Supposed to return a local user
   *          {@link Principal} (whithout
   *          the wiki name).
   * @return the {@link Principal}.
   * @throws XWikiException
   *           error when login.
  ...
  protected Principal ldapAuthenticateInContext(String ldapUid, String validXWikiUserName,
      String password,
      XWikiContext context, boolean local)
    ...
    ldapUtils.setUserSearchFormatString(
        config.getLDAPParam("ldap_user_search_fmt", "({0}={1})", context));
    ...
    if (filterGroupDN.length() > 0) {
      ...
      if (ldapDn == null) {
        throw new XWikiException(XWikiException.MODULE_XWIKI_USER,
            ...
            new Object[] { ldapUid, filterGroupDN });
      }
    }
    ...
    // them
    if (ldapDn == null) {
      searchAttributes = ldapUtils.searchUserAttributesByUid(ldapUid,
   ...
   * @param createuser
   *          indicate if the user is created or updated.
   * @param ldapUtils
   *          the LDAP communication tool.
   * @param context
   *          the XWiki context.
   ...
   */
  // TODO move this methods in a toolkit for all platform.
  protected void removeUserFromXWikiGroup(String xwikiUserName, String groupName,
   ...
   *          the XWiki context.
   * @throws XWikiException
   *           error when updating XWiki user.
   */
  ...
  protected XWikiDocument getAvailableUserProfile(String validXWikiUserName, String ldapUid,
      XWikiContext context)
      throws XWikiException {
    BaseClass userClass = context.getWiki().getUserClass(context);
    LDAPProfileXClass ldapXClass = new LDAPProfileXClass(context);

    DocumentReference userReference = new DocumentReference(context.getDatabase(), XWIKI_USER_SPACE,
        validXWikiUserName);
