...
public class XWikiAuthServiceImpl extends AbstractXWikiAuthService {
      ...
      if ("basic".equals(xwiki.Param("xwiki.authentication"))) {
        ...
        if (xwiki.Param("xwiki.authentication.errorpage") != null) {
          ...
          sconfig.setErrorPage(stripContextPathFromURL(
              context.getURLFactory().createURL("XWiki", "XWikiLogin", "loginerror", context),
              context));
        }
        ...
        if (xwiki.Param("xwiki.authentication.encryptionalgorithm") != null) {
          persistent
              .setEncryptionAlgorithm(xwiki.Param("xwiki.authentication.encryptionalgorithm"));
        }
  ...
  public XWikiUser checkAuth(XWikiContext context) throws XWikiException {
    ...
    SecurityRequestWrapper wrappedRequest = new SecurityRequestWrapper(request, null, null,
        auth.getAuthMethod());
    ...
    try {
      ...
      if (LOG.isInfoEnabled() && (userName != null)) {
      ...
      LOG.debug(
          "XWikiAuthServiceImpl.checkAuth(XWikiContext) took " + (System.currentTimeMillis() - time)
              + " milliseconds to run.");
    }
  }
  ...
  public XWikiUser checkAuth(String username, String password, String rememberme,
    ...
    try {
      ...
      if (LOG.isInfoEnabled() && (principal != null)) {
  ...
  private String getContextUserName(Principal principal, XWikiContext context) {
    ...
    if (principal != null) {
      // Ensures that the wiki part is removed if specified in the Principal name and if it's not
      // the same wiki
      // as the current wiki.
      // TODO: Warning the code below will fail if current doc's wiki != current wiki.
      DocumentReference userDocumentReference = this.currentDocumentReferenceResolver
          .resolve(principal.getName());
  ...
  public Principal authenticate(String username, String password, XWikiContext context)
     ...
     * exact error
     * messages to the context given as argument. However, the right way to do this would probably
     * be to throw
     * XWikiException-s.
     */
    ...
    // If we have the context then we are using direct mode, and we should be able to specify the
    // database
        ...
        try {
          ...
          if ((user != null) && checkPassword(user, password, context)) {
            return new SimplePrincipal(
                virtualXwikiName != null ? context.getDatabase() + ":" + user
                    : user);
          } else {
  ...
  protected String findUser(String username, XWikiContext context) throws XWikiException {
    ...
    if (context.getWiki().exists(new DocumentReference(context.getDatabase(), "XWiki", username),
        context)) {
      user = "XWiki." + username;
    } else {
      ...
      Object[][] whereParameters = { { "doc.space", "XWiki" },
          { "doc.name", username } };
    ...
    if ((param == null) || "".equals(param)) {
      try {
        param = context.getWiki()
            .Param("xwiki.authentication." + StringUtils.replace(name, "auth_", ""));
      } catch (Exception e) {}
    }
  ...
  protected String createUser(String user, XWikiContext context) throws XWikiException {
    ...
    if (createuser != null) {
      ...
      XWikiDocument userdoc = context.getWiki()
          .getDocument(new DocumentReference(context.getDatabase(), "XWiki", wikiname), context);
      if (userdoc.isNew()) {
        if (LOG.isDebugEnabled()) {
  ...
  /**
   * The authentication library we are using (SecurityFilter) requires that its URLs do not contain
   * the context path,
   * in order to be usable with <tt>RequestDispatcher.forward</tt>. Since our URL factory include
   * the context path in
   ...
   *          The URL to process.
   * @param context
   *          The ubiquitous XWiki request context.
  ...
  protected String stripContextPathFromURL(URL url, XWikiContext context) {
    ...
    // XWiki uses contextPath in the wrong way, putting a / at the end, and not at the start. Fix
    // this here.
    if (contextPath.endsWith("/") && !contextPath.startsWith("/")) {
      contextPath = "/" + StringUtils.chop(contextPath);
    }

    // URLFactory.getURL applies Util.escapeURL, which might convert the contextPath into an %NN
    // escaped string.
