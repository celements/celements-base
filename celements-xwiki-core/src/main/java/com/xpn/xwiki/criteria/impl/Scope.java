 ...
 * of documents. Here, a document can be a page, a space, a wiki or the entire application as a
 * unit. For instance, a
...
public class Scope {
  ...
  /**
   * The type of the scope. It can be {@link #PAGE_SCOPE}, {@link #SPACE_SCOPE}, {@link #WIKI_SCOPE}
   * or
   * {@link #GLOBAL_SCOPE}
   */
  private int type;
  ...
  /**
   * Specifies whether the document given by the {@link #name} field should be considered as a unit
   * or not. When
   * {@link #deep} is <code>false</code> the scope matches only the document with the given
   * {@link #name} (taken as a
   ...
   * @param name
   *          The name of the document associated with this scope
   * @param deep
   *          <code>true</code> for matching all sub documents; <code>false</code> for matching the
   *          associated
   *          document as a unit
   */
