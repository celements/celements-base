...
public class XWikiXmlRpcApiImpl implements XWikiXmlRpcApi {
  ...
  public String login(String userName, String password) throws Exception {
    ...
    Principal principal = this.xwiki.getAuthService().authenticate(userName, password,
        this.xwikiContext);
    if (principal != null) {
      ...
      XWikiUtils.getTokens(this.xwikiContext).put(token,
          new XWikiXmlRpcUser(principal.getName(), ip));
  ...
  public List getSpaces(String token) throws Exception {
    ...
    for (String spaceKey : spaceKeys) {
      ...
      if (!this.xwikiApi.exists(spaceWebHomeId)) {
        ...
        /*
         * If doc is null, then we don't have the rights to access the document, and therefore to
         * the space.
         */
   ...
   * @throws Exception
   *           An invalid token is provided or the user doesn't have enough rights to access the
   *           space.
   */
  @Override
   ...
   *          The authentication token.
   * @param spaceMap
   *          The map representing a Space object.
   * @return The newly created space as a Space object.
   * @throws Exception
   *           An invalid token is provided or the space cannot be created or it already exists and
   *           the user
   ...
   * @throws Exception
   *           An invalid token is provided or there was a problem while deleting the space.
   */
  @Override
      ...
      if (this.xwikiApi.exists(pageFullName)) {
        ...
        if (doc != null) {
          try {
            ...
            /*
             * An exception here means that we haven't succeeded in deleting a page, so there might
             * be some
  ...
  /**
   * Retrieves a page. The returned page title is set the current title if the current title != "",
   * otherwise it is
   ...
   *          The authentication token.
   * @param pageId
   *          The pageId in the 'Space.Page[?language=l&version=v&minorVersion=mv]' format.
   * @return A map representing a Page object containing information about the requested page.
   * @throws Exception
   *           An invalid token is provided or the user has not the right to access the page or the
   *           page does
   *           not exist.
   */
  @Override
   ...
   * </code>
   * <p>
   * In XWiki in order to rename a page we need to change its ID, and no client written for
   * confluence will do this.
   * Currently the authoritative source for the page location is the ID (basically storePage ignores
   * the space field)
   * and changing the title will only affect the page title. However if we agree to assume that the
   * when using XMLRPC
   * the semantics of the page title is that of the page name in an XWiki ID, we will be able to be
   * confluence
   * compatible.
   * </p>
   * </p>
   * There are three possible cases: Let P=(id, space, title) the definition of a page. Let CP be
   * the current
   * page and NP the page to be stored (i.e. the page passed to storePage):
   * </p>
   * <p>
   * 1) CP=("Space.Name", "Space", "Title") NP=("Space.Name", "NewSpace", "Title") Here it is clear
   * that the user
   * wants to "rename" the page by moving it to another space. So we rename the page to
   * ("NewSpace.Name", "NewSpace",
   * "Title")
   * </p>
   * <p>
   * 2) CP=("Space.Name", "Space", "Title") NP=("Space.Name", "NewSpace", "NewTitle"); Here it is
   * also clear that we
   * want to move the page to NewSpace but we have a problem about how to name the new page:
   * NewSpace.Name or
   * NewSpace.NewTitle? According to the assumption stated before, we rename the page and use
   * NewTitle as the page
   * name. The renamed page will have the NewSpace.NewTitle id. We also set the renamed page title
   * to NewTitle.
   * </p>
   * <p>
   * 3) CP=("Space.Name", "Space", "Title") NP=("Space.Name", "Space", "NewTitle"); Here we have an
   * ambiguity. Does
   * the user want to to rename the page or set its title? According to the assumption stated before
   * we assume that
   * the user wants to rename the page so we will rename the page to Space.NewTitle and set its
   * title to NewTitle.
   * </p>
   *
   * @param token
   *          The authentication token.
   * @param pageMap
   *          A map representing the Page object to be stored.
   * @return A map representing a Page object with the updated information.
   * @throws Exception
   *           An invalid token is provided or some data is missing or the page is locked or the
   *           user has no
  ...
  public Map storePage(String token, Map pageMap) throws Exception {
    ...
    /*
     * Confluence semantics compatibility. In order to say to create a page, Confluence sets the id
     * to null.
     */
    if (page.getId() == null) {
    ...
    /*
     * Even though the extended id could contain parameters such as language and version, here we
     * ignore them and
     * just consider the base page id. The language to be stored will be taken from the result of
     * page.getLanguage(). Version is not meaningful because we don't allow to replace previous
     * versions of the
     * page.
     */
    ...
    if (doc != null) {
      ...
      /*
       * Confluence semantics compatibility. Here we assume that the authoritative source for the
       * page ID is the
       * combination of the fields Space + Title as specified in the page structure passed to the
       * function. If the
       * title or the space (or both) are different from the current values then we must handle the
       * request as a
       * page rename.
       */
      ...
      /*
       * Check if the page already exists. If it doesn't this is surely not a rename request! Check
       * also that the
       * title is not null. This might happen if a client creates an XWikiPage object with an
       * existing page id and
       * initializes only the content field (which is perfectly legal for non-existing pages but
       * here causes a
       * NPE).
       */
      if (pageAlreadyExists && (page.getTitle() != null)) {
        ...
        /*
         * Here the title is different from the document's name. This check is necessary because
         * getPage
         * sets the title to the page name when the actual title is "". So when this is the case,
         * even
         * though the document title is different from the page title, the caller might not have
         * changed the
         * title field for renaming the page.
         */
        if ((!page.getTitle().equals(doc.getName()) && !page.getTitle().equals(doc.getTitle()))
            && !page.getTitle().equals("")) {
          /*
           * Check a degenerate case where the user tries to rename the page to a page with an
           * empty
        ...
        /*
         * Here the document space and the space field of the page passed as parameter are
         * different. So
         * this is surely a rename request.
         */
        if (!doc.getSpace().equals(page.getSpace()) && !page.getSpace().equals("")) {
          /*
           * Check a degenerate case where the user tries to rename the page to a space with an
           * empty
      ...
      /*
       * In order to rename a page we must rename the default language one. This simplifies the
       * logic for handling
       * Confluence compatibility. In fact every page translation has its own title and we consider
       * the default
      ...
      if (rename && page.getLanguage().equals("")) {
        ...
        /*
         * Given the conditions before, here we have that newSpace != "" AND newName != "", AND
         * either newSpace
        ...
        if ((page.getLanguage() != null) && !page.getLanguage().equals("")
          ...
          if (!doc.getLanguage().equals(page.getLanguage())) {
            /*
             * If we are here, then the document returned by getTranslatedDocument is the same of
             * the
             * default translation, i.e., the page in the current translation does not exist. So we
             * have to
             * create it. Here we have to use the low-level XWiki API because it is not possible to
             * set the
   ...
   *          The authentication token.
   * @param pageId
   *          The pageId in the 'Space.Page' format.
   * @throws Exception
   *           An invalid token is provided or if the page does not exist or the user has not the
   *           right to
   *           access it.
   */
  @Override
   ...
   *          The authentication token.
   * @param pageId
   *          The pageId in the 'Space.Page[?language=lang]' format.
  ...
  public List getPageHistory(String token, String pageId) throws Exception {
    ...
    for (Version version : versions) {
      ...
      /*
       * The returned document has the right content but the wrong content update date, that is
       * always equals to
   ...
   *          Ignored
   * @param pageId
   *          The page id in the form of Space.Page
   * @param content
   *          The content to be rendered. If content == "" then the page content is rendered.
  ...
  public String renderContent(String token, String space, String pageId, String content)
    ...
    XWikiDocument baseDocument = this.xwiki.getDocument(extendedId.getBasePageId(),
        this.xwikiContext);
  ...
  /**
   * Get a specific comment. This method is here just for completeness because getComments already
   * returns the list
   ...
   * @throws Exception
   *           An invalid token is provided or if the comment cannot be retrieved. access it.
   */
  @Override
  public Map getComment(String token, String commentId) throws Exception {
    ...
    int commentNumericalId = Integer
        .parseInt(extendedId.getParameter(XWikiExtendedId.COMMENT_ID_PARAMETER));

    Document doc = XWikiUtils.getDocument(this.xwikiApi, commentId, true);

    com.xpn.xwiki.api.Object commentObject = doc.getObject("XWiki.XWikiComments",
        commentNumericalId);
   ...
   * @throws Exception
   *           An invalid token is provided or if the user has not the right to access it.
   */
  @Override
   ...
   * @throws Exception
   *           An invalid token is provided or the user has not the right to access it.
   */
  @Override
  public Boolean removeComment(String token, String commentId) throws Exception {
    ...
    if (doc.getLocked()) {
      throw new Exception(
          String.format("[Unable to remove attachment. Document '%s' locked by '%s']",
              doc.getName(), doc.getLockingUser()));
    }
   ...
   *          Ignored
   * @param attachmentMap
   *          The Attachment object used to identify the page id, and attachment metadata.
  ...
  public Map addAttachment(String token, Integer contentId, Map attachmentMap,
    ...
    com.xpn.xwiki.api.Attachment xwikiAttachment = doc.addAttachment(attachment.getFileName(),
        attachmentData);
    doc.save();
  ...
  public byte[] getAttachmentData(String token, String pageId, String fileName,
    ...
    if (xwikiAttachment != null) {
      ...
      throw new Exception(
          String.format("[Attachment '%s' does not exist on page '%s']", fileName, pageId));
    }
  }
  ...
  public Boolean removeAttachment(String token, String pageId, String fileName) throws Exception {
    ...
    if (xwikiAttachment != null) {
      /*
       * Here we must use low-level XWiki API because there is no way for removing an attachment
       * through the XWiki
       * User's API. Basically we use the com.xpn.xwiki.XWiki object to re-do all the steps that we
       * have already
       * done so far by using the API. It is safe because if we are here, we know that everything
       * exists and we
       * have the proper rights to do things. So nothing should fail.
       */
      XWikiDocument baseXWikiDocument = this.xwiki.getDocument(extendedId.getBasePageId(),
          this.xwikiContext);
   ...
   * @throws Exception
   *           An invalid token is provided or if the given class does not exist.
   */
  @Override
   ...
   * in the mapping
   * 'field' -> 'value' In order to know all the available fields and their respective types and
   * attributes, clients
  ...
  public Map getObject(String token, String pageId, String className, Integer id) throws Exception {
    ...
    if (object != null) {
      return DomainObjectFactory.createXWikiObject(this.xwiki, this.xwikiContext, doc, object)
          .toRawMap();
    } else {
      throw new Exception(
   ...
   *          The authentication token.
   * @param objectMap
   *          A map representing the XWikiObject to be updated/created.
  ...
  public Map storeObject(String token, Map objectMap) throws Exception {
    ...
    /*
     * The passed object must specify a class name otherwise we cannot set properties on the xwiki
     * object
     */
    ...
    /*
     * Allow clients to specify/override a guid. This is necessary to allow replication of objects
     * between xwikis.
    ...
    /*
     * We iterate on the XWikiObject-passed-as-a-parameter's properties instead of the one retrieved
     * through the API
     * because, a newly created object has no properties, and they should be added via set.
     * Apparently setting
     * properties that do not belong to the object's class is harmless.
     */
    for (String propertyName : object.getProperties()) {
      /*
       * Object values are always sent as strings (or arrays/maps of strings)... let the actual
       * object perform the
       * conversion
       */
    ...
    return DomainObjectFactory.createXWikiObject(this.xwiki, this.xwikiContext, doc, xwikiObject)
        .toRawMap();
  }
  ...
  public Boolean removeObject(String token, String pageId, String className, Integer id)
    ...
    if (object != null) {
      ...
      throw new Exception(
          String.format("[Object %s[%d] on page '%s' does not exist]", className, id,
              extendedId.getBasePageId()));
    }
   ...
   *          The authentication token.
   * @param query
   *          The string to be looked for. If it is "__ALL_PAGES__" the search will return all the
   *          page ids
  ...
  public List search(String token, String query, int maxResults) throws Exception {
    ...
    if (query.equals("__ALL_PAGES__")) {
      ...
      for (String spaceKey : spaceKeys) {
        List<String> pageNames = this.xwikiApi.getSpaceDocsName(spaceKey);
        for (String pageName : pageNames) {
          result.add(DomainObjectFactory
              .createSearchResult(String.format("%s.%s", spaceKey, pageName)).toMap());
        }
      }
    } else {
  ...
  /**
   * Returns a list of XWikiPageHistorySummary containing all the pages that have been modified
   * since a given date in
   * all their versions.
   *
   * @param token
   * @param date
   ...
   * @param start
   *          The start offset in the result set
   * @param fromLatest
   *          True if the result set will list recent changed pages before.
   * @return A list of maps representing XWikiPageHistorySummary
   * @throws Exception
   *           An invalid token is provided.
   */
  @Override
  public List getModifiedPagesHistory(String token, Date date, int numberOfResults, int start,
      boolean fromLatest)
      throws Exception {
    XWikiXmlRpcUser user = XWikiUtils.checkToken(token, this.xwikiContext);
    ...
    List<Object> queryResult = queryManager.createQuery(query, Query.XWQL).bindValue("date", date)
        .setLimit(numberOfResults).setOffset(
            start)
        .execute();
  ...
  /**
   * This is a version of storePage that fails to store the page if the current version of the page
   * doesn't match the
   ...
   *          A map representing the Page object to be stored.
   * @param checkVersion
   *          True if the current version of the page and the one of the page passed as parameter
   *          must
   *          match.
   * @return A map representing an XWikiPage with updated information, or an XWikiPage whose fields
   *         are all empty if
  ...
  public Map storePage(String token, Map pageMap, boolean checkVersion) throws Exception {
    if (checkVersion) {
      ...
      /*
       * Build a new extended id by removing version information in order to retrieve the latest
       * version, in case
  ...
  /**
   * This is a version of storeObject that fails to store the object if the current version of the
   * page associated to
   * the object doesn't match the one of the object passed as parameter (i.e., the object's page has
   * been modified
   ...
   *          A map representing the XWikiObject to be updated/created.
   * @param checkVersion
   *          True if the current version of the object's page and the one of the one passed as
   *          parameter
   *          must match.
   * @return A map representing the XWikiObject with the updated information, or an XWikiObject
  ...
  public Map getObject(String token, String pageId, String guid) throws Exception {
    ...
    if (object == null) {
      throw new Exception(
          String.format("[Unable to find object with guid '%s' on page '%s']", guid, pageId));
    }
   ...
   * @param initialSyntaxId
   *          The initial syntax of the source.
   * @param targetSyntaxId
   *          The final syntax of the returned content.
   * @return The converted source.
   * @throws Exception
   *           An invalid token is provided, the syntaxId is not supported, the source is invalid or
   *           the
   *           conversion fails.
   */
  @Override
   ...
   * @throws Exception
   *           An invalid token is provided or the syntax lookup fails.
   */
  @Override
   ...
   * @param targetSyntaxId
   *          The target syntax of the rendered content
   * @return The rendered content.
   * @throws Exception
   *           If a invalid token is provided, an unsupported syntax id is given or the rendering
   *           fails.
   */
  @Override
  public String renderPageContent(String token, String pageId, String content,
