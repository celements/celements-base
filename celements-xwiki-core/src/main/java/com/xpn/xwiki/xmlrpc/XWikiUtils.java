...
public class XWikiUtils {
  ...
  public static XWikiXmlRpcUser checkToken(String token, XWikiContext context) throws Exception {
    ...
    if (token != null) {
      if (token.equals("")) {
        /*
         * If no token is provided, then grant guest access or refuse it, depending on the current
         * configuration
         */
        if (allowGuest) {
    ...
    if ((user == null) || (!user.getRemoteIp().equals(ip))) {
      throw new Exception(
          String.format("[Access Denied: authentication token '%s' for IP '%s' is invalid]",
              token, ip));
    }
  ...
  /**
   * <p>
   * Gets a document. This method can be used to retrieve a specific translation or a version of a
   * page by using
   * extended ids in the form of Space.Page[?language=l&version=v&minorVersion=mv] where all
   * parameters are optional.
   * </p>
   * <p>
   * For example:
   * <ul>
   * <li><code>Main.WebHome</code>: retrieves Main.WebHome at its latest version in the default
   * language</li>
   * <li><code>Main.WebHome?language=fr</code>: retrieves Main.WebHome in its french
   * translation</li>
   * <li><code>Main.WebHome?version=3</code>: retrieves Main.WebHome at version 3.1 in the default
   * language</li>
   * <li><code>Main.WebHome?language=fr&version=2</code>: retrieves the version 2.1 of the french
   * translation</li>
   * </ul>
   * </p>
   *
   * @param xwikiApi
   *          The api object for accessing XWiki functionalities.
   * @param extendedPageId
   *          The extended page id
   * @param failIfDoesntExist
   *          True is an exception has to be raised if the page doesn't exist.
   * @return Always returns a document if success. Otherwise an exception is raised. Never returns
   *         null.
   * @throws Exception
   * @throws XmlRpcException
   *           An exception is thrown if the requested document doesn't exist or cannot be accessed
   *           or
  ...
  public static Document getDocument(com.xpn.xwiki.api.XWiki xwikiApi, String extendedPageId,
    ...
    if (failIfDoesntExist && !xwikiApi.exists(pageId)) {
    ...
    if ((language != null) && !language.equals("") && doc.getTranslationList().contains(language)) {
      ...
      if (doc == null) {
        throw new Exception(
            String.format("[Page '%s' at translation '%s' cannot be accessed]", pageId,
                language));
      }
    }
    ...
    if (version != 0) {
      ...
      if (doc == null) {
        throw new Exception(
            String.format("[Page '%s' at version '%d.%d' cannot be accessed (language '%s')]",
  ...
  public static com.xpn.xwiki.api.Object getObjectByGuid(Document doc, String guid) {
    ...
    for (Entry<String, Vector<com.xpn.xwiki.api.Object>> classObjects : classToObjectsMap
        .entrySet()) {
