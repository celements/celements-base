...
public class XWikiPageNotification implements XWikiActionNotificationInterface {
  ...
  public void notify(XWikiNotificationRule rule, XWikiDocument doc, String action,
    ...
    try {
      ...
      if ((notifpages != null) && (!notifpages.equals(""))) {
        ...
        for (String element : notifpages2) {
          notifyPage(element, rule, doc, action, context);
        }
      }
      String xnotif = (context.getRequest() != null)
          ? context.getRequest().getParameter("xnotification")
          : null;
  ...
  protected void notifyPage(String page, XWikiNotificationRule rule, XWikiDocument doc,
