 ...
 *
 * @version $Id$
 * @todo there should be some protection so that if a listener fails the other listeners are still
 *       called.
 */
@Deprecated
public class XWikiNotificationManager {
  ...
  public void removeGeneralRule(XWikiNotificationRule rule) {
    // TODO: This should be improved since it's impossible to remove a rule if you loose its
    // instance
    // (since it's an equals on the pointer in memory that is done).
    // The solution is to implement equals/hashCode in XWikiNotificationRule implementations and
    // introduce
   ...
   *
   * @param name
   *          the document's name for which the notification rule should apply. For example, for the
   *          "Main.WebHome"
   *          document, the rule name should either be "Main.WebHome", or "database:Main.WebHome"
   * @param rule
   *          the rule to add for this name
   */
