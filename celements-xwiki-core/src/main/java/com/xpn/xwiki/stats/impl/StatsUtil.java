...
public final class StatsUtil {
  ...
  /**
   * The name of the property in XWiki configuration file containing the list of users and group to
   * filter in
  ...
  /**
   * The name of the XWiki preferences property containing the list of users and group to filter in
   * statistics view
   * requests.
   */
   ...
   *
   * @param date
   *          the date for which to return an integer representation.
   * @param type
   *          the date type. It can be {@link PeriodType#DAY} or {@link PeriodType#MONTH}.
  ...
  public static int getPeriodAsInt(Date date, PeriodType type) {
    ...
    if (type == PeriodType.MONTH) {
      ...
      period = (cal.get(Calendar.YEAR) * 10000) + ((cal.get(Calendar.MONTH) + 1) * 100)
  ...
  /**
   * @param context
   *          the XWiki context from where to get the HTTP session session.
   * @param action
   *          the action id.
  ...
  public static Collection<?> getRecentActionFromSessions(XWikiContext context, String action) {
    return (Collection<?>) context.getRequest().getSession()
        .getAttribute(SESSPROP_RECENT_PREFFIX + action);
  }
  ...
  /**
   * Try to find the visiting session of the current request, or create a new one if this request is
   * not part of a
    ...
    if (visitObject != null) {
      ...
      if (!visitObject.getCookie().equals(cookie.getValue())) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("Found visit with cookie " + visitObject.getCookie() + " in session "
              + session.getId()
        ...
        valid = false;
          ...
          || ((visitObject != null) && !context.getUser().equals(visitObject.getName()))) {
   ...
   *           error when searching for visit object.
   * @since 1.4M1
   */
  protected static VisitStats findVisitByField(String fieldName, String fieldValue,
    ...
    if (qm.hasLanguage(Query.XPATH)) {
      try {
        solist = qm.createQuery(
            "//element(*, xwiki:object)[@:{fieldName}=:{fieldValue}"
                + " and @endDate>:{date}]  order by @endDate descending",
            Query.XPATH)
      ...
      try {
        solist = qm.createQuery(
            "from VisitStats as obj " + "where obj." + fieldName
                + "=:fieldValue and obj.endDate > :date"
                + " order by obj.endDate desc",
            Query.HQL).bindValue(sfieldValue, fieldValue)
            .bindValue(sdate, currentDate).execute();
      } catch (Exception e) {
  ...
  protected static Cookie addCookie(XWikiContext context) {
    Cookie cookie = new Cookie(COOKPROP_VISITID,
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Setting cookie " + cookie.getValue() + " for name " + cookie.getName() + " with domain "
              + cookie.getDomain() + " and path " + cookie.getPath() + " and maxage "
              + cookie.getMaxAge());
    }
   ...
   *
   * @param pref
   *          field name in XWikiPreference
   * @param cfg
   *          field name in xwiki.cfg file
   * @param context
   *          the XWiki context
   * @return the list of users references
   * @throws XWikiException
   *           error when trying to resolve users
   */
  private static Collection<DocumentReference> getFilteredUsers(String pref, String cfg,
      XWikiContext context)
      throws XWikiException {
  ...
  public static Collection<DocumentReference> getStorageFilteredUsers(XWikiContext context)
      throws XWikiException {
    // TODO: cache
    return getFilteredUsers(PREFPROP_EXCLUDEDUSERSANDGROUPS_STORAGE,
        CFGPROP_STATS_EXCLUDEDUSERSANDGROUPS_STORAGE,
        context);
  }
  ...
  public static Collection<DocumentReference> getRequestFilteredUsers(XWikiContext context)
      throws XWikiException {
    // TODO: cache
    Collection<DocumentReference> users = getFilteredUsers(PREFPROP_EXCLUDEDUSERSANDGROUPS_REQUEST,
        CFGPROP_STATS_EXCLUDEDUSERSANDGROUPS_REQUEST,
        context);

    return users != null ? users
        : getFilteredUsers(DEPRECATED_PREFPROP_EXCLUDEDUSERSANDGROUPS,
