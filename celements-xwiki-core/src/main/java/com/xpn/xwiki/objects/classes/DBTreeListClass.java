...
public class DBTreeListClass extends DBListClass {
  ...
  protected List<ListItem> getCachedDBTreeList(XWikiContext context) {
    if (isCache()) {
      // If the property is supposed to be cached long term ({@link #isCache()}), then the list is
      // cached in
      // memory in the current object
      return this.cachedDBTreeList;
    } else {
      // Otherwise, to avoid re-computing the tree in case it is requested several times during the
      // same request,
      // it is cached in the request context.
      return (List<ListItem>) context
          .get(context.getDatabase() + ":" + getFieldFullName() + "-tree");
    }
  }
  ...
  /**
   * Gets an ordered list of items in the tree. This is necessary to make sure children are coming
   * right after their
   * parents.
   *
   * @param treemap
   *          the unordered list of tree nodes
   * @param map
  ...
  protected void addToTreeList(List<ListItem> treelist, Map<String, List<ListItem>> treemap,
    ...
    if (list != null) {
      for (ListItem item : list) {
        ListItem item2 = new ListItem(item.getId(), getDisplayValue(item.getId(), "", map, context),
            item.getParent());
        treelist.add(item2);
  ...
  private List<ListItem> getItemPath(String item, List<ListItem> treeList,
      ArrayList<ListItem> resList) {
  ...
  private String displayTree(String name, String prefix, List<String> selectlist, String mode,
      XWikiContext context) {
   ...
   * specified by the XClass
   * creator in the <tt>sql</tt> field, or, if the query field is blank, constructed using the
   * <tt>classname</tt>,
   * <tt>idField</tt>, <tt>valueField</tt> and <tt>parentField</tt> properties. The query is
   * constructed according to
   * the following rules:
   * </p>
   * <ul>
   * <li>If no classname, id and value fields are selected, return a query that return no rows, as
   * the parent is not
   * enough to make a query.</li>
   * <li>If no parent field is provided, use the document "parent" medatada.</li>
   * <li>If only the classname is provided, select all document names which have an object of that
   * type, preserving
   ...
   *
   * @param context
   *          The current {@link XWikiContext context}.
  ...
  public String getQuery(XWikiContext context) {
    ...
    if (StringUtils.isBlank(sql)) {
      if (context.getWiki().getHibernateStore() != null) {
        ...
        boolean usesObj = hasClassname || idField.startsWith("obj.")
        ...
        if (valueField.startsWith("doc.") || valueField.startsWith("obj.")) {
          ...
          if (valueField.equals(idField)) {
            ...
            whereStatements
                .add("obj.id=valueprop.id.id and valueprop.id.name='" + valueField + "'");
          }
        }
        ...
        if (parentField.startsWith("doc.") || parentField.startsWith("obj.")) {
          ...
          if (parentField.equals(idField)) {
            ...
            whereStatements
                .add("obj.id=parentprop.id.id and parentprop.id.name='" + parentField + "'");
          }
        }
