...
public abstract class ListClass extends PropertyClass {
   ...
   * @param map
   *          The value=name mapping specified in the "values" parameter of the property.
   * @param context
   *          The request context.
   * @return The text that should be displayed, representing a human-understandable name for the
   *         internal value.
   */
  protected String getDisplayValue(String value, String name, Map<String, ListItem> map,
  ...
  /**
   * Search for an internationalizable display text for the current value. The value can be either a
   * simple string, or
   * a value=name pair selected from the database.
   *
   * @see #getDisplayValue(String, String, Map, XWikiContext)
   * @param rawvalue
   *          The internal value, or a value=name pair.
   * @param name
   *          The name of the ListProperty.
   * @param map
   *          The value=name mapping specified in the "values" parameter of the property.
   ...
   *         internal value.
   */
  protected String getDisplayValue(Object rawvalue, String name, Map<String, ListItem> map,
      XWikiContext context) {
    if (rawvalue == null) {
      return "";
    }
  ...
  /**
   * If the list is populated with value=name pairs selected from the database, then return only the
   * value. Otherwise,
  ...
  protected void displayRadioEdit(StringBuffer buffer, String name, String prefix,
    ...
    for (Object rawvalue : list) {
      ...
      input radio = new input(
          (getDisplayType().equals("radio") && !isMultiSelect()) ? input.radio : input.checkbox,
          prefix
              + name,
          value);
      ...
      buffer.append(
          "<label class=\"xwiki-form-listclass\" for=\"xwiki-form-" + name + "-" + count++ + "\">");
  ...
  protected String displayRadioSearch(String name, String prefix, XWikiCriteria criteria,
    ...
    /*
     * BaseProperty prop = (BaseProperty)object.safeget(name); if (prop==null) { selectlist = new
     * ArrayList(); }
     * else if ((prop instanceof ListProperty)||(prop instanceof DBStringListProperty)) { selectlist
     * = (List)
    ...
    for (Iterator<String> it = list.iterator(); it.hasNext();) {
      ...
      input radio = new input(getDisplayType().equals("radio") ? input.radio : input.checkbox,
          prefix + name, value);
  ...
  protected String displaySelectSearch(String name, String prefix, XWikiCriteria criteria,
    ...
    List<String> selectlist = (selectArray != null) ? Arrays.asList(selectArray)
        : new ArrayList<>();
