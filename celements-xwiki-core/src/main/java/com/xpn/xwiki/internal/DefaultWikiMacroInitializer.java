 ...
 */
@Component
public class DefaultWikiMacroInitializer extends AbstractLogEnabled
    implements WikiMacroInitializer, WikiMacroConstants {
  ...
  /**
   * Registers the wiki macros for all the wikis or a specific wiki, according to the passed
   * parameter. <br />
   * FIXME: I don't like this way of passing params, but it's kinda the best I can do for the moment
   * without
   * duplicating at least the logic inside this function, if not some code as well.
   *
   * @param local
   *          false if only macros in a specified wiki are to be registered, in which case, the name
   *          of the wiki
   *          should be specified in the second parameter, false if the macros in all the wikis
   *          should be
   *          registered, in which case the value of the second parameter is ignored
   * @param wiki
   *          the name of the wiki to register macros for, if local is true
   * @throws Exception
   *           if xwiki classes required for defining wiki macros are missing or if an error occurs
   *           while
  ...
  private void registerExistingWikiMacros(boolean local, String wiki) throws Exception {
    ...
    XWikiDocument wikiMacroParameterClass = xcontext.getWiki()
        .getDocument(WIKI_MACRO_PARAMETER_CLASS, xcontext);
   ...
   *
   * @param wikiName
   *          the name of the wiki to process, lowercase database name
   * @param xcontext
  ...
  private void registerMacrosForWiki(String wikiName, XWikiContext xcontext) {
    try {
      ...
      // Set the context to be in that wiki so that both the search for XWikiMacro class objects and
      // the
      // registration of macros registered for the current wiki will work.
      // TODO: In the future when we have APIs for it, move the code to set the current wiki and the
      // current user
      ...
      for (Object[] wikiMacroDocumentData : getWikiMacroDocumentData(xcontext)) {
        // In the database the space and page names are always specified for a document. However the
        // wiki
   ...
   * @param xcontext
   *          the current request context
   * @return a list of documents containing wiki macros, each item as a vector of 3 strings: space
   *         name, document
  ...
  private List<Object[]> getWikiMacroDocumentData(XWikiContext xcontext) throws Exception {
    ...
    try {
      wikiMacroDocumentData = xcontext.getWiki().getStore().search(sql, 0, 0,
          Arrays.asList(WIKI_MACRO_CLASS),
          xcontext);
   ...
   *
   * @param wikiMacroDocumentReference
   *          the document holding the macro definition
   * @param wikiMacroDocumentAuthor
   *          the author of the macro document
   * @param xcontext
   *          the current request context
   */
  private void registerMacro(DocumentReference wikiMacroDocumentReference,
      String wikiMacroDocumentAuthor,
  ...
  public void installOrUpgradeWikiMacroClasses() throws Exception {
    ...
    needsUpdate |= bclass.addStaticListField(MACRO_VISIBILITY_PROPERTY, "Macro visibility", 1,
        false,
        "Current User|Current Wiki|Global", "select", "|");
    needsUpdate |= bclass.addStaticListField(MACRO_CONTENT_TYPE_PROPERTY, "Macro content type", 1,
        false,
    ...
    needsUpdate |= bclass.addTextAreaField(PARAMETER_DESCRIPTION_PROPERTY, "Parameter description",
        40, 5);
    needsUpdate |= bclass.addBooleanField(PARAMETER_MANDATORY_PROPERTY, "Parameter mandatory",
        "yesno");
    needsUpdate |= bclass.addTextField(PARAMETER_DEFAULT_VALUE_PROPERTY, "Parameter default value",
        30);
   ...
   *
   * @param doc
   *          xwiki document containing the wiki macro class.
