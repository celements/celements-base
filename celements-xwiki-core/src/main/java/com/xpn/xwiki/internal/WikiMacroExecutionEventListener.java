...
/**
 * Make sure to execute wiki macro with a properly configured context and especially which user
 * programming right is
 * tested on.
 *
 * @version $Id$
 */
...
public class WikiMacroExecutionEventListener extends AbstractLogEnabled implements EventListener {
  ...
  public void onEvent(Event event, Object source, Object data) {
    XWikiContext xwikiContext = (XWikiContext) this.execution.getContext()
        .getProperty("xwikicontext");
    XWikiDocument contextDoc = xwikiContext.getDoc();

    if (event instanceof WikiMacroExecutionStartsEvent) {
      // Set context document content author as macro author so that programming right is tested on
      // the right
      // user
      WikiMacro wikiMacro = (WikiMacro) source;
      XWikiDocument wikiMacroDocument;
      try {
        wikiMacroDocument = (XWikiDocument) this.documentAccessBridge
            .getDocument(wikiMacro.getDocumentReference());

        // Set context document content author as macro author so that programming right is tested
        // on the right
        // user. It's cloned to make sure it not really modifying the real document but only do that
        // for the
        // current context.
      ...
      try {
        contextDoc = (XWikiDocument) this.documentAccessBridge
            .getDocument(contextDoc.getDocumentReference());
