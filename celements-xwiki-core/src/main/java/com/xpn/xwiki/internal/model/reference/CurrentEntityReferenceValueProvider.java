 ...
 * <ul>
 * <li>The wiki value used is the current wiki if no wiki was specified in the passed reference (or
 * if it was empty). Note that this is different from using the current document's wiki value.</li>
 * <li>The space value used is the space from the current document reference if no space was
 * specified in the passed reference (or if it was empty). If the current document reference is not
 * defined then the default space value is used instead.</li>
 * <li>The page value used is the page from the current document reference if no page was specified
 * in the passed reference (or if it was empty). If the current document reference is not defined
 * then the default page value is used instead.</li>
 * </ul>
 *
 * @version $Id: a3093d0f7577a25909ddde9e1182e392240a57bf $
 * @since 2.3M1
 */
@Component("current")
@Singleton
public class CurrentEntityReferenceValueProvider extends DefaultEntityReferenceValueProvider {
  ...
  public String getDefaultValue(EntityType type) {
    ...
    if (type == EntityType.WIKI) {
      EntityReference wikiReference = modelContext.getCurrentEntityReference();
      if (wikiReference != null) {
        wikiReference = wikiReference.extractRef(EntityType.WIKI).orElse(null);
      }
      if (wikiReference != null) {
