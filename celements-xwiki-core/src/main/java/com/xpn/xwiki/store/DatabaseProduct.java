 ...
 * <p>
 * Ideally we shouldn't have to take care of Database specificities since we're using Hibernate to
 * abstract ourselves
 * from Databases. However it happens that Hibernate doesn't support setting Catalogs on some
 * databases and instead we
 * need to use our own tricks to do that and these tricks depend on the database. Hence the need to
 * differentiate them
...
public final class DatabaseProduct {
   ...
   * http://publib.boulder.ibm.com/infocenter/db2luw/v9r5/topic/com.ibm.db2.luw.apdv.java.doc/doc/c0053013.html,
   * the
   * database product name returned by the
   * {@link java.sql.DatabaseMetaData#getDatabaseProductName()} method of DB2
   * JDBC drivers varies by the OS and environment the product is running on. Hence the DB string
   * here uses only the
   * first 3 unique characters of the database product name. The {@link #toProduct(String)} method
   * also hence checks
   ...
   *
   * @param productName
   *          the database product name as returned by
   *          {@link java.sql.DatabaseMetaData#getDatabaseProductName()}.
   */
  ...
  /**
   * @return the database product name. Example: "Oracle". The returned value should correspond to
   *         the value returned
