...
public class XWikiHibernateMigrationManager extends AbstractXWikiMigrationManager {
  ...
  public XWikiDBVersion getDBVersion(XWikiContext context) throws XWikiException {
    ...
    return ver != null ? ver
        : getStore(context).executeRead(context, true, session -> {
          XWikiDBVersion result = (XWikiDBVersion) session.createCriteria(XWikiDBVersion.class)
              .uniqueResult();
  ...
  protected void setDBVersion(final XWikiDBVersion version, XWikiContext context)
    ...
    getStore(context).executeWrite(context, true, session -> {
  ...
  protected List<XWikiMigratorInterface> getAllMigrations(XWikiContext context)
      throws XWikiException {
    List<XWikiMigratorInterface> result = new ArrayList<>();
