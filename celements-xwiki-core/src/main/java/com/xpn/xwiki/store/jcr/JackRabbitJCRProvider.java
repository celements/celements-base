...
public class JackRabbitJCRProvider implements IJcrProvider {
  ...
  public JackRabbitJCRProvider(XWikiConfig config, XWikiContext context) throws IOException,
      LoginException, RepositoryException, InvalidNodeTypeDefException, ParseException {
    this.config = config;
    if (context != null) {
    ...
    String jrconfig = getRealConfigPath("xwiki.store.jcr.jackrabbit.repository.config",
  ...
  public XWikiJcrSession getSession(String workspace, String username, String password)
      throws LoginException, RepositoryException {
    Session s = repository.login(new SimpleCredentials(username, password.toCharArray()),
        workspace);
  ...
  public boolean initWorkspace(String workspace) throws RepositoryException, IOException {
    ...
    try {
      ...
      registerNodeTypes(s.getWorkspace(), getRealConfigPath(
          "xwiki.store.jcr.jackrabbit.nodetypes.config", "jackrabbit/nodetypes.cnd"));
      s.save();
    } catch (RepositoryException e) {
  ...
  private void registerNodeTypes(Workspace ws, String cndFileName)
    ...
    ntm.registerNodeTypes(fis, JackrabbitNodeTypeManager.TEXT_X_JCR_CND);
  }

  private boolean registerNamespace(Workspace w, String preffix, String uri)
