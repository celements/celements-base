...
public class XWikiJcrBaseStore {
  ...
  public XWikiJcrBaseStore(XWiki xwiki, XWikiContext context) throws SecurityException,
    ...
    if (xwiki.getStore() instanceof XWikiJcrBaseStore) {
      ...
      Constructor cn = Class.forName(sprovider)
          .getConstructor(XWikiConfig.class, XWikiContext.class);
      IJcrProvider jcr = (IJcrProvider) cn
          .newInstance(context.getWiki().getConfig(), context);
      this.jcr = jcr;
    }
  }

  protected Node getDocNodeById(XWikiJcrSession session, long docId)
  ...
  protected String getDocPathById(XWikiContext context, final long docId) throws Exception {
    return (String) executeRead(context, ses -> {
  ...
  public void cleanupWiki(String workspace) throws XWikiException {
    try {
      ...
      throw new XWikiException(XWikiException.MODULE_XWIKI_STORE,
          ...
          "Exception while cleanupWiki(" + workspace + ")", e);
    }
  }
  ...
  private XWikiJcrSession getWriteSession(XWikiContext context)
