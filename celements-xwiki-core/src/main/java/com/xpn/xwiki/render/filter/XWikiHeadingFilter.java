...
public class XWikiHeadingFilter extends LocaleRegexTokenFilter implements CacheFilter {
  ...
  public String handleMatch(MatchResult result, FilterContext context) {
    ...
    XWikiContext xcontext = ((XWikiRadeoxRenderEngine) rcontext.getRenderEngine())
        .getXWikiContext();
      ...
      try {
        ...
        if ((doc != null)
            && ((xcontext.getUser() != null)
                && xcontext.getWiki().getRightService().hasAccessLevel("edit",
    ...
    if (showEditButton) {
      if ((beforeAction != null) && !beforeAction.toString().equals("HeadingFilter")) {
      ...
      if ((level.equals("1") || level.equals("1.1"))
          && ((doc != null) && (doc.getContent().indexOf(title.trim()) != -1))) {
