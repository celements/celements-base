...
public class DefaultXWikiRenderingEngine implements XWikiRenderingEngine {
  ...
  private final String[] defaultRenderingOrder = { "macromapping", "groovy",
      "velocity", "plugin", "wiki", "wikiwiki" };

  private List<XWikiRenderer> renderers = new ArrayList<>();

  private HashMap<String, XWikiRenderer> renderermap = new LinkedHashMap<>();
  ...
  public DefaultXWikiRenderingEngine(XWiki xwiki, XWikiContext context) throws XWikiException {
    ...
    for (String element : renderingOrder) {
      if (xwiki.Param("xwiki.render." + element, "1").equals("1")) {

        if (element.equals("macromapping")) {
          addRenderer("mapping", new XWikiMacrosMappingRenderer(xwiki, context));

        } else if (element.equals("velocity")) {
          addRenderer("velocity", new XWikiVelocityRenderer());

        } else if (element.equals("groovy")) {
          addRenderer("groovy", new XWikiGroovyRenderer());

        } else if (element.equals("plugin")) {
          addRenderer("plugin", new XWikiPluginRenderer());

        } else if (element.equals("wiki")) {
          addRenderer("wiki", new XWikiRadeoxRenderer(false));

        } else if (element.equals("wikiwiki")) {
      ...
      try {
        ...
        try {
          for (XWikiRenderer element : this.renderers) {
            XWikiRenderer renderer = (element);
        ...
        try {
          ...
          if (cacheDuration > 0) {
            XWikiRenderingCache cacheObject = new XWikiRenderingCache(key, content, cacheDuration,
                new Date());
  ...
  private String getKey(String text, XWikiDocument contentdoc, XWikiDocument includingdoc,
      XWikiContext context) {
    String qs = (((context == null) || (context.getRequest() == null)) ? ""
    ...
    String cdoc = ((contentdoc == null) ? ""
        : contentdoc.getDatabase() + ":" + contentdoc.getFullName() + ":"
            + contentdoc.getRealLanguage() + ":" + contentdoc.getVersion());
    String idoc = ((includingdoc == null) ? ""
        : includingdoc.getDatabase() + ":" + includingdoc.getFullName() + ":"
