...
/**
 * Redefinition of the Java code formatter ({@code org.radeox.macro.code.JavaCodeFilter}) from
 * Radeox. This was needed
 * because of the inefficient regular expression for detecting string literals, which caused
...
public class XWikiJavaCodeFilter extends DefaultRegexCodeFormatter implements SourceCodeFormatter {
   ...
   */
  private static final String OBJECTS = "\\b(Boolean|Byte|Character|Class|ClassLoader|Cloneable|Compiler|Double|Float|Integer|Long|Math|"
  ...
  /**
   * Regular expression for matching string literals. Fixed to prevent exponential explosion after
   * an unclosed quote.
   */
  ...
  public int getPriority() {
    // Return a larger priority, so that our code formatter replaces the default formatter provided
    // by Radeox.
    return 20;
  }
}
