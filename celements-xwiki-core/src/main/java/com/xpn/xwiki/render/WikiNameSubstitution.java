...
public class WikiNameSubstitution extends WikiSubstitution {
   ...
   * s/([\*\s][\(\-\*\s]*)([A-Z]+[a-z]*)\.([A-Z]+[a-z]+[A-Z]+[a-zA-Z0-9]*)(\#[a-zA-Z0-9_]*)/&
   * internalLink($1,$2,$3,"$TranslationToken$3$4$TranslationToken",$4,1)/geo; #
   * 'Web.TopicName' link:
   * s/([\*\s][\(\-\*\s]*)([A-Z]+[a-z]*)\.([A-Z]+[a-z]+[A-Z]+[a-zA-Z0-9]*)/&internalLink($1,$2,$3,
   * "$TranslationToken$3$TranslationToken","",1)/geo; #
   * 'TopicName#anchor' link:
   * s/([\*\s][\(\-\*\s]*)([A-Z]+[a-z]+[A-Z]+[a-zA-Z0-9]*)(\#[a-zA-Z0-9_]*)/&internalLink($1,$theWeb
   * ,$2,"$TranslationToken$2$3$TranslationToken",$3,1)/geo; #
   * 'TopicName' link:
    ...
    switch (getType()) {
      case TYPE_ONE:
        XWikiWikiBaseRenderer.internalLink(stringBuffer, matchResult.group(1), matchResult.group(2),
            matchResult.group(3), matchResult.group(3) + matchResult.group(4), matchResult.group(4),
            true,
            getContext(), getUtil());
        break;
      case TYPE_TWO:
        XWikiWikiBaseRenderer.internalLink(stringBuffer, matchResult.group(1), matchResult.group(2),
            matchResult.group(3), matchResult.group(3), matchResult.group(4), true, getContext(),
            getUtil());
        break;
      case TYPE_THREE:
        XWikiWikiBaseRenderer.internalLink(stringBuffer, matchResult.group(1), getDoc().getSpace(),
            matchResult.group(2), matchResult.group(2) + matchResult.group(3), matchResult.group(3),
            true,
            getContext(), getUtil());
        break;
      case TYPE_FOUR:
  ...
  public String substitute(String line, int type) {
    ...
    return super.substitute(line); // To change body of overriden methods use Options | File
                                   // Templates.
  }
