...
public class Utils {
  ...
  private static final String PLACEHOLDERS_CONTEXT_KEY = Utils.class.getCanonicalName()
      + "_placeholders";
   ...
   *
   * @param template
   *          The name of the template to parse, without the {@code .vm} prefix. The template will
   *          be searched
   *          in the usual places: current XWikiSkins object, attachment of the current skin
   *          document, current skin
   *          folder, baseskin folder, /templates/ folder.
   * @param context
   *          the current context
   * @throws XWikiException
   *           when the response cannot be written to the client (for example when the client
   *           canceled
  ...
  /**
   * Generate the response by parsing a velocity template and (optionally) printing the result to
   * the
   ...
   *          folder, baseskin folder, /templates/ folder.
   * @param write
   *          Whether the generated response should be written to the client or not. If
   *          {@code false}, only the
  ...
  public static void parseTemplate(String template, boolean write, XWikiContext context)
    ...
    if ((("download".equals(action)) || ("skin".equals(action)))
        && (context.getWiki().getXWikiPreferenceAsLong("headers_nocache", 1, context) != 0)) {
      response.setHeader("Cache-Control", "no-cache");
    }

    context.getWiki().getPluginManager().beginParsing(context);
    // This class allows various components in the rendering chain to use placeholders for some
    // fragile data. For
    // example, the URL generated for the image macro should not be further rendered, as it might
    // get broken by wiki
    // filters. For this to work, keep a map of [used placeholders -> values] in the context, and
    // replace them when
  ...
  /**
   * Retrieve the URL to which the client should be redirected after the successful completion of
   * the requested
   * action. This is taken from the {@code xredirect} parameter in the query string. If this
   * parameter is not set, or
   * is set to an empty value, return the default redirect specified as the second argument.
   *
   * @param request
   *          the current request
   * @param defaultRedirect
   *          the default value to use if no {@code xredirect} parameter is present
   * @return the destination URL, as specified in the {@code xredirect} parameter, or the specified
   *         default URL
   */
   ...
   * action. This is taken from the {@code xredirect} parameter in the query string. If this
   * parameter is not set, or
   * is set to an empty value, compose an URL back to the current document, using the specified
   * action and query
   * string, and return it.
   *
   * @param action
   *          the XWiki action to use for composing the default redirect URL ({@code view},
   *          {@code edit}, etc)
   * @param queryString
   *          the query parameters to append to the fallback URL
   * @param context
   *          the current context
   * @return the destination URL, as specified in the {@code xredirect} parameter, or computed using
   *         the current
  ...
  /**
   * Retrieve the name of the velocity template which should be used to generate the response. This
   * is taken from the
   * {@code xpage} parameter in the query string. If this parameter is not set, or is set to an
   * empty value, return
   ...
   *          the current request
   * @param defaultpage
   *          the default value to use if no {@code xpage} parameter is set
   * @return the name of the requested template, as specified in the {@code xpage} parameter, or the
   *         specified default
   *         template
   */
   ...
   *
   * @param filelist
   *          the list of uploaded files, computed by the FileUpload plugin
   * @param name
   *          the name of the form field
   * @return the original name of the file, if the specified field name does correspond to an
   *         uploaded file, or
   *         {@code null} otherwise
   */
   ...
   * @param name
   *          the name of the form field
   * @return the content of the file, if the specified field name does correspond to an uploaded
   *         file, or {@code null}
   *         otherwise
   * @throws XWikiException
   ...
   * NOTE</strong>: URL decoding is performed individually on the parsed name and value elements,
   * rather than on the
   * entire query string ahead of time, to properly deal with the case where the name or value
   * includes an encoded "="
   ...
   * @param encoding
   *          the encoding to use for transforming bytes into characters
   ...
   *
   * @param data
   *          input byte array containing request parameters
   * @param encoding
  ...
  /**
   * Put name-value pair in map. When an entry for {@code name} already exist, add the new value to
   * the array of
   * values.
   * <p>
   * Code borrowed from Apache Tomcat 5.0
   ...
   *
   * @param map
   *          the map that is being constructed
   * @param name
   ...
   * @deprecated starting with 2.7 use {@link XMLScriptService#escape(Object)
  ...
  /**
   * @deprecated replaced by {@link com.xpn.xwiki.util.Util#encodeURI(String, XWikiContext)} since
   *             1.3M2
   */
  @Deprecated
  ...
  /**
   * @deprecated replaced by {@link com.xpn.xwiki.util.Util#decodeURI(String, XWikiContext)} since
   *             1.3M2
   */
  @Deprecated
  ...
  public static FileUploadPlugin handleMultipart(HttpServletRequest request, XWikiContext context) {
    ...
    try {
      ...
      if ((e instanceof XWikiException)
          && (((XWikiException) e)
              .getCode() == XWikiException.ERROR_XWIKI_APP_FILE_EXCEPTION_MAXSIZE)) {
  ...
  /**
   * @return the component manager used by {@link #getComponent(Class)} and
   *         {@link #getComponent(Class, String)}
   */
  ...
  /**
   * Create a placeholder key for a string that should be protected from further processing. The
   * value is stored in
   * the context, and the returned key can be used by the calling code as many times in the
   * rendering result. At the
   ...
   *
   * @param content
   *          The rendered content, with placeholders.
   * @param context
   *          The current context.
