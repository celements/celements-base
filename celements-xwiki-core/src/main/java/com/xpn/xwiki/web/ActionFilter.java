...
/**
 * <p>
 * A filter that dispatches requests to the right action, depending on the button that was pressed
 * in the editing form.
 * This is needed since the browser cannot send the form data to different URLs depending on the
 * button pressed, and an
 * XHTML form has only one target URL. In previous versions of XWiki this was accomplished using
 * javascript code, with a
 * fall-back on a pseudo-dispatcher inside the {@link PreviewAction}, which was on obvious case of
 * bad code design.
 * </p>
 * <p>
 * The filter dispatches requests based on the presence of a request parameter starting with
 * <tt>action_</tt> followed
 * by the name of the struts action that should actually process the request. For example, the
 * button that does
...
public class ActionFilter implements Filter {
  ...
  /**
   * The name of the request attribute that specifies if the action has been already dispatched.
   * This flag is required
   * to prevent recursive dispatch loop and allows us to map this filter to INCLUDE and FORWARD. The
   * value of this
   * request attribute is a string. The associated boolean value is determined using
      ...
      while (parameterNames.hasMoreElements()) {
        ...
        if (parameter.startsWith(ACTION_PREFIX)) {
          ...
          if (dispatcher != null) {
            ...
            // If the request was forwarder to another path, don't continue the normal processing
            // chain.
            return;
          }
        }
  ...
  /**
   * Compose a new URL path based on the original request and the specified action. The result is
   * relative to the
   * application context, so that it can be used with
   * {@link HttpServletRequest#getRequestDispatcher(String)}. For
   * example, calling this method with a request for <tt>/xwiki/bin/edit/Some/Document</tt> and
   * <tt>action_save</tt>,
   ...
   * @return The rebuilt URL path, with the specified action in place of the original Struts action.
   *         Note that unlike
  ...
  private String getTargetURL(HttpServletRequest request, String action) {
    ...
    // Extract the document name from the requested path. We don't use getPathInfo() since it is
    // decoded
    ...
    // Third step, remove the struts mapping. This step is mandatory, so this filter will fail if
    // the
    ...
    if ("1".equals(
        XWikiConfigurationService.getProperty("xwiki.virtual.usepath", "0", this.servletContext))
        && servletPath.equals(PATH_SEPARATOR
