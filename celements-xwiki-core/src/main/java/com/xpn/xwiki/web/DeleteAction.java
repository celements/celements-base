...
public class DeleteAction extends XWikiAction {
  ...
  public boolean action(XWikiContext context) throws XWikiException {
    ...
    // If confirm=1 then delete the page. If not, the render action will go to the "delete" page so
    // that the
    ...
    if ((sindex != null) && xwiki.hasRecycleBin(context)) {
      ...
      XWikiDeletedDocument dd = xwiki.getRecycleBinStore().getDeletedDocument(doc, index, context,
          true);
      // If the document hasn't been previously deleted (i.e. it's not in the deleted document
      // store) then
      ...
      // Redirect the user to the view template so that he gets the "document doesn't exist" dialog
      // box.
      sendRedirect(response, Utils.getRedirect("view", context));
      redirected = true;
    } else {
    ...
    if (!redirected) {
      // If a xredirect param is passed then redirect to the page specified instead of going to the
      // default
      // confirmation page.
  ...
  public String render(XWikiContext context) throws XWikiException {
    ...
    if (sindex != null) {
      long index = Long.parseLong(sindex);
      if (context.getWiki().getRecycleBinStore().getDeletedDocument(doc, index, context,
          true) != null) {
