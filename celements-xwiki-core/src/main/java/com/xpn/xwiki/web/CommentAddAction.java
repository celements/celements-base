...
/**
 * Action used to post a comment on a page, adds a comment object to the document and saves it,
 * requires comment right
 * but not edit right.
 *
 * @version $Id$
 */
public class CommentAddAction extends XWikiAction {
  ...
  public boolean action(XWikiContext context) throws XWikiException {
    ...
    if (doc.isNew()) {
      return true;
    } else if (context.getUser().equals(XWikiRightService.GUEST_USER_FULLNAME)
        && !checkCaptcha(context)) {
      ...
      // ObjectAddAction, so that properties missing from the request are still added to the
      // database.
      ...
      // if contentDirty is false, in order for the change to create a new version metaDataDirty
      // must be true.
      doc.setMetaDataDirty(true);
      xwiki.saveDocument(doc, context.getMessageTool().get("core.comment.addComment"), true,
          context);
    }
  ...
  /**
   * Checks the request parameter captcha_answer against the captcha module. This makes xwiki-core
   * dependant on
   * xwiki-captcha and should be removed as soon as possible.
   *
   * @param context
   *          The XWikiContext for getting the request and whether guest comment requires a captcha.
   * @return true if the captcha answer is correct or if no captcha answer and captcha is not
   *         required.
   * @throws XWikiException
   *           if something goes wrong in the captcha module.
   * @since 2.3M1
   */
  private boolean checkCaptcha(XWikiContext context) throws XWikiException {
    ...
    if ((answer != null) && (answer.length() > 0)) {
      org.xwiki.captcha.CaptchaVerifier cv = Utils.getComponent(
          org.xwiki.captcha.CaptchaVerifier.class, context.getRequest().get("captcha_type"));
      try {
      ...
      return (context.getWiki().getSpacePreferenceAsInt("guest_comment_requires_captcha", 0,
          context) != 1);
    }
  }
}
