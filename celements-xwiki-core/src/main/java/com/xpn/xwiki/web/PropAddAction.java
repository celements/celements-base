...
public class PropAddAction extends XWikiAction {
  ...
  public boolean action(XWikiContext context) throws XWikiException {
    ...
    if (bclass.get(propName) != null) {
      // TODO: handle the error of the property already existing when we want to add a class
      // property
    } else {
      ...
      if (pmclass != null) {
        ...
        xwiki.saveDocument(doc, context.getMessageTool().get("core.comment.addClassProperty"), true,
            context);
      }
    }
    // forward to edit
