 ...
 * looked for and the
 * returned value is the message in the language requested. There are 3 sources where properties are
 * looked for (in the
 * specified order):
 * <ol>
 ...
 * value. In addition
 * the property values are cached for better performance but if one of the XWiki documents
 * containing the properties is
...
public class XWikiMessageTool {
  ...
  /**
   * The default Resource Bundle to fall back to if no document bundle is found when trying to get a
   * key.
   */
   ...
   * used so that we can
   * reload them if they've been modified since last time they were cached. The map is of type
   * (Long, Date) where Long
   * is the XWiki document ids.
   */
  private Map<Long, Date> previousDates = new HashMap<>();

  /**
   * List of document bundles that have been modified since the last time they were cached. The Set
   * contains Long
   * objects which are the XWiki document ids. TODO: This instance variable should be removed as
  ...
  /**
   * @param bundle
   *          the default Resource Bundle to fall back to if no document bundle is found when trying
   *          to get a key
   * @param context
   *          the {@link com.xpn.xwiki.XWikiContext} object, used to get access to XWiki primitives
   *          for loading
   *          documents
   */
   ...
   * <p>
   * Note: The reason we're using a List instead of an Object array is because we haven't found how
   * to easily create
   * an Array in Velocity whereas a List is easily created. For example:
