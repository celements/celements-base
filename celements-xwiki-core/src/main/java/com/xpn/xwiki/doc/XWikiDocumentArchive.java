...
public class XWikiDocumentArchive {
   ...
   * @param doc
   *          - document for that patch created
   * @param context
   ...
   *          - version comment
   * @param version
   *          - preferably document version in history
   * @param context
  ...
  public void updateArchive(XWikiDocument doc, String author, Date date, String comment,
    ...
    if ((newVer == null)
        || ((oldLatestVer != null) && (newVer.compareVersions(oldLatestVer) <= 0))) {
  ...
  public void removeVersions(Version newerVersion, Version olderVersion, XWikiContext context)
    ...
    for (Iterator<XWikiRCSNodeInfo> it = getNodes(upperBound, lowerBound).iterator(); it
        .hasNext();) {
  ...
  public String getVersionXml(Version version, XWikiContext context) throws XWikiException {
    ...
    List<XWikiRCSNodeContent> lstContent = loadRCSNodeContents(nearestFullVersion, version,
        context);
   ...
   *           if any error
   */
  private List<XWikiRCSNodeContent> loadRCSNodeContents(Version vfrom, Version vto,
