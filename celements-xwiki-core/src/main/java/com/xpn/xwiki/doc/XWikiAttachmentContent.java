...
/**
 * The content of an attachment. Objects of this class hold the actual content which will be
 * downloaded when a user
...
public class XWikiAttachmentContent implements Cloneable {
  ...
  /**
   * True if the content is out of sync with the content stored in the database and thus needs to be
   * saved.
   */
   ...
   *
   * @param original
   *          the XWikiAttachmentContent to clone.
   * @since 2.6M1
   */
  ...
  /**
   * The default Constructor. For creating content which will be associated with an attachment
   * later.
   */
  ...
  private void newFileItem() {
    ...
    try {
      final DiskFileItem dfi = new DiskFileItem(null, null, false, null, 10000,
          new File(tempFileLocation));
  ...
  /**
   * This is used so that Hibernate will associate this content with the right attachment
   * (metadata).
   *
  ...
  /**
   * This function does nothing and exists only for Hibernate to be able to load a value which is
   * not used.
   *
   * @param id
   *          is ignored.
  ...
  public void setId(long id) {
    // Do nothing here.
    // The id is taken from the attachment which is set in
    // XWikiHibernateAttachmentStore#loadAttachmentContent.
  }
   ...
   *
   * @param contentDirty
   *          if true then the content is regarded as out of sync with the database and in need of
   *          saving,
