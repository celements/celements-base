...
public class XWikiLDAPConfigTest extends AbstractBridgedXWikiComponentTestCase {
  ...
  private static final Map<String, String> RESULT_CFG_USERMAPPING = new Hashtable<>();

  private static final Map<String, String> RESULT_PREF_USERMAPPING = new Hashtable<>();

  private static final Map<String, Set<String>> RESULT_CFG_GROUPMAPPING = new Hashtable<>();

  private static final Map<String, Set<String>> RESULT_PREF_GROUPMAPPING = new Hashtable<>();

  private static final Collection<String> RESULT_CFG_GROUPCLASSES = new HashSet<>();

  private static final Collection<String> RESULT_PREF_GROUPCLASSES = new HashSet<>();

  private static final Collection<String> RESULT_CFG_GROUPMEMBERFIELDS = new HashSet<>();

  private static final Collection<String> RESULT_PREF_GROUPMEMBERFIELDS = new HashSet<>();
  ...
  static {
    ...
    addProperty("ldap_groupcache_expiration", "xwiki.authentication.ldap.groupcache_expiration",
        "10000", "11111");
    ...
    addProperty("ldap_mode_group_sync", "xwiki.authentication.ldap.mode_group_sync", "always",
        "create");

    addProperty("ldap_fields_mapping", "xwiki.authentication.ldap.fields_mapping",
        "name=uid,last_name=sn",
    ...
    addProperty("ldap_group_mapping", "xwiki.authentication.ldap.group_mapping",
        XADMINGROUP_FULLNAME + "="
            + LDAPTOTOGRP_DN + "|" + XADMINGROUP_FULLNAME + "=" + LDAPTITIGRP_DN + "|"
            + XADMINGROUP2_FULLNAME + "="
            + LDAPTOTOGRP_DN + "|" + XADMINGROUP2_FULLNAME + "=" + LDAPTITIGRP_DN,
        XADMINGROUP_FULLNAME + "="
    ...
    Set<String> ldapgroups2 = new HashSet<>();
    ...
    addProperty("ldap_group_classes", "xwiki.authentication.ldap.group_classes",
        "groupclass1,groupclass2",
        "groupclass12");
  ...
  protected void setUp() throws Exception {
    ...
    XWikiConfig cfg = new XWikiConfig();
    new XWiki(this.prefContext, null) {
    ...
    new XWiki(this.cfgContext, null) {
  ...
  public void testGetLDAPParam1() {
    assertEquals("0",
        XWikiLDAPConfig.getInstance().getLDAPParam("ldap", "xwiki.authentication.ldap", null,
            prefContext));
    assertEquals("1",
        XWikiLDAPConfig.getInstance().getLDAPParam("ldap", "xwiki.authentication.ldap", null,
            cfgContext));
  ...
  public void testGetLDAPParam2() {
    assertEquals("localhost",
        XWikiLDAPConfig.getInstance().getLDAPParam("ldap_server", null, prefContext));
    assertEquals("127.0.0.1",
        XWikiLDAPConfig.getInstance().getLDAPParam("ldap_server", null, cfgContext));
    assertEquals("127.0.0.1",
  ...
  public void testGetGroupMappings() {
    Map<String, Set<String>> prefMapping = XWikiLDAPConfig.getInstance()
        .getGroupMappings(prefContext);

    assertEquals(RESULT_PREF_GROUPMAPPING, prefMapping);

    Map<String, Set<String>> cfgMapping = XWikiLDAPConfig.getInstance()
        .getGroupMappings(cfgContext);

    assertEquals(RESULT_CFG_GROUPMAPPING, cfgMapping);
  }

  public void testGetUserMappings() {
    List<String> prefAttrList = new ArrayList<>();

    Map<String, String> prefMapping = XWikiLDAPConfig.getInstance().getUserMappings(prefAttrList,
        prefContext);
    ...
    Map<String, String> cfgMapping = XWikiLDAPConfig.getInstance().getUserMappings(cfgAttrList,
        cfgContext);
  ...
  public void testGetGroupClasses() {
    assertEquals(RESULT_PREF_GROUPCLASSES,
        XWikiLDAPConfig.getInstance().getGroupClasses(prefContext));
    assertEquals(RESULT_CFG_GROUPCLASSES,
        XWikiLDAPConfig.getInstance().getGroupClasses(cfgContext));
  }

  public void testGetGroupMemberFields() {
    assertEquals(RESULT_PREF_GROUPMEMBERFIELDS,
        XWikiLDAPConfig.getInstance().getGroupMemberFields(prefContext));
    assertEquals(RESULT_CFG_GROUPMEMBERFIELDS,
