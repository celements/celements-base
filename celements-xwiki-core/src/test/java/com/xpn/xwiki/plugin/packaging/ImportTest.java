...
public class ImportTest extends AbstractBridgedXWikiComponentTestCase {
  ...
  public void testImportDocument() throws Exception {
    ...
    byte[] zipFile = this.createZipFile(new XWikiDocument[] { doc1 },
    ...
    XWikiDocument nonExistingDocument = this.xwiki.getDocument("Test.DocImportNonexisting",
        getContext());
    assertTrue(nonExistingDocument.isNew());

    XWikiDocument foundTranslationDocument = foundDocument.getTranslatedDocument("fr",
        getContext());
  ...
  public void testImportOverwriteDocument() throws Exception {
    ...
    zipFile = this.createZipFile(new XWikiDocument[] { overwritingDoc },
        new String[] { "ISO-8859-1" });
    ...
    XWikiDocument foundOverwritingDoc = this.xwiki.getDocument("Test.DocImportOverwrite",
        getContext());
  ...
  private String getPackageXML(XWikiDocument docs[]) {
    ...
    for (XWikiDocument doc : docs) {
      ...
      sb.append("<file defaultAction=\"0\" language=\"" + doc.getLanguage() + "\">"
          + doc.getFullName()
          + "</file>\n");
    }
