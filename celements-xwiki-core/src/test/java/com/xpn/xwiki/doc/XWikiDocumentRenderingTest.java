...
public class XWikiDocumentRenderingTest extends AbstractBridgedXWikiComponentTestCase {
  ...
  public void testGetRenderedTitleWithTitle() throws XWikiException {
    ...
    assertEquals("<strong>title</strong>",
    ...
    this.mockXWikiRenderingEngine.stubs().method("interpretText")
        .with(eq("#set($key = \"title\")$key"), ANYTHING,
            ANYTHING)
  ...
  public void testGetRenderedTitleWithoutTitleHTML() throws XWikiException {
    ...
    assertEquals(
        "<span class=\"wikicreatelink\"><a href=\"/reference\"><span class=\"wikigeneratedlinkcontent\">"
            + "Page" + "</span></a></span>",
        this.document.getRenderedTitle(Syntax.XHTML_1_0, getContext()));

    this.document
        .setContent("content not in section\n" + "= #set($var ~= \"value\")=\nheader 1 content\n"
            + "== header 2==\nheader 2 content");
  ...
  public void testGetRenderedContentCleansVelocityMacroCache() throws Exception {
    ...
    this.baseObject.setLargeStringValue("area",
        "{{velocity}}#macro(testmacrocache)ok#end{{/velocity}}");
    ...
    VelocityEngine vengine = getComponentManager().lookup(VelocityFactory.class)
