...
/**
 * Implementation of the {@link ServletContextListener}. Initializes component manager and
 * application context.
 *
 * @version $Id$
 */
public class XWikiServletContextListener implements ServletContextListener {
  ...
  public void contextInitialized(ServletContextEvent servletContextEvent) {
    ...
    // Use a Component Event Manager that stacks Component instance creation events till we tell it
    // to flush them.
    // The reason is that the Observation Manager used to send the events but we need the
    // Application Context to
    // be set up before we start sending events since there can be Observation Listener components
    // that require
    // the Application Context (this is the case for example for the Office Importer Lifecycle
    // Listener).
    ...
    try {
      ServletContainerInitializer containerInitializer = this.componentManager
    ...
    // Send an Observation event to signal the XWiki application is started. This allows components
    // who need to do
    ...
    // Now that the Application Context is set up, send the Component instance creation events we
    // had stacked up.
  ...
  public void contextDestroyed(ServletContextEvent sce) {
    // Send an Observation event to signal the XWiki application is stopped. This allows components
    // who need to do
    // something on stop to do it.
    try {
      ObservationManager observationManager = this.componentManager
          .lookup(ObservationManager.class);
    ...
    try {
      ApplicationContextListenerManager applicationContextListenerManager = this.componentManager
      ...
      applicationContextListenerManager
          .destroyApplicationContext(container.getApplicationContext());
