 ...
 * this class is a
 * workaround. It manually constructs an <strong>array of uberspectors</strong>, loading the classes
 * in the order
 * defined in the <code>"runtime.introspector.uberspect.arrayClasses"</code> property, and after
 * that forwards calls to
 * each of the uberspectors, in order, until one of them returns something different than
 * <code>null</code>. Note that
 * the calls will be made from the leftmost class to the rightmost one. This allows building and
 * combining different
 * small uberspectors that perform a specialised search for methods, instead of bloating a single
 * class with different
 * introspection tricks.
 * </p>
 * <p>
 * This is not actually part of the array, but is more of a handle that allows the calls intended
 * for only one
 * uberspector to reach the array. It duplicates some of the code from the velocity runtime
 ...
 * <code>runtime.introspector.uberspect.arrayClasses</code>. This
 ...
 * @version $Id$
 */
public class LinkingUberspector extends UberspectImpl
  ...
  public void init() {
    ...
    // Create the array
    String[] arrayClassnames = this.runtime.getConfiguration()
        .getStringArray(UBERSPECT_ARRAY_CLASSNAMES);
  ...
  /**
   * Instantiates and initializes an uberspector class and adds it to the array. Also set the log
   * and runtime
