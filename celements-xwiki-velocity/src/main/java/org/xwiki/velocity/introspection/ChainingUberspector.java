...
/**
 * <p>
 * Since the current version of the Velocity engine (1.5) does not allow more than one uberspector,
 * this class is a
 * workaround. It manually constructs a <strong>chain of uberspectors</strong>, loading the classes
 * in the order defined
 * in the <code>"runtime.introspector.uberspect.chainClasses"</code> property, and after that simply
 * forwarding all
 * calls to the top of the chain. Note that the calls will be made from the rightmost class to the
 * leftmost one. Along
 * the chain, each uberspectors can forward the call to the rest of the chain, build its own result,
 * and/or process in
 * any way the resulting value. This allows uberspectors to enhance the list of returned methods,
 * block out methods
 * returned by other uberspectors, or take various actions on the returned method (for example add
 * or remove parameters
 * before searching the method, log executed method names, or catch exceptions when executing those
 * methods).
 * </p>
 * <p>
 * This is not actually part of the chain, but is more of a handle that allows the calls intended
 * for only one
 * uberspector to reach the chain. It duplicates some of the code from the velocity runtime
 * initialization code, hoping
 * that a future version of the engine will support chaining natively.
 * </p>
 * <p>
 * The chain is defined using the configuration parameter
 * <code>runtime.introspector.uberspect.chainClasses</code>. This
 * property should contain a list of canonical class names. Any wrong entry in the list will be
 * ignored. If this
 * property is not defined or contains only wrong classnames, then by default a
 * <code>SecureUberspector</code> is used
 * as the only entry in the chain. The first (leftmost) uberspector does not have to be chainable
 * (as it will not need
 * to forward calls). If a uberspector in the middle of the chain is not chainable, then it will
 * break the chain at that
...
public class ChainingUberspector extends AbstractChainableUberspector
  ...
  public void init() {
    ...
    String[] chainClassnames = this.runtime.getConfiguration()
        .getStringArray(UBERSPECT_CHAIN_CLASSNAMES);
   ...
   *
   * @param classname
   *          The name of the uberspector class to instantiate.
   * @return An instance of the specified Uberspector. If the class cannot be instantiated using the
   *         default
  ...
  protected Uberspect instantiateUberspector(String classname) {
    ...
    try {
      ...
      this.log
          .warn(String.format("The specified uberspector [%s] cannot be instantiated.", classname));
    } catch (ExceptionInInitializerError e) {
      this.log.warn(
          String.format("Exception while instantiating the Uberspector [%s]: %s", classname,
              e.getMessage()));
    }
