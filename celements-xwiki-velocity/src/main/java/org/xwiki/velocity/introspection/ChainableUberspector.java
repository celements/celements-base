...
public interface ChainableUberspector extends Uberspect {
  ...
  /**
   * Since uberspectors are dynamically enabled using a configuration string, we cannot simply call
   * the constructors
   * to pass the inner uberspector. We can either instantiate uberspectors using reflection and
   * passing the wrapped
   * object to the constructor, or explicitely "wrap" the inner uberspector. The second method has
   * several advantages,
