 ...
 * if the returned
 * method has a Deprecated annotation. Because this is a chainable uberspector, it has to re-get the
 * method using a
 * default introspector, which is not safe; future uberspectors might not be able to return a
 * precise method name, or a
...
public class DeprecatedCheckUberspector extends AbstractChainableUberspector
    implements Uberspect, ChainableUberspector, UberspectLoggable {
  ...
  public VelPropertySet getPropertySet(Object obj, String identifier, Object arg, Info i)
    ...
    if (method != null) {
      Method m = introspector.getMethod(obj.getClass(), method.getMethodName(),
          new Object[] { arg });
      if (m != null && m.isAnnotationPresent(Deprecated.class)) {
   ...
   *
   * @param deprecationType
   *          the type of deprecation (eg "getter", "setter", "method")
   * @param object
   *          the object that has a deprecation
   * @param methodName
   *          the deprecated method's name
   * @param info
   *          a Velocity {@link org.apache.velocity.util.introspection.Info} object containing
   *          information about
