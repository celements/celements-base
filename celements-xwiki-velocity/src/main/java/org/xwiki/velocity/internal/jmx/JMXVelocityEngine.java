...
/**
 * Uses non-stable (ie might need to be modified when we upgrade the Velocity JAR) introspection to
 * access private
 * fields of Velocity. This is needed since Velocity doesn't offer a way to access information about
 * the template
...
public class JMXVelocityEngine implements JMXVelocityEngineMBean {
  ...
  public TabularData getTemplates() {
    ...
    try {
      ...
      // Note: JDK 1.6 has the notion of MXBean which support returning a Map type but since we must
      // use
      ...
      String[] descriptions = new String[] { "The Template Name (namespace)",
          "The names of registered Macros" };
      ...
      TabularType type = new TabularType("templates",
          "Template management data (namespaces, macros)", rowType,
          columnNames);
  ...
  /**
   * @return the data using standard Java classes, {@link #getTemplates()} wraps it in generic Open
   *         types to make the
   ...
   * @see #getTemplates()
   */
  private Map<String, String[]> getInternalTemplates()
    ...
    Map<String, Map<String, ?>> namespaceHash = (Map<String, Map<String, ?>>) getField(vmManager,
        "namespaceHash");
