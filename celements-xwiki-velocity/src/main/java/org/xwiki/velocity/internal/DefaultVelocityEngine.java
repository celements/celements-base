 ...
 * defined in the component's configuration. Note that the {@link #initialize} method has to be
 * executed before any
...
public class DefaultVelocityEngine implements VelocityEngine, LogChute {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(DefaultVelocityEngine.class);

  /**
   * The name of the Velocity configuration property that specifies the ResourceLoader that Velocity
   * should use when
  ...
  /**
   * The Container component. We need it in order to store the ServletContext as a property in the
   * Application Context
  ...
  private final Map<String, Integer> namespaceUsageCount = new HashMap<>();
  ...
  public void initialize(Properties properties) throws XWikiVelocityException {
    ...
    String resourceLoader = properties.getProperty(RESOURCE_LOADER,
        velocityProperties.getProperty(RESOURCE_LOADER));
    ...
    if (properties != null) {
      for (Enumeration<?> e = properties.propertyNames(); e.hasMoreElements();) {
        ...
        getEngine().setProperty(key, value);
        LOGGER.debug("Overriding property [{}] = [{}]", key, value);
      }
    }
