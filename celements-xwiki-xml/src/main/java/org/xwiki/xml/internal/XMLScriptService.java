...
public class XMLScriptService implements ScriptService {
    ...
    /**
     * Escapes all the XML special characters in a <code>String</code> using numerical XML entities.
     * Specifically,
     * escapes &lt;, &gt;, ", ' and &amp;.
     * 
     * @param content
    ...
    /**
     * Escapes the XML special characters in a <code>String</code> using numerical XML entities, so
     * that the resulting
    ...
    public String serialize(Node node, boolean withXmlDeclaration) {
        ...
        try {
            ...
            if (withXmlDeclaration && xmlString.startsWith("<?xml")) {
                xmlString = xmlString.replaceFirst("(<\\?xml[^>]*\\?>)(?!\\r?\\n)", "$1\n");
            }
            if (xmlString.contains("<!DOCTYPE")) {
                xmlString = xmlString.replaceFirst("(<!DOCTYPE[^>]*>)(?!\\r?\\n)", "$1\n");
            }
            return xmlString;
        } catch (Exception ex) {
